...
Building and running OpenIFS depends on a set of global environment variables. These variabels are set in `oifs-config.edit_me.sh`, which can be found in the top-level of your OpenIFS package. Here we describe some of the variables and their purpose
...
> `$OIFS_HOME/oifs-config.edit_me.sh` also contains a environment variables that relate to the Single-Column Model (SCM). 