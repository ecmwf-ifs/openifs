...
SUBROUTINE CPEDIA(YDVAB,YDECUMF,YDML_GCONF,KPROMA,KIDIA,KFDIA,KFLEVG,KSATSIM,KLEVSN,&
 ...
 & PDPTYPEOCC6, PDCLBT ,PDCSBT, PDCDRFL, PDCRRFL)
...
!        PFSNNU1  : snow mass per unit area (multi-layer)
!        PRSNU1   : snow density (multi-layer)
!        PVFCVH   : high-vegetation cover
! Output
...
!        PDCDRFL  : Cloudy reflectance                    (output)
!        PDCRRFL  : Clear-sky reflectance                 (output)  
!        --------------------
...
!      P. Lopez     : July-2023   Add ground snow fields for SATSIM visible simulator
!     ------------------------------------------------------------------
...
USE YOM_GRIB_CODES         , ONLY : NGRBMX2T, NGRBMN2T, NGRB10FG, NGRBMXTPR, NGRBMNTPR, &
 &                                  NGRBCLBT, NGRBCSBT, NGRBCAPES, NGRBMXCAPS6, NGRBCDRFL, NGRBCRRFL
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PICH4(KPROMA)
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PFSNNU1(KPROMA,KLEVSN)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PRSNU1(KPROMA,KLEVSN)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PVFCVH(KPROMA)
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PDACH4(KPROMA)
...
REAL(KIND=JPRB) :: ZSNDEP(KPROMA), ZSNRHO(KPROMA), ZSNFRAC(KPROMA)
...
INTEGER(KIND=JPIM) :: ICLBT, ICSBT, IPRECTYPE, ICDRFL, ICRRFL
INTEGER(KIND=JPIM) :: JK, JROF, JGHG, JCHEM, JAERO
REAL(KIND=JPRB) :: ZEPSILON, ZSNMASS, ZSNDEPT
INTEGER(KIND=JPIM) :: IDATE_NOW, ISEC_NOW
...
ELSE
  ...
  DO JROF=KIDIA,KFDIA
    ...
    PDACH4(JROF)=PDACH4(JROF)+(PICH4 (JROF)      )*ZTSTEP    
...
IF (LDMLPP) THEN
  ...
  ICDRFL = ISRCHEQ(NFPPHY,MFPPHY,1,NGRBCDRFL)
  ICRRFL = ISRCHEQ(NFPPHY,MFPPHY,1,NGRBCRRFL)

  IF (KSATSIM > 0 .AND. (ICLBT <= NFPPHY .OR. ICSBT <= NFPPHY .OR. ICDRFL <= NFPPHY .OR. ICRRFL <= NFPPHY)) THEN

    ZEPSILON = 10._JPRB * EPSILON(ZEPSILON)
    DO JROF=KIDIA,KFDIA
      ZSNMASS = SUM(PFSNNU1(JROF,1:KLEVSN))   ! Total snow mass [kg/m2].
      ZSNDEP(JROF) = ZSNMASS * 0.001_JPRB     ! Snow depth [m equiv. water].
      ZSNDEPT = SUM(PFSNNU1(JROF,1:KLEVSN)/PRSNU1(JROF,1:KLEVSN)) ! True snow depth [m].
      IF (ZSNMASS > ZEPSILON .AND. ZSNDEPT > ZEPSILON) THEN
        ZSNRHO(JROF) = ZSNMASS / ZSNDEPT  ! Snow density over whole snow layer [kg/m3].
      ELSE
        ZSNRHO(JROF) = 100._JPRB  ! Default value when no snow.
      ENDIF
     
      ZSNFRAC(JROF)=MAX(MIN(ZSNDEP(JROF) * 10000._JPRB / ZSNRHO(JROF), 1.0_JPRB), 0.0_JPRB) ! Estimate snow cover      
    ENDDO

    ! Compute current date/time.
    CALL DATE_INCR(NINDAT, NSSSSS + NSTEP * INT(ZTSTEP), IDATE_NOW, ISEC_NOW)
    ...
    CALL SATSIM(YDML_GCONF%YGFL,KPROMA,KIDIA,KFDIA,KSATSIM,KFLEVG,&
      ...
      & PDCLBT,PDCSBT,PDCDRFL,PDCRRFL,PSNFRAC=ZSNFRAC,PHVFRAC=PVFCVH,KDATE_NOW=IDATE_NOW)
  ENDIF
ENDIF
