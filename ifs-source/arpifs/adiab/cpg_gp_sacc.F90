...
IF(YRSPEC%LGP) THEN
  CALL GPRT(YDDYNA%LSPRT,NPROMA,KST,KEND,NFLEVG,RD,RV,&
   & PRCP0(1,1,YYTRCP0%M_R),YDVARS%T%T0,YDVARS%T%DL,YDVARS%T%DM,&
...
IF ( LSFORC .AND. (LSW_FRC.OR.LSOMEGA_FRC) ) THEN
  CALL GPCTY_FORC(YDGEOMETRY,YDMODEL%YRML_GCONF,YDMODEL%YRML_PHY_MF%YRPHY2,KST,KEND, &
   & PGFL(1,1,YFORC(1)%MP),PRE0F,YDVARS%T%T0,PRCP0(1,1,YYTRCP0%M_R),PCTY0(1,0,YYTCTY0%M_EVEL),PCTY0(1,1,YYTCTY0%M_VVEL))
ENDIF
...
CALL GPGEO_SACC(NPROMA,KST,KEND,NFLEVG,PHI0,PHIF0,YDVARS%T%T0,YDVARS%U%T0,&
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    PKENE0(JROF,JLEV)=0.5_JPRB*(YDVARS%U%T0(JROF,JLEV)**2+YDVARS%V%T0(JROF,JLEV)**2)  
  ENDDO
ENDDO
...
!*     3.1.13 COMPUTES HALF-LEVEL WINDS.
  ...
  CALL GPHLUV(YDGEOMETRY%YRDIMV,NPROMA,KST,KEND,YDVARS%U%T0,YDVARS%V%T0,PUVH0)
  LLDER=.FALSE.
...
!*     3.1.13 COMPUTES TERM "NHX".
  CALL GPXX(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,ZGPHL,ZGPHM,PHI0FL,PHI0FM,&
   ...
   & YDVARS%U%T0,YDVARS%V%T0,PUVH0(1,0,YYTHW0%M_UH),PUVH0(1,0,YYTHW0%M_VH),&
   & PNHXT0)
!*     3.1.14 COMPUTES dver.
  ...
  IF (YDDYNA%L_RDRY_VD) THEN
    ...
    DO JLEV=1,NFLEVG
      DO JROF=KST,KEND
        ZDVER0(JROF,JLEV)=-(PRCP0(JROF,JLEV,YYTRCP0%M_R)/RD)*(YDVARS%DIV%T0(JROF,JLEV)&
...
CALL GP_TNDLAGADIAB_UV_SACC(YDGEOMETRY,YDGMV,YDMODEL%YRML_PHY_EC%YREPHY,YDMODEL%YRML_DYN%YRDYN,&
 ...
 & ZPSGRTL,ZPSGRTM,&
 & PGWFT0,PHIF0,&
 & YDVARS,PATND(1,1,YDDYNA%YYTTND%M_TNDU),PATND(1,1,YDDYNA%YYTTND%M_TNDV),&
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    PATND(JROF,JLEV,YDDYNA%YYTTND%M_TNDT)=&
     & PRCP0(JROF,JLEV,YYTRCP0%M_KAP)*YDVARS%T%T0(JROF,JLEV)*PCTY0(JROF,JLEV,YYTCTY0%M_VVEL)
  ENDDO
ENDDO
...
!*     3.2.4 COMPUTE "RT".
  ...
  ZR0T9(KST:KEND,1:NFLEVG)=YDVARS%T%T9(KST:KEND,1:NFLEVG)*PRCP0(KST:KEND,1:NFLEVG,YYTRCP0%M_R)
  ...
  ELSE
    ...
    DO JLEV=1,NFLEVG
      DO JROF=KST,KEND
        ZRDT9(JROF,JLEV)=PRCP0(JROF,JLEV,YYTRCP0%M_R)*YDVARS%T%T9(JROF,JLEV)
      ENDDO
    ENDDO
  ENDIF
...
!*     3.2.8 COMPUTES THE GEOPOTENTIAL HEIGHT "gz" AND ITS GRADIENT.
  ...
  IF(LDLDIAB.OR.LSIMPH) THEN
    ...
    CALL GPGEO_SACC(NPROMA,KST,KEND,NFLEVG,PHI9,PHIF9,YDVARS%T%T9,YDVARS%U%T9,&
...
!*     3.2.14 COMPUTES dver.
  ...
  IF (LDGW) THEN
    IF (YDDYNA%L_RDRY_VD) THEN
      ...
      DO JLEV=1,NFLEVG
        DO JROF=KST,KEND
          ZDVER9(JROF,JLEV)=-(PRCP0(JROF,JLEV,YYTRCP0%M_R)/RD)*(YDVARS%DIV%T9(JROF,JLEV)&
