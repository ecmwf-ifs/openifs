...
SUBROUTINE GP_TNDLAGADIAB_UV_SACC(YDGEOMETRY,YDGMV,YDEPHY,YDDYN,KST,KPROF,PRCORI,PGEMU,PGNORDL,PGNORDM,&
 ...
 & YDVARS,PTNDU,PTNDV,PTNDU_NOC,PTNDV_NOC)
...
IF ((YDDYN%LRFRIC .AND. .NOT.LEGWWMS).AND.YDDYN%LRFRICISOTR) THEN
  ...
  DO JLEV=1,NFLEVG
    DO JROF=KST,KPROF
      ZTNDU_RF(JROF,JLEV)=-RKRF(JLEV)*YDVARS%U%T0(JROF,JLEV)
      ZTNDV_RF(JROF,JLEV)=-RKRF(JLEV)*YDVARS%V%T0(JROF,JLEV)
    ENDDO
  ENDDO
ELSEIF ((YDDYN%LRFRIC .AND. .NOT.LEGWWMS).AND..NOT.YDDYN%LRFRICISOTR) THEN
  ...
  ELSE
    ...
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
        ZTNDU_RF(JROF,JLEV)=&
         & -(ZKRFU(JLEV)*PGNORDM(JROF)*PGNORDM(JROF)+ZKRFV(JLEV)*PGNORDL(JROF)*PGNORDL(JROF))*YDVARS%U%T0(JROF,JLEV)&
         & + (ZKRFU(JLEV)-ZKRFV(JLEV))*PGNORDM(JROF)*PGNORDL(JROF)*YDVARS%V%T0(JROF,JLEV)
        ZTNDV_RF(JROF,JLEV)=&
         & (ZKRFU(JLEV)-ZKRFV(JLEV))*PGNORDM(JROF)*PGNORDL(JROF)*YDVARS%U%T0(JROF,JLEV)&
         & -(ZKRFU(JLEV)*PGNORDL(JROF)*PGNORDL(JROF)+ZKRFV(JLEV)*PGNORDM(JROF)*PGNORDM(JROF))*YDVARS%V%T0(JROF,JLEV)
      ENDDO
    ENDDO
  ENDIF
ELSE
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KPROF
    ...
    PTNDU(JROF,JLEV)=PTNDU_NOC(JROF,JLEV)+PRCORI(JROF)*(1._JPRB+ZRTWORGRA*PHIF0(JROF,JLEV))*YDVARS%V%T0(JROF,JLEV)&
        & -ZRGOM*PGWFT0(JROF,JLEV)*(1._JPRB-PGEMU(JROF)*PGEMU(JROF))
    PTNDV_NOC(JROF,JLEV)=-PSGRTM(JROF,JLEV)+ZTNDV_RF(JROF,JLEV)
    PTNDV(JROF,JLEV)=PTNDV_NOC(JROF,JLEV)-PRCORI(JROF)*(1._JPRB+ZRTWORGRA*PHIF0(JROF,JLEV))*YDVARS%U%T0(JROF,JLEV)
  ENDDO
ENDDO
