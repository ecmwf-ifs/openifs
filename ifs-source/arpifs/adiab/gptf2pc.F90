...
SUBROUTINE GPTF2PC(&
 ! --- INPUT ---------------------------------------------------------
 & YDGEOMETRY,YDDYN,YDDYNA,KST,KEN,&
...
IF (YDDYNA%LTWOTL.OR.(.NOT.YDDYNA%LPC_FULL).OR.(NCURRENT_ITER/=0)) THEN
...
IF( NSTEP <= 0 )THEN
  ...
  IF(YDDYNA%LNHDYN)THEN
    ...
    ENDDO
  ENDIF
  IF(YDDYNA%LNHDYN.AND.(YDDYNA%NVDVAR==4.OR.YDDYNA%NVDVAR==5))THEN
    DO JL=1,NFLEVG
      DO JK=KST,KEN
