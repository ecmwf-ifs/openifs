...
SUBROUTINE LAPINEB(YDGEOMETRY,YDGMV,&
 ...
 ! --- OUTPUT -----------------------------------------------------------------
 & PUP9,PVP9,PTP9,PGFLP9,PSPPTCLEARP9)
...
!                        grid-point fields
!                        for split physics (if LDSLPHY=.T. only).
!          PSPPTCLEARP9- Interpolated tendency of clear sky radiation at O.
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PSPPTCLEARP9(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)
!     ------------------------------------------------------------------
...
REAL(KIND=JPRB),POINTER, CONTIGUOUS :: ZGPPC(:,:)
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, &
 ...
 & YDPTRSLB1=>YDMODEL%YRML_DYN%YRPTRSLB1,YDCVER=>YDGEOMETRY%YRCVER, &
 ...
 & YDTSCO=>YDMODEL%YRML_DYN%YYTSCO,YDSPPT_CONFIG=>YDMODEL%YRML_GCONF%YRSPPT_CONFIG)
...
ASSOCIATE(NDIM1=>YGFL%NDIM1, NDIMPC=>YGFL%NDIMPC, NUMFLDS=>YGFL%NUMFLDS, &
 ...
 & RCMSLP0=>YDDYN%RCMSLP0, RW2TLFF=>YDDYN%RW2TLFF, LRHS_CURV=>YDDYN%LRHS_CURV, &
 ...
 & NFGPPC=>YDPTRGPPC%NFGPPC,MGPPCF_SPPTCLEARP=>YDPTRGPPC%MGPPCF_SPPTCLEARP, &
...
LLUPDATE_NHX=YDDYNA%LNHX.AND.(YDDYNA%NVDVAR == 4 .OR. YDDYNA%NVDVAR == 5).AND.(.NOT.YDDYNA%LGWADV.OR.(YDDYNA%ND4SYS==2.AND.NCURRENT_ITER<NSITER))
...
ENDIF
IF (YDDYNA%LNHDYN.AND.YDDYNA%LGWADV.AND.YDDYNA%LSLINL.AND.YDDYNA%NGWADVSI==2.AND.NCURRENT_ITER==0) THEN
...
IF ( .NOT. (YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
  ...
  CALL LARCINB(YDGEOMETRY,YDMODEL,YDGMV,&
   ! --- INPUT ---------------------------------------------------------
   ...
   & ZUZ9,ZVZ9,ZUP9,ZVP9,PTP9,PGFLP9,PSPPTCLEARP9,ZDBBC9,ZDPHI9,ZGWS9)
...
IF ( YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER == 0) ) THEN
  ...
  DO JLEV=1,NFLEVG 
    DO JROF=KST,KPROF 
      ZGPPC(JROF,MGPPCF_U+JLEV-1)= ZGMV9(JROF,JLEV,YGP%MU)
      ZGPPC(JROF,MGPPCF_V+JLEV-1)= ZGMV9(JROF,JLEV,YGP%MV)
      IF( LDSLPHY ) THEN
        ZGPPC(JROF,MGPPCF_UP+JLEV-1)=ZUP9(JROF,JLEV)
        ZGPPC(JROF,MGPPCF_VP+JLEV-1)=ZVP9(JROF,JLEV)
      ENDIF
    ENDDO
  ENDDO
  ELSE
  ...
  IF( LDSLPHY ) THEN
    ...
    DO JGFL=1,NUMFLDS
      IF(YCOMP(JGFL)%LPHY)  &
       & ZGPPC(KST:KPROF,MGPPCF_GFLP+(YCOMP(JGFL)%MPSLP-1)*NFLEVG:MGPPCF_GFLP+YCOMP(JGFL)%MPSLP*NFLEVG-1)=&
       & PGFLP9(KST:KPROF,1:NFLEVG,YCOMP(JGFL)%MPSLP)
    ENDDO
  ENDIF
  DO JGFL=1,NUMFLDS
...
IF ( YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0) ) THEN
  ...
  IF( LDSLPHY ) THEN
    DO JGFL=1,NUMFLDS
      IF(YCOMP(JGFL)%LPHY) PGFLP9(KST:KPROF,1:NFLEVG,YCOMP(JGFL)%MPSLP) = &
       & ZGPPC(KST:KPROF,MGPPCF_GFLP+(YCOMP(JGFL)%MPSLP-1)*NFLEVG:MGPPCF_GFLP+YCOMP(JGFL)%MPSLP*NFLEVG-1)
    ENDDO
  ENDIF
  DO JGFL=1,NUMFLDS
...
ELSEIF (NVLAG == 3) THEN
  ...
  IF(YDCVER%LVERTFE) THEN
    ...
    CALL VERDISINT(YDVFE,YDCVER,'ITOP','11',NPROMA,KST,KPROF,NFLEVG,ZSPT1,ZSPTO)
    PGMVT1S(KST:KPROF,YT1%MSP)=PGMVT1S(KST:KPROF,YT1%MSP)&
...
ELSE
  ...
  IF (YDDYNA%LSLINL.AND.YDDYNA%NGWADVSI==1.AND.NCURRENT_ITER==0) THEN
    DO JLEV=1,NFLEVG
      ...
      ENDDO
    ENDDO
  ELSEIF (YDDYNA%LSLINL.AND.YDDYNA%NGWADVSI==2.AND.NCURRENT_ITER==0) THEN
...
ELSE
  CALL GPRCP(NPROMA,KST,KPROF,NFLEVG,PGFL=PGFLT1,PR=ZR1,KGFLTYP=1)
  DO JLEV=1,NFLEVG
    DO JROF=KST,KPROF
...
IF ( .NOT. (YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
  ...
  IF ( LLCOMPUTE_OMVAK_O .AND. (.NOT.L2TLFF)&
   & .AND. YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER == 0) ) THEN
    ...
    DO JLEV=1,NFLEVG 
      DO JROF=KST,KPROF 
        ZGPPC(JROF,MGPPCF_U+JLEV-1)=ZGPPC(JROF,MGPPCF_U+JLEV-1)+ZGMV9(JROF,JLEV,YGP%MU)
        ZGPPC(JROF,MGPPCF_V+JLEV-1)=ZGPPC(JROF,MGPPCF_V+JLEV-1)+ZGMV9(JROF,JLEV,YGP%MV)
      ENDDO
    ENDDO
    ELSE
    DO JLEV=1,NFLEVG
...
IF(YDDYNA%LGWADV) THEN
  ...
  IF (YDDYNA%NVDVAR==5) ALLOCATE (ZNHYT1(NPROMA,0:NFLEVG))
  ...
  IF (YDCVER%LVERTFE.AND.YDCVER%LVFE_GW) THEN
    ...
    IF (YDDYNA%NVDVAR==3 .OR. YDDYNA%NVDVAR==4) THEN
    ...
    CALL GNHGW2SVD(YDGEOMETRY,KST,KPROF,YDDYNA%LNHEE,YDDYNA%NPDVAR,IVDVAR,&
  ...
  ELSE
    ...
    IF (YDDYNA%NVDVAR==3 .OR. YDDYNA%NVDVAR==4) THEN
      ZVWVT1(KST:KPROF,NFLEVG)=ZUST1(KST:KPROF)*ZOROGL_SR(KST:KPROF)&
       & +ZVST1(KST:KPROF)*ZOROGM_SR(KST:KPROF)
    ELSEIF (YDDYNA%NVDVAR==5) THEN
      CALL GNHY(YDGEOMETRY,YDDYNA,NPROMA,KST,KPROF,ZOROGL_SR,ZOROGM_SR,&
...
IF (YDDYNA%LRDBBC.AND.(YDDYNA%NVDVAR==3 .OR. YDDYNA%NVDVAR==4)) THEN
...
IF (YDDYNA%LNHDYN.AND.YDDYNA%LGWADV.AND.YDDYNA%NGWADVSI==2.AND.NCURRENT_ITER==0) THEN
  ...
  IF (YDDYNA%LSLINL.AND.YDDYNA%LNHEE) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
