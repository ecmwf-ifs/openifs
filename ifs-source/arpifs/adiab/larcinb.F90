...
@PROCESS NOCHECK
#endif
SUBROUTINE LARCINB(YDGEOMETRY,YDMODEL,YDGMV,&
 ! --- INPUT ----------------------------------------------------------
 & KST,KPROF,KASLB1,KHVI,LDSLPHY,LD2TLFF1,&
 & KL0,KLH0,PLSCAW,PRSCAW,PB1,KIBL,KNOWENO,&
 ! --- INOUT ----------------------------------------------------------
 & PGMVTNDSL,PGFLTNDSL,PGMVTNDSI,&
 ! --- OUTPUT --------------------------------------------------------
 & PGMVF,PGMVF_SI,PGMVF_NL,PDP,PGFLF,PCF,PCF_SI,PCF_NL,&
 & PUFZ,PVFZ,PUPF,PVPF,PTPF,PGFLPF,PSPPTCLEARPF,PDBBCF,PDPHIF,PGWSF)
...
!          PSPPTCLEARPF - Interpolated tendency at O of clear sky radiation.
...
!   F. Vana   21-Mar-2023: Option LRHS_CURV_FIX
! End Modifications
!------------------------------------------------------------------------------
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PB1(KASLB1,YDMODEL%YRML_DYN%YRPTRSLB1%NFLDSLB1)
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGFLF(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG,YDMODEL%YRML_GCONF%YGFL%NUMFLDS) 
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGFLPF(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG,YDMODEL%YRML_GCONF%YGFL%NDIMSLP) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PSPPTCLEARPF(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)
...
REAL(KIND=JPRB) :: ZFAC(YDGEOMETRY%YRDIM%NPROMA)
REAL(KIND=JPRB),PARAMETER :: ZEPS=100._JPRB*EPSILON(1._JPRB)

INTEGER(KIND=JPIM) :: JLEV, JROF, JGFL, JF 
INTEGER(KIND=JPIM) :: IWK, IWNT, IWKM, IWKH, IWKGFL, IQM

LOGICAL :: LLTDIABLIN,LLHORTURB,LLCOMAD,LLSLPHY
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV, YDGSGEOM=>YDGEOMETRY%YRGSGEOM, &
 & YDML_DYN=>YDMODEL%YRML_DYN, YDDYN=>YDMODEL%YRML_DYN%YRDYN, YDDYNA=>YDMODEL%YRML_DYN%YRDYNA, &
 & YDPTRSLB1=>YDMODEL%YRML_DYN%YRPTRSLB1, YDTLSCAW=>YDMODEL%YRML_DYN%YYTLSCAW, YDTRSCAW=>YDMODEL%YRML_DYN%YYTRSCAW, &
 & YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY, &
 & YDMDDH=>YDMODEL%YRML_DIAG%YRMDDH, YDLDDH=>YDMODEL%YRML_DIAG%YRLDDH, &
 & YGFL=>YDMODEL%YRML_GCONF%YGFL, YDDIMF=>YDMODEL%YRML_GCONF%YRDIMF,&
 & YDSPPT_CONFIG=>YDMODEL%YRML_GCONF%YRSPPT_CONFIG)

 
ASSOCIATE(NUMFLDS=>YGFL%NUMFLDS, YCOMP=>YGFL%YCOMP, NUMFLDS_SL1=>YGFL%NUMFLDS_SL1, NUMFLDS_SPL=>YGFL%NUMFLDS_SPL, &
 & NPROMA=>YDDIM%NPROMA, NFLEN=>YDDIMV%NFLEN, NFLEVG=>YDDIMV%NFLEVG, NFLSA=>YDDIMV%NFLSA, RRINTOPT=>YDDYN%RRINTOPT, &
 & LQMHP=>YDDYN%LQMHP,LQMHPD=>YDDYN%LQMHPD, LQMHT=>YDDYN%LQMHT, LQMHVD=>YDDYN%LQMHVD, LQMHW=>YDDYN%LQMHW, LQMP=>YDDYN%LQMP, &
 & LQMPD=>YDDYN%LQMPD, LQMT=>YDDYN%LQMT, LQMVD=>YDDYN%LQMVD,   LQMW=>YDDYN%LQMW, LSPLTHOIGFL=>YDDYN%LSPLTHOIGFL, &
 & LRHS_CURV=>YDDYN%LRHS_CURV, LRHS_CURV_FIX=>YDDYN%LRHS_CURV_FIX, &
 ...
 & MSLB1ZR9=>YDPTRSLB1%MSLB1ZR9, MSLB1Z9=>YDPTRSLB1%MSLB1Z9, MSLB1ZP9=>YDPTRSLB1%MSLB1ZP9, MSLB1Z0=>YDPTRSLB1%MSLB1Z0, &
 & MSLB1V9_NL=>YDPTRSLB1%MSLB1V9_NL, MSLB1VD9_NL=>YDPTRSLB1%MSLB1VD9_NL, MSLB1SPPTCLEARP9=>YDPTRSLB1%MSLB1SPPTCLEARP9)
!     ------------------------------------------------------------------
...
! LSLPHY must be off here when LPC_CHEAP.AND.(NCURRENT_ITER > 0)
LLSLPHY=LDSLPHY
IF(YDDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0)) LLSLPHY=.false.
...
!    IWK=NSLDIMK
!  ELSE
!    LLHOI2=YDDYNA%LSLHD_W
!    IWK=IWKM
!  ENDIF
!  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
!   & LL_LIN,YDDYNA%LCOMAD_W,LLHOI2,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF(NWLAG >= 3) THEN
  ...
  IF (LD2TLFF1) THEN
    ...
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
     & LL_LIN,YDDYNA%LCOMAD_W,YDDYNA%LSLHD_W,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
     & PB1(1,MSLB1UR9),ZGMVFU)
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
     & LL_LIN,YDDYNA%LCOMAD_W,YDDYNA%LSLHD_W,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
     & PB1(1,MSLB1VR9),ZGMVFV)
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
     & LL_LIN,YDDYNA%LCOMAD_W,YDDYNA%LSLHD_W,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
     & PB1(1,MSLB1ZR9),ZGMVFZ)

    ! Conversion
    DO JLEV=1,NFLEVG
      PUFZ(KST:KPROF,JLEV)=-ZGMVFU(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GESLO(KST:KPROF)  &
       &                   +ZGMVFV(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GECLO(KST:KPROF)
      PVFZ(KST:KPROF,JLEV)=-ZGMVFU(KST:KPROF,JLEV)                            &
       &      *YDGSGEOM(KIBL)%GECLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF) &
       &                   -ZGMVFV(KST:KPROF,JLEV)                            &
       &      *YDGSGEOM(KIBL)%GESLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF) &
       &                   +ZGMVFZ(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GSQM2(KST:KPROF)
      ! ensure the quantity is conserved
      ZFAC(KST:KPROF)=(ZGMVFU(KST:KPROF,JLEV)*ZGMVFU(KST:KPROF,JLEV) &
       &              +ZGMVFV(KST:KPROF,JLEV)*ZGMVFV(KST:KPROF,JLEV) &
       &              +ZGMVFZ(KST:KPROF,JLEV)*ZGMVFZ(KST:KPROF,JLEV) )
      WHERE((ZFAC(KST:KPROF) > ZEPS ) .AND. LRHS_CURV_FIX)
        ZFAC(KST:KPROF)=ZFAC(KST:KPROF)/ &
         & MAX(ZEPS,(PUFZ(KST:KPROF,JLEV)*PUFZ(KST:KPROF,JLEV) &
         &          +PVFZ(KST:KPROF,JLEV)*PVFZ(KST:KPROF,JLEV)))
        ZFAC(KST:KPROF)=SQRT(ZFAC(KST:KPROF))
        PUFZ(KST:KPROF,JLEV)=ZFAC(KST:KPROF)*PUFZ(KST:KPROF,JLEV)
        PVFZ(KST:KPROF,JLEV)=ZFAC(KST:KPROF)*PVFZ(KST:KPROF,JLEV)
      ENDWHERE
    ENDDO
  ELSE
    ...
    ! Conversion
    DO JLEV=1,NFLEVG
      PUFZ(KST:KPROF,JLEV)=-ZGMVU(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GESLO(KST:KPROF)   &
       &                   +ZGMVV(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GECLO(KST:KPROF)
      PVFZ(KST:KPROF,JLEV)=-ZGMVU(KST:KPROF,JLEV)                             &
       &     *YDGSGEOM(KIBL)%GECLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF)  &
       &                   -ZGMVV(KST:KPROF,JLEV)                             &
       &     *YDGSGEOM(KIBL)%GESLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF)  &
       &                   +ZGMVZ(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GSQM2(KST:KPROF)
      ! ensure the quantity is conserved
      ZFAC(KST:KPROF)=(ZGMVU(KST:KPROF,JLEV)*ZGMVU(KST:KPROF,JLEV) &
       &              +ZGMVV(KST:KPROF,JLEV)*ZGMVV(KST:KPROF,JLEV) &
       &              +ZGMVZ(KST:KPROF,JLEV)*ZGMVZ(KST:KPROF,JLEV) )
      WHERE((ZFAC(KST:KPROF) > ZEPS ) .AND. LRHS_CURV_FIX)
        ZFAC(KST:KPROF)=ZFAC(KST:KPROF)/ &
         & MAX(ZEPS,(PUFZ(KST:KPROF,JLEV)*PUFZ(KST:KPROF,JLEV) &
         &          +PVFZ(KST:KPROF,JLEV)*PVFZ(KST:KPROF,JLEV)))
  ...
  IF (YDDYNA%LCOMAD_W) THEN
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     ...
     & PB1(1,MSLB1U0),ZGMVFU)
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
     & PB1(1,MSLB1V0),ZGMVFV)
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
     & PB1(1,MSLB1Z0),ZGMVFZ)
  ELSE
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
  ...
  ! Accumulating interpolation quantities
  ZGMVU(KST:KPROF,1:NFLEVG)=ZGMVU(KST:KPROF,1:NFLEVG)+ZGMVFU(KST:KPROF,1:NFLEVG)
  ZGMVV(KST:KPROF,1:NFLEVG)=ZGMVV(KST:KPROF,1:NFLEVG)+ZGMVFV(KST:KPROF,1:NFLEVG)
  ZGMVZ(KST:KPROF,1:NFLEVG)=ZGMVZ(KST:KPROF,1:NFLEVG)+ZGMVFZ(KST:KPROF,1:NFLEVG)

  ! Conversion
  DO JLEV=1,NFLEVG
    PGMVF(KST:KPROF,JLEV,YGP%MU)=-ZGMVU(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GESLO(KST:KPROF)  &
       &                         +ZGMVV(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GECLO(KST:KPROF)
    PGMVF(KST:KPROF,JLEV,YGP%MV)=-ZGMVU(KST:KPROF,JLEV)                            &
       &           *YDGSGEOM(KIBL)%GECLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF) &
       &                         -ZGMVV(KST:KPROF,JLEV)                            &
       &           *YDGSGEOM(KIBL)%GESLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF) &
       &                         +ZGMVZ(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GSQM2(KST:KPROF)
    ...
    WHERE((ZFAC(KST:KPROF) > ZEPS ) .AND. LRHS_CURV_FIX)
      ZFAC(KST:KPROF)=ZFAC(KST:KPROF)/ &
       & MAX(ZEPS,(PGMVF(KST:KPROF,JLEV,YGP%MU)**2   &
       &          +PGMVF(KST:KPROF,JLEV,YGP%MV)**2))
      ZFAC(KST:KPROF)=SQRT(ZFAC(KST:KPROF))
      PGMVF(KST:KPROF,JLEV,YGP%MU)=ZFAC(KST:KPROF)*PGMVF(KST:KPROF,JLEV,YGP%MU)
      PGMVF(KST:KPROF,JLEV,YGP%MV)=ZFAC(KST:KPROF)*PGMVF(KST:KPROF,JLEV,YGP%MV)
    ENDWHERE
  ENDDO
...
ELSE ! Have to finish the conversion for NWLAG=2
  ! Conversion
  DO JLEV=1,NFLEVG
    PGMVF(KST:KPROF,JLEV,YGP%MU)=-ZGMVU(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GESLO(KST:KPROF)  &
     &                           +ZGMVV(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GECLO(KST:KPROF)
...
IF(LLSLPHY) THEN
  IF (YDDYNA%LCOMAD_W) THEN
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
     & PB1(1,MSLB1UP9),ZGMVU)
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
     & PB1(1,MSLB1VP9),ZGMVV)
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
     & PB1(1,MSLB1ZP9),ZGMVZ)
  ELSE
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAW(1,1,YDTLSCAW%M_WDLAT),PLSCAW(1,1,YDTLSCAW%M_WDLO+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVER),&
     & PB1(1,MSLB1UP9),ZGMVU)
  ...
  ! Conversion
  DO JLEV=1,NFLEVG
    PUPF(KST:KPROF,JLEV)=-ZGMVU(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GESLO(KST:KPROF)  &
     &                   +ZGMVV(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GECLO(KST:KPROF)
    PVPF(KST:KPROF,JLEV)=-ZGMVU(KST:KPROF,JLEV)                            &
     &     *YDGSGEOM(KIBL)%GECLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF) &
     &                   -ZGMVV(KST:KPROF,JLEV)                            &
     &     *YDGSGEOM(KIBL)%GESLO(KST:KPROF)*YDGSGEOM(KIBL)%GEMU(KST:KPROF) &
     &                   +ZGMVZ(KST:KPROF,JLEV)*YDGSGEOM(KIBL)%GSQM2(KST:KPROF)
    ...
    WHERE((ZFAC(KST:KPROF) > ZEPS ) .AND. LRHS_CURV_FIX)
      ZFAC(KST:KPROF)=ZFAC(KST:KPROF)/ &
       & MAX(ZEPS,(PUPF(KST:KPROF,JLEV)*PUPF(KST:KPROF,JLEV) &
       &          +PVPF(KST:KPROF,JLEV)*PVPF(KST:KPROF,JLEV)))
      ZFAC(KST:KPROF)=SQRT(ZFAC(KST:KPROF))
      PUPF(KST:KPROF,JLEV)=ZFAC(KST:KPROF)*PUPF(KST:KPROF,JLEV)
      PVPF(KST:KPROF,JLEV)=ZFAC(KST:KPROF)*PVPF(KST:KPROF,JLEV)
    ENDWHERE
  ENDDO
ENDIF
...
CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKH,&
 & LL_LIN,YDDYNA%LCOMAD_T,YDDYNA%LSLHD_T,LVWENO_T,WENO_ALPHA_T,LLMOM,LQMT,LQMHT,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF ((NSPLTHOI /= 0).AND.(NTLAG < 4)) THEN
  ...
  IF (NSPLTHOI == 1) THEN
    ...
    IWK=NSLDIMK
  ELSE
    LLHOI2=YDDYNA%LSLHD_T
    IWK=IWKH
  ENDIF
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
   & LL_LIN,YDDYNA%LCOMAD_T,LLHOI2,LVWENO_T,WENO_ALPHA_T,LLMOM,LQMT,LQMHT,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF (YDDYNA%LNHEE) THEN
  ...
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
   & LL_LIN,YDDYNA%LCOMAD_SPD,YDDYNA%LSLHD_SPD,LVWENO_SPD,WENO_ALPHA_SPD,LLMOM,LQMPD,LQMHPD,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF (YDDYNA%LNHDYN) THEN
  ...
  LL2=YDDYNA%LGWADV.AND.YDGEOMETRY%YRCVER%LVFE_GW
  LL3=YDDYNA%LGWADV.AND.(YDDYNA%ND4SYS == 2).AND.(NCURRENT_ITER < NSITER)

  ! --- part of the RHS requiring a "high order" interpolation:
  IF (LL1.OR.LL2) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
     & LL_LIN,YDDYNA%LCOMAD_SVD,YDDYNA%LSLHD_SVD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
  ...
  IF ((NSPLTHOI /= 0).AND.(NSVDLAG < 4)) THEN
    ...
    IF (NSPLTHOI == 1) THEN
      ...
      IWK=NSLDIMK
    ELSE
      LLHOI2=YDDYNA%LSLHD_SVD
      IWK=IWKM
    ENDIF
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
     & LL_LIN,YDDYNA%LCOMAD_SVD,LLHOI2,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
  ...
  IF ((LL1.OR.LL3).AND.(YDDYNA%NVDVAR == 4 .OR. YDDYNA%NVDVAR == 5)) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
     & LL_LIN,YDDYNA%LCOMAD_SVD,LLNOSLHD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
DO JGFL=1,NUMFLDS
  ...
  IF (YCOMP(JGFL)%LADV.AND.(.NOT.YCOMP(JGFL)%LMGRID)) THEN
    ...
    IF (RRINTOPT /= 1._JPRB) THEN
      DO JLEV=1,NFLEVG
        DO JROF=KST,KPROF
          PGFLF(JROF,JLEV,JGFL)=PGFLF(JROF,JLEV,JGFL)*RRINTOPT
        ENDDO
      ENDDO
    ENDIF
    ...
    IF(LLTDIABLIN) THEN
      ...
      ENDDO
    ENDIF
  ENDIF ! YCOMP(JGFL)%LADV.AND.(.NOT.YCOMP(JGFL)%LMGRID)

  IF (YCOMP(JGFL)%LADV) THEN

    ! --- Split physics requiring a trilinear interpolation:
    IF(YCOMP(JGFL)%LPHY.AND.LLSLPHY) THEN
      ...
      IF (LLCOMAD ) THEN
        CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
         ...
         & PB1(1,MSLB1GFLP9+(YCOMP(JGFL)%MPSLP-1)*(NFLEN-NFLSA+1)),PGFLPF(1,1,YCOMP(JGFL)%MPSLP))
      ELSE
        CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
...
IF(NVLAG == 2) THEN
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
   & LL_LIN,YDDYNA%LCOMAD_SP,LLNOSLHD,LVWENO_SP,WENO_ALPHA_SP,LLMOM,LQMP,LQMHP,KNOWENO,KL0,PLSCAW,PRSCAW,&
   & PB1(1,MSLB1C9),PCF)
ENDIF
...
IF (YDDYNA%LCOMAD_SP) THEN
  CALL LAIDDI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IQM,&
   & PRSCAW(1,1,YDTRSCAW%M_WCLAMAD(IWNT)),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD),&
   & PRSCAW(1,1,YDTRSCAW%M_WCLOMAD(IWNT)),KLH0,&
   & PB1(1,MSLB1SP9),PDP)
ELSE
  CALL LAIDDI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IQM,&
   & PRSCAW(1,1,YDTRSCAW%M_WCLA(IWNT)),PLSCAW(1,1,YDTLSCAW%M_WDLO),&
   & PRSCAW(1,1,YDTRSCAW%M_WCLO(IWNT)),KLH0,&
   & PB1(1,MSLB1SP9),PDP)
ENDIF
...
IF (YDDYNA%LNHDYN.AND.YDDYNA%LRDBBC) THEN
  ...
  IF (NSVDLAG == 2) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,1,NFLSA,NFLEN,IWNT,&
     & LL_LIN,YDDYNA%LCOMAD_SVD,LLNOSLHD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0(1:NPROMA,NFLEVG,0:3),&
