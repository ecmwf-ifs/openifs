...
!   F. Vana 21-Mar-2023: Option LSLDP_CURV_FIX.
...
REAL(KIND=JPRB) :: ZPU0, ZPV0, ZPZ0, ZFAC
...
REAL(KIND=JPRB), POINTER :: ZSLB1UR00(:),ZSLB1VR00(:),ZSLB1WR00(:),ZSLB1ZR00(:)
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, &
 ...
 & YDDYNA=>YDML_DYN%YRDYNA,YDPTRSLB1=>YDML_DYN%YRPTRSLB1, YDPTRSLB2=>YDML_DYN%YRPTRSLB2)
ASSOCIATE(NPROMA=>YDDIM%NPROMA, &
 ...
 & VMAX1=>YDDYN%VMAX1, VMAX2=>YDDYN%VMAX2, LSLDP_CURV_FIX=>YDDYN%LSLDP_CURV_FIX, &
...
CALL SC2PRG(MSLB1UR00 ,PB1      ,ZSLB1UR00)
CALL SC2PRG(MSLB1VR00 ,PB1      ,ZSLB1VR00)
CALL SC2PRG(MSLB1WR00 ,PB1      ,ZSLB1WR00)
CALL SC2PRG(MSLB1ZR00 ,PB1      ,ZSLB1ZR00)
...
ELSE
  ...
  IF (YDDYNA%LTWOTL) CALL ABOR1(' LARMES_RK 1.2 ')
  ZDTS22=RDTS22
  ZDTSA=RDTSA
  ZDTS62=RDTS62
ENDIF
...
IF(LSVTSM .OR. YDDYNA%LRALTVDISP .OR. YDDYNA%LELTRA) THEN
...
CALL LARCINA(YDGEOMETRY,YDML_DYN,KST,KPROF,YDSL,IHVI,KSTABUF,LFINDVSEP,LLSLHD,LLSLHDQUAD,LLINTV,&
 ...
 & ZSLB1UR00,ZSLB1VR00,ZSLB1ZR00,ZSLB1WR00,&
 & KVSEPC,KVSEPL,PCCO,&
...
IF (NITMP==1) THEN
  ...
  DO JLEV=1,NFLEVG
    DO JROF=KST,KPROF
      ...
      ! ensure the converted quantity is conserved
      ZFAC=ZUS(JROF,JLEV,2)*ZUS(JROF,JLEV,2) &
       &  +ZVS(JROF,JLEV,2)*ZVS(JROF,JLEV,2) &
       &  +ZZS(JROF,JLEV,2)*ZZS(JROF,JLEV,2)
      IF ((ZFAC > ZEPS).AND.LSLDP_CURV_FIX) THEN
        ZFAC=SQRT(ZFAC/MAX(ZEPS,(ZPU*ZPU+ZPV*ZPV)))
        ZPU=ZFAC*ZPU
        ZPV=ZFAC*ZPV
      ENDIF
...
DO JITER=2,NITMP
  ...
  DO JLEV=1,NFLEVG
    DO JROF=KST,KPROF
      ...
      ! ensure the converted quantity is conserved
      ZFAC=ZPU0*ZPU0 + ZPV0*ZPV0 + ZPZ0*ZPZ0
      IF ((ZFAC > ZEPS).AND.LSLDP_CURV_FIX) THEN
        ZFAC=SQRT(ZFAC/MAX(ZEPS,(ZPU*ZPU+ZPV*ZPV)))
        ZPU=ZFAC*ZPU
        ZPV=ZFAC*ZPV
...
IF (.NOT.(LLO.OR.YDDYNA%LELTRA)) THEN
  ! this case may occur only for SL3TL scheme.
  IF (YDDYNA%LTWOTL) CALL ABOR1(' LARMES_RK 3 ')
