...
SUBROUTINE LATTEX(YDGEOMETRY,YDGMV,&
 ...
 & PRDELP,PEVEL,PATND,&
 & PNHXT0,PGWT0,PGWFT0,PNHYT0,PNHXT9,PGWT9,PNHYT9,PGFL,PMIXNL,&
 ! --- INPUT/OUTPUT -------------------------------------------
...
!   F. Vana January 2023: LRHS_CURV fixed for ICI
! End Modifications
!------------------------------------------------------------------------------
...
IF (LRSIDDH) THEN
  ZSIDDH_U0(KST:KPROF,1:NFLEVG)=0.0_JPRB
  ZSIDDH_V0(KST:KPROF,1:NFLEVG)=0.0_JPRB
  ZSIDDH_T0(KST:KPROF,1:NFLEVG)=0.0_JPRB
  DO JLEV=1,NFLEVG
  ...
  IF (YDDYNA%LNHDYN) THEN !! ?????? LNHDYN or LNHEE?
    ZSIDDH_PD0(KST:KPROF,1:NFLEVG)=0.0_JPRB
  ENDIF
  IF (YDDYNA%LNHDYN) THEN
    ZSIDDH_VD0(KST:KPROF,1:NFLEVG)=0.0_JPRB
  ENDIF
...
IF (LRHS_CURV.AND..NOT.LLCTC) THEN
  DO JLEV=1,NFLEVG
