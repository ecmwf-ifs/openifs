...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PWRL9(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)
...
IF (LSLDP_CURV.OR.LSLDP_XYZ) THEN
  ...
  IF (LSLDP_XYZ) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
        ZUCOMP=PB2(JROF,MSLB2URL+JLEV-1)
        PB2(JROF,MSLB2ZRL+JLEV-1)= PB2(JROF,MSLB2VRL+JLEV-1)*YDGSGEOM%GSQM2(JROF)
        PB2(JROF,MSLB2URL+JLEV-1)=-PB2(JROF,MSLB2URL+JLEV-1)*YDGSGEOM%GESLO(JROF) &
          &      -PB2(JROF,MSLB2VRL+JLEV-1)*YDGSGEOM%GECLO(JROF)*YDGSGEOM%GEMU(JROF)
        PB2(JROF,MSLB2VRL+JLEV-1)=ZUCOMP*YDGSGEOM%GECLO(JROF)          &
          &      -PB2(JROF,MSLB2VRL+JLEV-1)*YDGSGEOM%GESLO(JROF)*YDGSGEOM%GEMU(JROF)
      ENDDO
    ENDDO
  ENDIF
  IF (LSLDP_RK) THEN
...
IF (LRHS_CURV.AND.LD2TLFF1) THEN
  DO JLEV=1,NFLEVG
    DO JROF=KST,KPROF
      ZUCOMP=PB1(JROF,MSLB1UR9+JLEV-NFLSA)
      PB1(JROF,MSLB1ZR9+JLEV-NFLSA)=PB1(JROF,MSLB1VR9+JLEV-NFLSA)*YDGSGEOM%GSQM2(JROF)
      PB1(JROF,MSLB1UR9+JLEV-NFLSA)=-PB1(JROF,MSLB1UR9+JLEV-NFLSA)*YDGSGEOM%GESLO(JROF) &
       &             -PB1(JROF,MSLB1VR9+JLEV-NFLSA)*YDGSGEOM%GECLO(JROF)*YDGSGEOM%GEMU(JROF)
      PB1(JROF,MSLB1VR9+JLEV-NFLSA)= ZUCOMP*YDGSGEOM%GECLO(JROF) &
       &             -PB1(JROF,MSLB1VR9+JLEV-NFLSA)*YDGSGEOM%GESLO(JROF)*YDGSGEOM%GEMU(JROF)
    ENDDO
  ENDDO
ENDIF
!     ------------------------------------------------------------------
