...
!   I. Polichtchouk (Nov 2020): Added option to not damp m=0 mode in the sponge
! End Modifications
!------------------------------------------------------------------------------
...
INTEGER(KIND=JPIM) :: IOFF_VOR, IOFF_DIV, IOFF_U, IOFF_V, IOFF_NS_T, &
                    & IOFF_EW_U, IOFF_EW_V, IOFF_EW_T, IN_MIN
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, YDMP=>YDGEOMETRY%YRMP, &
 &  YDLAP=>YDGEOMETRY%YRLAP, YDDYN=>YDMODEL%YRML_DYN%YRDYN,YDDYNA=>YDMODEL%YRML_DYN%YRDYNA, &
...
ASSOCIATE(NUMFLDS=>YGFL%NUMFLDS, YA=>YGFL%YA, YCOMP=>YGFL%YCOMP, YI=>YGFL%YI, &
 ...
 & NPTRLL=>YDMP%NPTRLL, &
 & TDT=>YDRIP%TDT, LM0DAMP=>YDDYN%LM0DAMP, &
...
IF (LRHDDDH) THEN
  ...
  IF (YDDYNA%LNHDYN) THEN
    ...
    IF (YDDYNA%LNHX) PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)=-PSPSNHX(1:NFLEVG,KSTA:KEND)
  ENDIF
  IF (YQ%LSP) THEN
    DO JSP=KSTA,KEND
      DO JLEV=1,NFLEVG
...
!         - GMVS variables.
  ...
  IF ((YDDYNA%LSLHD_W.OR.YDDYNA%LSLHD_SVD).AND.(.NOT.YDDYNA%LSLHD_STATIC)) THEN
    ...
    IVTHS=0
    IF (YDDYNA%LSLHD_W) IVTHS=IVTHS+2
    IF (YDDYNA%LNHDYN.AND.YDDYNA%LSLHD_SVD) IVTHS=IVTHS+1
    IF (YDDYNA%LNHX.AND.YDDYNA%LSLHD_SVD) IVTHS=IVTHS+1
    IL=IVTHS*NFLEVL
    ...
    DO JMLOC=1,NUMP
      ...
      DO JN=IM,NSMAX
        ...
        ENDIF
        ! * Vertical divergence.
        IF (YDDYNA%LNHDYN.AND.YDDYNA%LSLHD_SVD) THEN
          DO JLEV=1,NFLEVL
        ...
        ENDIF
        ! * NHX.
        IF (YDDYNA%LNHX.AND.YDDYNA%LSLHD_SVD) THEN
          DO JLEV=1,NFLEVL
...
!           2.2  Diffusion when no stretching.
  ...
  ZDT=ABS(TDT)
  CALL GSTATS(1037,0)
  IF (.NOT.LM0DAMP) THEN  !no damping on m=0 mode
     DO JMLOC=1,NUMP
      IM=YDLAP%MYMS(JMLOC)
      IF (IM>0) THEN
        IN_MIN=IM
        DO JN=IN_MIN,NSMAX
          ...
          ! * Vor,Div.
          DO JLEV=1,NFLEVL
              PSPVOR(JLEV,IJSE  )=PSPVOR(JLEV,IJSE  )/(1.0_JPRB+ZDT*RDIVOR(JLEV,JN))
              PSPVOR(JLEV,IJSE+1)=PSPVOR(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDIVOR(JLEV,JN))
              PSPDIV(JLEV,IJSE  )=PSPDIV(JLEV,IJSE  )/(1.0_JPRB+ZDT*RDIDIV(JLEV,JN))
              PSPDIV(JLEV,IJSE+1)=PSPDIV(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDIDIV(JLEV,JN))
          ENDDO
          IF (YDDYNA%LSLHD_W.AND.(.NOT.YDDYNA%LSLHD_STATIC)) THEN
          ! * Additional diffusion 
            DO JLEV=1,NFLEVL
               PSPVOR(JLEV,IJSE)=PSPVOR(JLEV,IJSE)/(1.0_JPRB+ZDT*RDSVOR(JLEV,JN))
               PSPVOR(JLEV,IJSE+1)=PSPVOR(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDSVOR(JLEV,JN))
               PSPDIV(JLEV,IJSE)=PSPDIV(JLEV,IJSE)/(1.0_JPRB+ZDT*RDSDIV(JLEV,JN))
               PSPDIV(JLEV,IJSE+1)=PSPDIV(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDSDIV(JLEV,JN))
            ENDDO
          ENDIF
          ! * Temperature.
          IF (LSPT) THEN
            DO JLEV=1,NFLEVL
             ILEVG=ILEV0+JLEV
               PSPT(JLEV,IJSE  )= (PSPT  (JLEV,IJSE  )&
               & +RCORDIT(ILEVG)*PSPSP(IJSE  )*ZDT*RDITG(ILEVG,JN))&
               & /(1.0_JPRB+ZDT*RDITG(ILEVG,JN))
               PSPT(JLEV,IJSE+1)= (PSPT  (JLEV,IJSE+1)&
               & +RCORDIT(ILEVG)*PSPSP(IJSE+1)*ZDT*RDITG(ILEVG,JN))&
               & /(1.0_JPRB+ZDT*RDITG(ILEVG,JN))
            ENDDO
         ! * 2.2.3: GMVS variables
            PSPSP(IJSE)=PSPSP(IJSE)/(1.0_JPRB+ZDT*RDISP(YDLAP%NVALUE(IJSE)))
            PSPSP(IJSE+1)=PSPSP(IJSE+1)/(1.0_JPRB+ZDT*RDISP(YDLAP%NVALUE(IJSE+1)))
          ENDIF

          IF (YDDYNA%LNHDYN) THEN
            DO JLEV=1,NFLEVL
              PSPSPD(JLEV,IJSE)=PSPSPD(JLEV,IJSE)/(1.0_JPRB+ZDT*RDIPD(JLEV,JN))
              PSPSPD(JLEV,IJSE+1)=PSPSPD(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDIPD(JLEV,JN))
              PSPSVD(JLEV,IJSE)=PSPSVD(JLEV,IJSE)/(1.0_JPRB+ZDT*RDIVD(JLEV,JN))
              PSPSVD(JLEV,IJSE+1)=PSPSVD(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDIVD(JLEV,JN))
            ENDDO
            IF (YDDYNA%LNHX) THEN
              DO JLEV=1,NFLEVL
                PSPSNHX(JLEV,IJSE)=PSPSNHX(JLEV,IJSE)/(1.0_JPRB+ZDT*RDIVD(JLEV,JN))
                PSPSNHX(JLEV,IJSE+1)=PSPSNHX(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDIVD(JLEV,JN))
              ENDDO
            ENDIF
            IF (YDDYNA%LSLHD_SVD.AND.(.NOT.YDDYNA%LSLHD_STATIC)) THEN
            ! * Additional diffusion
              DO JLEV=1,NFLEVL
                PSPSVD(JLEV,IJSE)=PSPSVD(JLEV,IJSE)/(1.0_JPRB+ZDT*RDSVD(JLEV,JN))
                PSPSVD(JLEV,IJSE+1)=PSPSVD(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDSVD(JLEV,JN))
              ENDDO
              IF (YDDYNA%LNHX) THEN
                DO JLEV=1,NFLEVL
                  PSPSNHX(JLEV,IJSE)=PSPSNHX(JLEV,IJSE)/(1.0_JPRB+ZDT*RDSVD(JLEV,JN))
                  PSPSNHX(JLEV,IJSE+1)=PSPSNHX(JLEV,IJSE+1)/(1.0_JPRB+ZDT*RDSVD(JLEV,JN))
                ENDDO
              ENDIF
            ENDIF
          ENDIF
          ...
          DO JGFL=1,NUMFLDS
            IF(YCOMP(JGFL)%LSP) THEN
              DO JLEV=1,NFLEVL
                 PSPGFL(JLEV,IJSE,YCOMP(JGFL)%MPSP) =&
                 & PSPGFL(JLEV,IJSE,YCOMP(JGFL)%MPSP)/&
                 & (1.0_JPRB+ZDT*RDIGFL(JLEV,JN,YCOMP(JGFL)%MPSP))
                 PSPGFL(JLEV,IJSE+1,YCOMP(JGFL)%MPSP) =&
                 & PSPGFL(JLEV,IJSE+1,YCOMP(JGFL)%MPSP)/&
                 & (1.0_JPRB+ZDT*RDIGFL(JLEV,JN,YCOMP(JGFL)%MPSP))
              ENDDO
...
IF (LRHDDDH) THEN
  ...
  IF (YDDYNA%LNHDYN) THEN
    ...
    IF (YDDYNA%LNHX) PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)=PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)&
