...
! nor does it submit to any jurisdiction
 ...
 &    PLP,   PIP,  PRP,   PSP, PAP, PQSAT, PFPLCL, PFPLCN , PFPLSL , PFPLSN , PCOVPTOT,PLU,&
 &    PALB, PWND, PLSM, PCSZA, PGELAT, PGELAM, PGEMU, PKOZO, PCFLX, PCFLXO, PDV, PGFL,&
 &    PAEROC,PCLOUDPH, PRAINPH, PCHEMPH, PCHEMPHR, PWETDIAM, PWETVOL, PND, PAERAOT, PAERAAOT, PAERASY,&
 &    PTENGFL, PCEN ,PTENC,PNEEFLX, PCH4FLX,PCHEM2AER,PCHEM2GHG,IKLEVTROP)
...
! PCLOUDPH(KLON,KLEV,NACTAERO)  : Cloud pH from EQSAM4Clim
! PRAINPH(KLON,KLEV,NACTAERO)  : Rain pH from EQSAM4Clim
! PCHEMPH(KLON,KLEV,NACTAERO) : cloud PH computed in tm5_wetchem
! PCHEMPHR(KLON,KLEV,NACTAERO) : rain PH computed in tm5_wetchem
...
! PCH4FLX(KLON)               : CH4 FLUX FROM WETLANDS
!
! OUTPUTS:
! -------
! PTENC  (KLON,KLEV,KTRAC)     : TENDENCY OF CONCENTRATION OF TRACERS including chemistry(kg/kg s-1)
! PCHEM2AER  (KLON,KLEV,4)     : TENDENCY OF Selected TRACERS because specific reactions (kg/kg s-1)
...
! ZOUT(KLON,0:KLEV,5)             : Any additional output such as tendencies of BC or Photolysis Rates   O3, NO2, O1D , XX, XX
...
USE YOMCST   , ONLY : RMD
USE TM5_CHEM_MODULE,    ONLY : NSOA_BUDG, NCHEM2AER, NCHEM2GHG, NBUD_EXTRA
USE TM5_TRACERS,        ONLY : ICH4_TM5=>ICH4, IHNO3_TM5=>IHNO3, ISO2_TM5=>ISO2, ISO4_TM5=>ISO4, &
 &                             INH4_TM5=>INH4, IFLCO
USE BASCOETM5_TRACERS,  ONLY : ICH4_BASCOETM5=>ICH4, NBC_BT=>NBC, BASCOETM5_BC=>BASCOE_BC, &
 &                             ISO2_BASCOETM5=>ISO2, ISO4_BASCOETM5=>ISO4, INH4_BASCOETM5=>INH4
USE BASCOE_TRACERS,   ONLY : BASCOE_BC, NBC
...
INTEGER(KIND=JPIM) :: ICO,IGHGCH4,FCO2,FCO2NOBIO,FCO2NOFLX, FCH4,FCH4NOWET, FCH4NOFLX
...
REAL(KIND=JPRB)    :: ZOUT(KLON,0:KLEV,5), ZDUM(KLON,KLEV)
...
ASSOCIATE(NACTAERO=>YGFL%NACTAERO, NCHEM=>YGFL%NCHEM, NCHEM_DV=>YGFL%NCHEM_DV, &
 ...
 & LCHEM_DIAC=>YDCHEM%LCHEM_DIAC, LCHEM_FIX_CH4=>YDCHEM%LCHEM_FIX_CH4, &
...
! copy meto input in local arrays
 ...
 ZLU(KIDIA:KFDIA,1:KLEV) = PLU(KIDIA:KFDIA,1:KLEV)
...
! correct unphysical values in input
   ...
   WHERE (ZLU(KIDIA:KFDIA,1:KLEV) < 0.0_JPRB) ZLU(KIDIA:KFDIA,1:KLEV) = 0.0_JPRB
   WHERE (ZLU(KIDIA:KFDIA,1:KLEV) > 1.0_JPRB) ZLU(KIDIA:KFDIA,1:KLEV) = 1.0_JPRB
...
&                      ZTENC0,PEXTRA(:,:, IEXTR_CHTR),KTOP=IKLEVTROP)
      ...
      DO JT=1,NBC
        ! LBC long-lived components as deposition. Note the zeroth index in ZOUT is preserved for CH4 boundary condition
        PEXTRA(KIDIA:KFDIA,BASCOE_BC(JT),IEXTR_DD) =   PEXTRA(KIDIA:KFDIA,BASCOE_BC(JT),IEXTR_DD) + ZOUT(KIDIA:KFDIA,JT,1)
        !substract from chemistry
        PEXTRA(KIDIA:KFDIA,BASCOE_BC(JT), IEXTR_CH) = PEXTRA(KIDIA:KFDIA,BASCOE_BC(JT),IEXTR_CH) - ZOUT(KIDIA:KFDIA,JT,1)
        !also substract from trop chemistry
        PEXTRA(KIDIA:KFDIA,BASCOE_BC(JT), IEXTR_CHTR) = PEXTRA(KIDIA:KFDIA,BASCOE_BC(JT),IEXTR_CHTR) - ZOUT(KIDIA:KFDIA,JT,1)
      ENDDO
 ...
 &     PTSTEP ,ZDELP, PRS1, PRSF1, PGEOH, ZQP, PTP, PCLOUDPH, PRAINPH, PCHEMPH, PCHEMPHR,&
 &     ZLP, ZRP, ZSP, ZIP, ZAP, ZCOVPTOT, &
 &     ZFPLSL, ZFPLSN,ZFPLCL, ZFPLCN, &
 &     IKLEVTROP,ZALB, PWND, PLSM, ZCSZA, PGELAT, PGELAM, &
 ...
 &     PCHEM2GHG )
    ...
    IF (NACTAERO > 0 .AND. LAERCHEM) THEN
      ...
      PCHEM2AER(KIDIA:KFDIA,1:KLEV,2) = -1.0_JPRB*ZBUDR(KIDIA:KFDIA,1:KLEV,ISO2_BASCOETM5)
      PCHEM2AER(KIDIA:KFDIA,1:KLEV,3) =  ZTENC1(KIDIA:KFDIA,1:KLEV,ISO2_BASCOETM5) - ZTENC0(KIDIA:KFDIA,1:KLEV,ISO2_BASCOETM5)

      ZTENC1(KIDIA:KFDIA,1:KLEV,ISO4_BASCOETM5) = ZTENC0(KIDIA:KFDIA,1:KLEV,ISO4_BASCOETM5)
...
&                      PEXTRA(:,:, IEXTR_CHTR),KTOP=IKLEVTROP)
      ...
      IF (.NOT. LCHEM_ANACH4) THEN
        ...
        PEXTRA(KIDIA:KFDIA,IICH4,IEXTR_DD) =   PEXTRA(KIDIA:KFDIA,IICH4,IEXTR_DD) + ZOUT(KIDIA:KFDIA,0,1)
        !substract from chemistry
        PEXTRA(KIDIA:KFDIA,IICH4, IEXTR_CH) = PEXTRA(KIDIA:KFDIA,IICH4,IEXTR_CH) - ZOUT(KIDIA:KFDIA,0,1)
        !also substract from trop chemistry
        PEXTRA(KIDIA:KFDIA,IICH4, IEXTR_CHTR) = PEXTRA(KIDIA:KFDIA,IICH4,IEXTR_CHTR) - ZOUT(KIDIA:KFDIA,0,1)
      ENDIF

      DO JT=1,NBC_BT
        ! LBC long-lived components as deposition, not emissions.
        PEXTRA(KIDIA:KFDIA,BASCOETM5_BC(JT),IEXTR_DD) =   PEXTRA(KIDIA:KFDIA,BASCOETM5_BC(JT),IEXTR_DD) + ZOUT(KIDIA:KFDIA,JT,1)
        !substract from chemistry
        PEXTRA(KIDIA:KFDIA,BASCOETM5_BC(JT), IEXTR_CH) = PEXTRA(KIDIA:KFDIA,BASCOETM5_BC(JT),IEXTR_CH) - ZOUT(KIDIA:KFDIA,JT,1)
        !substract from trop chemistry
        PEXTRA(KIDIA:KFDIA,BASCOETM5_BC(JT), IEXTR_CHTR) = PEXTRA(KIDIA:KFDIA,BASCOETM5_BC(JT),IEXTR_CHTR)&
&         - ZOUT(KIDIA:KFDIA,JT,1)
      ENDDO
 ...
 &     ZLP, ZRP, ZSP, ZIP, ZAP, ZCOVPTOT,  &
 &     ZFPLSL, ZFPLSN,ZFPLCL, ZFPLCN, &
 &     ZPTROP, ZALB, PWND, PLSM, ZCSZA, PGELAT, PGELAM,&
 &     PGEMU, PKOZO, IKLEVTROP,PDV, ZCON, ZTENC1, ZBUDR, ZBUDJ, ZBUDX, ZOUT,&
 &     PAEROC,PWETDIAM, PWETVOL,PND, PAERAOT, PAERAAOT, PAERASY, ZSOGTOSOA,PCHEM2GHG)
...
! Linear ozone only
 ...
 &     KLEVX,ZDELP,PTSTEP,ZTENC1,ZTENC0,PEXTRA(:,:, IEXTR_CH))
  ...
  CASE  ("carbontracers")
    ...
    DO JT = 1,NCHEM
      ...
      FCH4NOWET = INDEX(TRIM( YCHEM(JT)%CNAME ), 'CH4_NWE')
      FCH4NOFLX = INDEX(TRIM( YCHEM(JT)%CNAME ), 'CH4_NFX')
      FCH4 = INDEX(TRIM( YCHEM(JT)%CNAME ), 'CH4')
 ...
 &        PNEEFLX(KIDIA:KFDIA)  * PTSTEP
      ...
      ! store online ch4 wetland flux as dry deposition (as done for ch4_ghg) in carbon tracers
      IF ( FCH4NOWET == 0 .AND. FCH4NOFLX == 0 .AND. FCH4 == 1 ) THEN
       PEXTRA(KIDIA:KFDIA,JT, IEXTR_DD) = PEXTRA(KIDIA:KFDIA,JT, IEXTR_DD) - &
...
IF (.NOT.LCHEM_ARPCLIM) THEN
  ...
  & KIDIA , KFDIA  , KLON , KLEV , NCHEM ,NCHEM_SCV,  NSTEP, .FALSE., KCHEM_WETDEP, PTSTEP,&
  & PCHEMPHR, PRSF1, ZDELP, PTP, ZFPLCL, ZFPLCN, ZAP, ZLU, ZDUM, ZRP, ZSP, PLSM, ZCON,ZTENC0,&
  &   ZTENC1, ZWLOSS )
ENDIF
...
IF (.NOT.LCHEM_ARPCLIM) THEN
  ...
  & ( YDEAERATM, YDCHEM, YDCOMPO, YDMODEL%YRML_PHY_EC%YRECLDP, YDMODEL%YRML_PHY_AER%YREDBUG, YGFL, &
  & KIDIA , KFDIA  , KLON , KLEV , NCHEM ,NCHEM_SCV , NSTEP, .FALSE. , KCHEM_WETDEP, PTSTEP,&
  & PCHEMPHR, PRSF1, ZDELP, PTP, ZFPLSL, ZFPLSN, ZAP, ZLP, ZIP, ZRP, ZSP, PLSM, ZCON,ZTENC0,&
...
! Set CH4 constant if LCHEM_FIX_CH4

IF (LCHEM_FIX_CH4 .AND. RCH4CONST > 0.0_JPRB ) THEN
