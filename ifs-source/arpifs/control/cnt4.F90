...
!   F. Vana  (Jan 2023): Nemo only called during predictor.
! End Modifications
!      ----------------------------------------------------------------
...
CLASS(DBASE), TARGET,INTENT(INOUT) :: YDODB
...
DO
 ...
 &                YDFIELDS%YRSPEC%SP3D,YDFIELDS%YRSPEC%SP2D,YDFIELDS%YRSPEC%SP1D, .TRUE.)
  ...
  IF (LECV) THEN
    CALL GET_VALUE_FROM_ECV(YDGEOMETRY, YDFIELDS%FIELD5_ECV, 'SOLAR_CONSTANT', ZI05, LLFOUND)
  ELSE
    LLFOUND = .FALSE.
  ENDIF
  IF (LLFOUND) THEN
  ...
  IF(LMCC04.AND.LNEMOCOUP) THEN
    ...
    ENDIF
    ! Make it compliant with ICI
    LLNEMOUPD=LLNEMOUPD.AND.(NCURRENT_ITER==0) ! only predictor step does the update
    IF(LLNEMOUPD) THEN
      CALL GSTATS(33,0)
  ...
  IF (NSTOP > 0.OR.LDFI) THEN
    ...
    IF (.NOT.LELAM.OR.NSTOP > 0) THEN
      IF (YDDYNA%LNHEE.AND.YDDYNA%LSI_NHEE) THEN
        CALL SUNHEESI(YDGEOMETRY,YDRIP,YDDYN,YDDYNA,NULOUT,.FALSE.)
      ELSEIF (YDDYNA%LNHEE) THEN
        CALL SUNHSI(YDGEOMETRY,YDRIP,YDDYN,YDDYNA,YDEDYN,YDDYNA%LNESC,NULOUT,.FALSE.)
      ELSEIF (YDDYNA%LNHQE.AND..NOT.YDDYNA%LNHQE_SIHYD) THEN
        CALL SUNHQESI(YDGEOMETRY,YDRIP,YDDYN,YDDYNA,NULOUT,.FALSE.)
      ELSE
        IF (LELAM) THEN
          IF (LESIDG) CALL SUEHEG(YDGEOMETRY,YDDYN,YDDYNA,YDEDYN,YDRIP)
        ELSE
          IF (LSIDG) CALL SUHEG(YDGEOMETRY,YDRIP,YDDYN, YDDYNA)
        ENDIF
      ENDIF
    ENDIF
    ...
    IF (.NOT.LELAM.AND.LSTRHD) CALL SUHDU(YDGEOMETRY,YDMODEL%YRML_GCONF,YDDYN,YDDYNA)
  ENDIF
  ...
  IF (LEJK) CALL EVARJKINI(YDGEOMETRY,YDGFL,YDGFL5,YDDYN,YDDYNA,YDMODEL%YRML_GCONF,YDMODEL%YRML_LBC,YDFIELDS%YRSPEC)
  ...
  ENDIF
  IF( YDFIELDS%LHAS_ECV .AND. NDIAECV>0 .AND. JSTEP==0)  CALL FIELD_CONTAINER_GPNORM(YDGEOMETRY,YDFIELDS%FIELD5_ECV)
...
!J-------------------------
  ...
  IF (JSTEP  <  NSTOP) THEN
    IF (LWCOU) THEN
      !!!! WAM is called every NSTPW time step(s)
      ! CONTROL THE WAVES RESTART
      LLWRRC  =  IRESTS((JSTEP+NSTPW)/NFRRES)  ==  1.AND.&
       & MOD((JSTEP+NSTPW),NFRRES)  ==  0

      ! CONTROL THE ATMOSPHERE RESTART
      LLWRRE  =  IRESTS((JSTEP+1)/NFRRES)  ==  1.AND.&
       & MOD(JSTEP+1,NFRRES)  ==  0
  ...
  !IF (LLWVTIME.AND.(NCURRENT_ITER==NSITER)) THEN ! OK but Wam fields may be
  !            missing at the I/O event. (To be fixed there...)
  IF (LLWVTIME) THEN
...
#ifdef WITH_NEMO
  ! Make sure however this is complies with the I/O trigger
  IF(NFRCO /= 0 .AND. LNEMOCOUP .AND. LMCC04 .AND. (NCURRENT_ITER == NSITER)) THEN
  ...
  IF (LLELSRW .AND. IO_SERV_C001%LIO_SERV_RD) THEN
    IF (LLSWAP .OR. (NSTEP == 0)) THEN
      CALL ERLBC_POST_REQ(YDGEOMETRY,YDGFL,YDGFL5,YDMODEL%YRML_GCONF,YDMODEL%YRML_LBC,YDDYNA,YDSPEC)
    ENDIF
  ENDIF
...
IF ( ((LLOBSC1.AND.LOBS) .OR. (((&
  ...
  IF(LLOBSC1) CALL DEALMOD(YDMODEL%YRML_PHY_RAD%YRRADF,YDMODEL%YRML_PHY_G%YRSLPHY, &
   &                       YDMODEL%YRML_SPPT, YDMODEL%YRML_GCONF%YRSPPT_CONFIG)
