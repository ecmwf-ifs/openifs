...
!   F. Vana and S. Lang (Jan 2023): more flexibility to phys-dyn interface
! End Modifications
!-------------------------------------------------------------------------------
...
USE YOMCT0       , ONLY : NCONF, LR2D, LELAM, &
 &                        LSCREEN, LIFSTRAJ, LIFSMIN, LECMWF
...
#include "smap_process.intfb.h"
#include "smap_update.intfb.h"
...
ASSOCIATE(YCOMP=>YGFL%YCOMP,NUMFLDS=>YGFL%NUMFLDS, &
 ...
 & NVEXTRDYN=>YDDPHY%NVEXTRDYN, LPC_SPHY=>YDDYNA%LPC_SPHY, &
 & LPC_FULL=>YDDYNA%LPC_FULL, LSLAG=>YDDYNA%LSLAG, LTWOTL=>YDDYNA%LTWOTL, LPGFSF=>YDDYNA%LPGFSF, &
 & LNHQE=>YDDYNA%LNHQE, LNHQE_SIHYD=>YDDYNA%LNHQE_SIHYD, LGRADSP=>YDDYNA%LGRADSP, &
 ...
 & LERADI=>YDEPHY%LERADI, LERAINGG=>YDEPHY%LERAINGG, &
 ...
 & NSTOP=>YDRIP%NSTOP, NSTART=>YDRIP%NSTART, &
...
! ----------------------------------------------------------------------
  ...
  ! Ensure all elements of ZWRL9 are initialised to a sensible value:
  ZWRL9(:,:,:) = 0.0_JPRB
...
!   ----------------------------------------------------
  ...
  CALL ZPGTERM%INIT(YDGEOMETRY, PGEO0=ZGEO0, LPGFSF=LPGFSF)
...
IF (LSLAG) THEN
  IF (LSLPHY) THEN
    ...
    ALLOCATE (ZPBSPPTCLEARP9(1,NFLEVG,NGPBLKS))
  ELSE
    ! minimal alloc to avoid false alarms with bound checking option
    IF (.NOT.LR2D) THEN
...
IF (.NOT.LR2D) THEN
  ...
  IF(LSLAG) THEN
    ...
    CALL CALL_SL_DRV(YDGEOMETRY,YDGMV,YDMODEL,YDGFL%GFL,YDSL,IVSEPC,IVSEPL,PSLBUF1,PSLBUF2,&
      & ZWRL9, YDGMV%GMVT1,YDGMV%GMVT1S,YDGFL%GFLT1,GFLPC,ZPBUP9,ZPBVP9,ZPBTP9,ZPBGFLP9,&
      & ZPBSPPTCLEARP9,ZEXTRADYN)
...
! cop
  ...
  IF (KCHEM_NOXADV>0) THEN
    ...
    ! NOTE: This is not compatible with ICI scheme yet
...
!$OMP END PARALLEL DO
  ...
  IF((LEPHYS.OR.LHELDSUAREZ) .AND. LAGPHY) THEN
    ...
    CALL EC_PHYS_DRV(YDGEOMETRY,YDFIELDS,YDFIELDS%VARIABLES,YDGMV,YDSURF,YDMODEL,CLCONF,&
        & PSLBUF2,ZPBUP9,ZPBVP9,ZPBTP9,ZPBGFLP9,ZPBSPPTCLEARP9,ZEXTRADYN,SD_PF,&
  ...
  IF (ALLOCATED(ZPBSPPTCLEARP9)) DEALLOCATE(ZPBSPPTCLEARP9)
