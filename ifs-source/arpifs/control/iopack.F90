...
SUBROUTINE IOPACK(YDGEOMETRY,YDFIELDS,YDGFL5,YDMODEL,CDCONF,PTRAJEC)
...
USE YOMVAR             , ONLY : LTRREF, LECV
USE YOMOPH0            , ONLY : LINC
USE YOMJBECV           , ONLY : YRECVCONFIG, YRECVDATA, NDIAECV
USE YOMTRAJ            , ONLY : TRAJ_TYPE
USE SPECTRAL_FIELDS_MOD, ONLY : SPECTRAL_FIELD, ASSIGNMENT(=)
USE FIELD_CONTAINER_OPER_MOD , ONLY : FIELD_CONTAINER_WRITE_FILE,FIELD_CONTAINER_GPNORM
...
ASSOCIATE(NFLEVL=>YDGEOMETRY%YRDIMV%NFLEVL, &
 & NSTOP=>YDRIP%NSTOP, NVIDS_ECV=>YRECVDATA%NVIDS_ECV, VIDS_ECV=>YRECVDATA%VIDS_ECV)
!     ------------------------------------------------------------------
...
IF (.NOT. ANY(SPREAD(CDCONF,1,13)==(/'A','B','D','F','I','J','L','Q','R','S','T','V','f'/))) THEN
  WRITE(NULERR,*) "IOPACK: CDCONF=",CDCONF
...
IF (LLA) THEN
 ...
 &              YDMODEL,CDCONF,PTRAJEC=PTRAJEC,YDMCUF=YDFIELDS%YMCUF)
    IF(LECV .AND. NVIDS_ECV > 0 .AND. NSTEP==0) THEN
      CALL SUECFNAME(LINC,'m',NSTEP,YDRIP%TSTEP,NSTOP,CLFNGG,CLFNSH)
      WRITE(CLFNGG(1:1),'(A1)') YRECVCONFIG%CFNECVINC
      IF (NCONF == 131) THEN 
        IF (NDIAECV>0) THEN
          WRITE(NULOUT,*) 'NORMS 4I ECV'
          CALL FIELD_CONTAINER_GPNORM(YDGEOMETRY,YDFIELDS%FIELD_ECV,VIDS_ECV(1:NVIDS_ECV))
        ENDIF
        CALL FIELD_CONTAINER_WRITE_FILE(YDGEOMETRY,YDFIELDS%FIELD_ECV,VIDS_ECV,CLFNGG)
      ELSEIF (NCONF == 1) THEN 
        IF (NDIAECV>0) THEN
          WRITE(NULOUT,*) 'NORMS FG HR ECV'
          CALL FIELD_CONTAINER_GPNORM(YDGEOMETRY,YDFIELDS%FIELD5_ECV,VIDS_ECV(1:NVIDS_ECV))
        ENDIF
        CALL FIELD_CONTAINER_WRITE_FILE(YDGEOMETRY,YDFIELDS%FIELD5_ECV,VIDS_ECV,CLFNGG)
      ENDIF
    ENDIF
  ENDIF
ENDIF
