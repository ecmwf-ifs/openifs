...
SUBROUTINE QMFIXER(YDGEOMETRY,YDGMV,YDDPHY,YGFL,YDML_DYN,KL0,PLSCAW,PB1,YDSL,PGMVS,PGMVT1S,PGFL,&
                 & PGFLT1,PEXTRADYN)
!
...
! compute a scaling factor for the weight to allow calculation of correction for small numbers
  IF (ZDM(JFIX)>=0.0_JPRB) THEN
    ZSCALE=MAX(ZDM(JFIX),ZEPS)
  ELSE
    ZSCALE=MIN(ZDM(JFIX),-ZEPS)
  ENDIF
  JGFL=NMFIXFLDS(JFIX)
...
!$OMP END PARALLEL DO
  ...
  DO JFIX=1,NFLDSFIX
    JGFL=NMFIXFLDS(JFIX)   
    IF (.NOT.LLMFPROP(JFIX).AND.(ZTOTMASS1(1,JFIX)>ZGFLMIN)) THEN
