...
INTEGER(KIND=JPIM) :: I, IEND, IPROCA, IPROCB,&
 & IRET, ISDDH, IT, IUNIT, IWSTEP, JLEV, ILEN_CPL, ILEN_IO, J2D, JPERT, JRF, IRF,&
...
ASSOCIATE(NDIM=>YGFL%NDIM, NDIM0=>YGFL%NDIM0, NDIM9=>YGFL%NDIM9, &
 ...
 & CBEGDAT=>YDEWCOU%CBEGDAT, LWCOU=>YDEWCOU%LWCOU, NSTPW=>YDEWCOU%NSTPW, &
 & NGPTOT=>YDGEM%NGPTOT, &
  ...
  IF (YDSPP_CONFIG%LSPP) THEN
    DO JPERT=1, YDSPP_CONFIG%SM%NACT
      ...
      DO JRF=1, YDSPP_CONFIG%SM%PN(JPERT)%NRF
        ...
        !
        CALL IO_GET(YL_IOSTREAM, YL_IOREQUEST,PR2=YDSPP%SP_ARP(IRF)%SF%SP2D)
        IF (YDSPP_CONFIG%NPATFR/=1) THEN
          CALL IO_GET(YL_IOSTREAM, YL_IOREQUEST,PR3=YDSPP%GP_ARP0(IRF)%GP2D)
          CALL IO_GET(YL_IOSTREAM, YL_IOREQUEST,PR3=YDSPP%GP_ARP1(IRF)%GP2D)
          CALL IO_GET(YL_IOSTREAM, YL_IOREQUEST,PR3=YDSPP%GP_ARP(IRF)%GP2D)  
        ENDIF
      ENDDO
    ENDDO
  ENDIF
