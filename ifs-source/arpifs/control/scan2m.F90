...
!   R. El Khatib 21-Apr-2021 option NFPWRITE=2 to write output files but disable norms
!   R. El Khatib 29-Sep-2021 fix t1 arrays initialisation
!-----------------------------------------------------------------------------
...
USE YOMCT0             , ONLY : NCONF, LIFSMIN, LARPEGEF, &
...
USE YOMTRAJ            , ONLY : NTRAJ_CST, MSTEPTRAJW, LPRTTRAJ, MTRAJ_GRIB,&
 &                              MKINDTRAJ, MTRAJ_GRIB2, MAIN_GRIB,&
...
REAL(KIND=JPRB),ALLOCATABLE :: ZPSD_VX(:,:,:), ZPSD_VF(:,:,:), ZPSD_OC(:,:,:), ZPSD_VV(:,:,:), ZPSD_VN(:,:,:)
...
INTEGER(KIND=JPIM) :: IBL, ICEND, IEND, IST, JKGLO, JLEV, JL,&
 & IGPCOMP, JGFL, ISTEP, JSTEP, IDUMMY(1), IFPLAG, ITIME, J

CHARACTER(LEN=20) :: CLFILE

LOGICAL :: LLTRAJHR, LLFIRST, LLAST, LLFPOS, LLOCEDELAY, LLOCE
LOGICAL :: LLCOBSALL, LL_GP_DONE, LL_UPD_DONE
LOGICAL :: LLALLOCATED_GMVT1, LLALLOCATED_GMVT1S, LLALLOCATED_GFLT1
...
ASSOCIATE(YDGFL=>YDFIELDS%YRGFL,YDGMV=>YDFIELDS%YRGMV, YDSURF=>YDFIELDS%YRSURF, &
 ...
 & YGFL=>YDMODEL%YRML_GCONF%YGFL,YDDYN=>YDMODEL%YRML_DYN%YRDYN,YDDYNA=>YDMODEL%YRML_DYN%YRDYNA, &
...
ASSOCIATE(NDIM=>YGFL%NDIM, NDIM5=>YGFL%NDIM5, NUMFLDS=>YGFL%NUMFLDS, &
 ...
 & YT1=>YDGMV%YT1, NDIM1=>YGFL%NDIM1, &
 ...
 & SD_OC=>YDSURF%SD_OC, YSD_OC=>YDSURF%YSD_OC, YSD_OCD=>YDSURF%YSD_OCD, &
...
!$OMP END PARALLEL DO
          ...
          CALL WRITE_GRID_TRAJ(YDGEOMETRY,YDRIP,CLFILE,MTRAJ_GRIB,IDUMMY,MTYPE_SURF_TRAJ,&
           & 0,NGP5,ZTRAJ_BUF,ZDUMMY,NINTERPTRAJ,LLFIRST,LLAST,MTRAJ_GRIB2)
...
IF (LLCOBSALL) THEN
  ...
  ALLOCATE(ZPSD_OC(NPROMA,YSD_OCD%NDIM,NGPBLKS))
...
IF (LLFPOS) THEN
  ...
  IF (NSTOP > 0 .AND. LFPPACKING) THEN
    ...
    CALL FULLPOS_PRECOND(YDGEOMETRY,YDFIELDS,YDMODEL,YLGMV,YLGFL,NSTEP,YLSURF,ZCFUBUF,ZXFUBUF)
...
IF (CDCONF(4:4) /= '0') THEN
  CALL GSTATS(1228,0)
  LLALLOCATED_GMVT1=ALLOCATED(YDGMV%GMVT1)
  LLALLOCATED_GMVT1S=ALLOCATED(YDGMV%GMVT1S)
  LLALLOCATED_GFLT1=ALLOCATED(YDGFL%GFLT1)
  IF(.NOT.LLALLOCATED_GMVT1)  THEN
    ALLOCATE(YDGMV%GMVT1(NPROMA,NFLEVG,YT1%NDIM,NGPBLKS))
    IF (SIZE(YDGMV%GMVT1) > 0) YDGMV%GMVT1(1,1,1,1)=0._JPRB ! enforce allocation here
  ENDIF
  IF(.NOT.LLALLOCATED_GMVT1S) THEN
    ALLOCATE(YDGMV%GMVT1S(NPROMA,YT1%NDIMS,NGPBLKS))
    IF (SIZE(YDGMV%GMVT1S) > 0) YDGMV%GMVT1S(1,1,1)=0._JPRB ! enforce allocation here
  ENDIF
  IF(.NOT.LLALLOCATED_GFLT1)  THEN
    ALLOCATE(YDGFL%GFLT1(NPROMA,NFLEVG,NDIM1,NGPBLKS))
    IF (SIZE(YDGFL%GFLT1) > 0) YDGFL%GFLT1(1,1,1,1)=0._JPRB  ! enforce allocation here
  ENDIF
!$OMP PARALLEL PRIVATE(J)
  IF(.NOT.LLALLOCATED_GMVT1)  THEN
!$OMP DO SCHEDULE(STATIC)
    DO J=1,NGPBLKS
      YDGMV%GMVT1(:,:,:,J)=0.0_JPRB
    ENDDO
!$OMP END DO NOWAIT
  ENDIF
  IF(.NOT.LLALLOCATED_GMVT1S) THEN
...
IF (LLCOBSALL) THEN 
  ...
  ITIME=INT(NSTEP*TSTEP)
  CALL YDGOM5%MODEL_IN(&
    & YDGEOMETRY,YDGMV,YDSURF,YDMODEL%YRML_PHY_MF%YRPHY,YDMODEL%YRML_GCONF,&
    & YDMODEL%YRML_DYN%YRSLINT,YDMODEL%YRML_DYN%YRDYNA,&
    & NDIMGMV,YECVAR,YGFL,&
    & YDFIELDS%YEC_PHYS_FIELDS%YRPHYSMWAVE,GFL,GMV,GMVS,&
    & ZPSP_SB,ZPSP_SG,ZPSP_SL,ZPSP_RR,ZPSP_CL,ZPSP_X2,ZPSP_CI,&
    & ZPSD_VF,ZPSD_OC,ZPSD_VV,ZPSD_VD,ZPSD_WS,ZPSD_VX,ZPSD_VN,YDFIELDS%FIELD5_ECV,ITIME,&
    & LL_GP_DONE,LL_UPD_DONE)
  DEALLOCATE(ZPSP_SB,ZPSP_SG,ZPSP_SL,ZPSP_RR,ZPSP_CL,ZPSP_CI,ZPSP_X2,ZPSD_WS,ZPSD_VD,ZPSD_VX,ZPSD_VF,ZPSD_OC,ZPSD_VV,ZPSD_VN)
ENDIF
...
IF (LD_TST_GPGFL) THEN
  CALL GSTATS(1032,0)
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(JKGLO,ICEND,IBL,JGFL)
...
CONTAINS
  ...
  SUBROUTINE GPOPERTRAJ(YDDIM,YDDYN,CDNAME,YDSURF,KBL,KEND,KFLD,PFIN,PFOUT)
    TYPE(TDIM),INTENT(IN) :: YDDIM
    TYPE(TDYN),INTENT(IN) :: YDDYN
    TYPE(TSURF),INTENT(INOUT) :: YDSURF
    INTEGER(KIND=JPIM),INTENT(IN) :: KBL,KEND,KFLD
    ...
    CALL GPOPER(YDDIM,YDDYN,CDNAME,YDSURF,KBL=KBL,PFIELD=ZTEMP)

    IF (PRESENT(PFOUT)) PFOUT(1:KEND,1:KFLD) = ZTEMP(1:KEND,:)
  END SUBROUTINE GPOPERTRAJ
