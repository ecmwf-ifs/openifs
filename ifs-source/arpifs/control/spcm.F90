...
LL_CALL_SPCIMPF=LIMPF.AND.(.NOT.(YDDYNA%LNHEE.AND.(.NOT.YDDYNA%LSI_NHEE)))
...
IPART_2B2C=2
IPART_2D=3
IF (YDDYNA%LNHDYN.AND.YDDYNA%LNHQE.AND.(.NOT.YDDYNA%LNHQE_SIHYD)) THEN
  IF (NITERHELM==0) THEN
    ! Predictor:
...
IF (CDCONF == 'A'.OR.CDCONF == 'I') THEN
 ...
 IF (LR3D) THEN
  ...
  IF (LLDOSI) THEN
    ...
    IF (YDDYNA%LNHQE.AND.(.NOT.YDDYNA%LNHQE_SIHYD)) ALLOCATE(ZSPBUFG(NFLEVG,ISPEC2V,3))

    IF (LRSIDDH) THEN
      ALLOCATE(ZSPTNDSI_VORG(NFLEVG,ISPEC2V))
      ALLOCATE(ZSPTNDSI_DIVG(NFLEVG,ISPEC2V))
      ALLOCATE(ZSPTNDSI_TG(NFLEVG,ISPEC2V))
      IF (YDDYNA%LNHDYN) ALLOCATE(ZSPTNDSI_SPDG(NFLEVG,ISPEC2V))
      IF (YDDYNA%LNHDYN) ALLOCATE(ZSPTNDSI_SVDG(NFLEVG,ISPEC2V))
    ELSE
      ALLOCATE(ZSPTNDSI_VORG(1,1))
      ALLOCATE(ZSPTNDSI_DIVG(1,1))
      ALLOCATE(ZSPTNDSI_TG(1,1))
      IF (YDDYNA%LNHDYN) ALLOCATE(ZSPTNDSI_SPDG(1,1))
      IF (YDDYNA%LNHDYN) ALLOCATE(ZSPTNDSI_SVDG(1,1))
    ENDIF
...
!$OMP END PARALLEL DO
    ...
    IF (LL_CALL_SPCIMPF) THEN
      CALL TRMTOS(YDGEOMETRY,YDDYNA%LNHDYN,YDDYNA%LNHX,PSPVOR=YDSP%VOR,PSPDIV=YDSP%DIV,PSPT=YDSP%T,PSPSPD=YDSP%SPD,&
    ...
    IF (LLONEM) THEN
      ...
      IF (YDDYNA%LNHEE.AND.(.NOT.YDDYNA%LSI_NHEE)) THEN
        ...
        DO JMLOC=NPTRMF(MYSETN),NPTRMF(MYSETN+1)-1
          ...
          ISTA=NSPSTAF(IM)
          IEND=ISTA+2*(NSMAX+1-IM)-1
          CALL SPNHSI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,JMLOC,ISTA,IEND,LLONEM,&
      ...
      ELSEIF (YDDYNA%LNHEE.AND.YDDYNA%LSI_NHEE) THEN
        ...
        IF (LIMPF) THEN
          DO JMLOC=NPTRMF(MYSETN),NPTRMF(MYSETN+1)-1
            ...
            ISTA=NSPSTAF(IM)
            IEND=ISTA+2*(NSMAX+1-IM)-1
            CALL SPNHEESI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,JMLOC,ISTA,IEND,LLONEM,&
             & ZSPVORG,ZSPDIVG,ZSPTG,ZSPSPG,ZSPSPDG,ZSPSVDG,&
      ...
      ELSEIF (YDDYNA%LNHQE.AND.(.NOT.YDDYNA%LNHQE_SIHYD)) THEN
        ...
        DO JITER=0,NITERHELM+1
          ...
          IF (JITER == 0) THEN
            ZSPBUFG(:,:,1)=ZSPDIVG(:,:)
          ENDIF
          CALL SPNHQESI_PRODSKAP(LLONEM,YDGEOMETRY,YDDYNA,YDGMV,IDIMV,IDIMH,ISPEC2V,ZSPBUFG(1,1,1),ZSPBUFG(1,1,3))
          ...
          IF (LIMPF) THEN
            DO JMLOC=NPTRMF(MYSETN),NPTRMF(MYSETN+1)-1
              ...
              ISTA=NSPSTAF(IM)
              IEND=ISTA+2*(NSMAX+1-IM)-1
              CALL SPNHQESI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,&
      ...
      ELSE
        ...
        IF (LIMPF) THEN
          DO JMLOC=NPTRMF(MYSETN),NPTRMF(MYSETN+1)-1
            ...
            ISTA=NSPSTAF(IM)
            IEND=ISTA+2*(NSMAX+1-IM)-1
            CALL SPCSI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,JMLOC,ISTA,IEND,LLONEM,&
        ...
        ELSE
          ...
          ENDDO
        ENDIF
        IF(YDDYNA%LNHQE.AND.YDDYNA%LNHQE_SIHYD) ZSPSPDG(:,:)=0.0_JPRB
    ...
    ELSE
      ...
      IF (NSPEC2V > 0) THEN
        ...
        IF (YDDYNA%LNHEE.AND.(.NOT.YDDYNA%LSI_NHEE)) THEN
          ...
          CALL SPNHSI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,IMLOC,1,NSPEC2V,LLONEM,&
           & ZSPVORG,ZSPDIVG,ZSPTG,ZSPSPG,ZSPSPDG,ZSPSVDG,&
           & ZSPTNDSI_VORG,ZSPTNDSI_DIVG,ZSPTNDSI_TG,ZSPTNDSI_SPDG,ZSPTNDSI_SVDG)
        ...
        ELSEIF (YDDYNA%LNHEE.AND.YDDYNA%LSI_NHEE) THEN
          ...
          IF (LIMPF) THEN
            CALL SPNHEESI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,IMLOC,1,NSPEC2V,LLONEM,&
             & ZSPVORG,ZSPDIVG,ZSPTG,ZSPSPG,ZSPSPDG,ZSPSVDG,&
             & ZSPTNDSI_VORG,ZSPTNDSI_DIVG,ZSPTNDSI_TG,ZSPTNDSI_SPDG,ZSPTNDSI_SVDG,PSPAUXG=ZSPAUXG)
          ELSE
            CALL SPNHEESI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,IMLOC,1,NSPEC2V,LLONEM,&
        ...
        ELSE
          ...
          IF (LIMPF) THEN
            CALL SPCSI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,IMLOC,1,NSPEC2V,LLONEM,&
             & ZSPVORG,ZSPDIVG,ZSPTG,ZSPSPG,ZSPTNDSI_VORG,&
             & ZSPTNDSI_DIVG,ZSPTNDSI_TG,PSPAUXG=ZSPAUXG)
          ELSE
            CALL SPCSI(YDGEOMETRY,YDLDDH,YDMODEL%YRML_GCONF%YRRIP,YDDYN,YDDYNA,IM,IMLOC,1,NSPEC2V,LLONEM,&
    ...
    CALL TRSTOM(&
       & YDGEOMETRY,YDDYNA%LNHDYN,YDDYNA%LNHX,PSPVORG=ZSPVORG,PSPDIVG=ZSPDIVG,PSPTG=ZSPTG,PSPSPDG=ZSPSPDG,&
       & PSPSVDG=ZSPSVDG,PSPSPG=ZSPSPG,&
    ...
    IF (LRSIDDH) THEN
      CALL TRSTOM(&
       & YDGEOMETRY,YDDYNA%LNHDYN,YDDYNA%LNHX,PSPVORG=ZSPTNDSI_VORG,PSPDIVG=ZSPTNDSI_DIVG,PSPTG=ZSPTNDSI_TG,&
