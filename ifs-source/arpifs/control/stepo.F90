...
!   F. Vana and S. Lang January 2023: SPPT updated for ICI
!
! End Modifications
!------------------------------------------------------------------------------
...
LOGICAL :: LL_DFISTEP
LOGICAL :: LLFPOS
LOGICAL :: LLPRED       ! true if in the first predictor step
LOGICAL :: LLLCOR       ! true if in the last corrector step
...
ASSOCIATE(NPROMA=>YDDIM%NPROMA, NSPEC2=>YDDIM%NSPEC2, &
 & NFLEVG=>YDDIMV%NFLEVG, LPC_FULL=>YDDYNA%LPC_FULL, &
...
! Keys controlling SPP/SPPT perturbations for ICI
LLPRED=.NOT.LPC_FULL .OR. NCURRENT_ITER==0
LLLCOR=.NOT.LPC_FULL .OR. NCURRENT_ITER==NSITER
...
IF(CDCONF(2:2) /= '0') THEN
  IF (LELAM) THEN
    CALL ETRANSINVH(YDGEOMETRY,YDFIELDS%YRGFL,YDFIELDS%YRGMV,YDMODEL%YRML_DIAG,YDMODEL%YRML_GCONF,YDMODEL%YRML_DYN%YRDYNA,&
    & CDCONF, YDSPEC,YDMTRAJ)
  ELSE
 ...
 &                 YDMODEL%YRML_DYN%YRDYN%LRFRIC,YDMODEL%YRML_DYN%YRDYN%RKRF,CDCONF,YDSPEC,YDMTRAJ=YDMTRAJ,LDFPOS=LLFPOS)
  ...
  IF (YDSPPT_CONFIG%LSPSDT.AND.(CDCONF(2:2)=='A').AND.LLPRED) THEN
    !
    ...
    DO J2D=1,YDSPPT%N2D
      ...
      ELSE
        ...
        ELSE
          ...
          ZW0=1._JPRB - ZW1
          YLGPAUX%NGRIB(1) = YDSPPT%YGPSDT0(J2D)%NGRIB(1)
      ...
      IF (YDSPPT_CONFIG%LWRPATTRUN_SDT.AND.ANY(NSTEP==YDSPPT_CONFIG%NSTEP_PATTRUN(1:YDSPPT_CONFIG%NWRPATTRUN_SDT))) THEN
        ...
        CALL WRITE_SPEC_GRIB(YDGEOMETRY,YDRIP,TRIM(CLPATOUTN),YDSPPT%YSPSDT_AR1(J2D)%SF,CDMODE="w",CDLEVTYPE="AL")
      ENDIF
    ENDDO
    !
    IF (NPATFR_SPPT>1) THEN
  ...
  ENDIF
  !
  IF (YDSPP_CONFIG%LSPP.AND.(CDCONF(2:2)=='A').AND.LLPRED) THEN
    !
    ...
    DO JPERT=1, YDSPP_CONFIG%SM%NACT
      ...
      DO JRF=1, YDSPP_CONFIG%SM%PN(JPERT)%NRF
        ...
        IF (YDSPP_CONFIG%NPATFR==1) THEN    !Pattern evolves every timestep
          CALL SUM_ARPS(YDSPP%SP_ARP(IRF))
          CALL SPECTRAL_NORM(YDSPP%SP_ARP(IRF)%SF, 'YDSPP%SP_ARP['//TRIM(CLLABEL) //']'//CLJRF)
          CALL SPECTRAL_NORM(YDSPP%SP_ARP(IRF)%SFSUM, 'sum YDSPP%SP_ARP['//TRIM(CLLABEL) //']'//CLJRF)
          CALL SPEC2GRID(YDGEOMETRY, YDSPP%SP_ARP(IRF)%SFSUM, YDSPP%GP_ARP(IRF) )
        ELSE
          IF (NSTEP==NSTART.OR.ITMODSPP==1) THEN  !Initial pattern state/First timestep between pattern updates (NPATFR>1)
            ...
            ENDIF
            CALL SPEC2GRID(YDGEOMETRY, YDSPP%SP_ARP(IRF)%SFSUM, YDSPP%GP_ARP1(IRF) )
          ENDIF
          !
          !
          ...
          ELSE
            ...
            ZW0=1._JPRB - ZW1
            YLGPSPP%NGRIB(1) = YDSPP%GP_ARP0(IRF)%NGRIB(1)
          ...
          ENDIF
        ENDIF
        IF (YDSPP_CONFIG%LGPNORM_RF) THEN
    ...
    IF (YDSPP_CONFIG%LWRPATTRUN.AND.ANY(NSTEP==YDSPP_CONFIG%NSTEP_PATTRUN(1:YDSPP_CONFIG%NWRPATTRUN))) THEN
      DO JPERT=1, YDSPP_CONFIG%SM%NACT
        ...
        DO JRF=1, YDSPP_CONFIG%SM%PN(JPERT)%NRF
          ...
          ENDIF
          WRITE(NULOUT,*) TRIM(CLPATOUTN)
          CALL WRITE_SPEC_GRIB(YDGEOMETRY,YDRIP,TRIM(CLPATOUTN),YDSPP%SP_ARP(IRF)%SF,CDMODE="w",CDLEVTYPE="AL")
        ENDDO
      ENDDO
    ENDIF
  ENDIF
...
IF(CDCONF(7:8) /= '00')THEN
  ...
  ENDIF
  IF (LELAM) THEN
    CALL ETRANSDIRH(YDGEOMETRY,YDMODEL%YRML_DYN%YRDYNA%LGRADSP, &
     & YDFIELDS%YRGFL,YDFIELDS%YRGMV,&
     & CLCONF_TRANSDIR, YDMODEL%YRML_GCONF%YRDIMF%NFTHER, YDSPEC)
  ELSE
...
!$OMP END PARALLEL DO
  ...
  IF (YDSPPT_CONFIG%LSPSDT.AND.CDCONF(9:9) == 'A'.AND.LLLCOR) THEN
    !
    DO J2D=1,YDSPPT%N2D
      IF (YDSPPT_CONFIG%LRESETSEED_SDT.AND.MOD(NSTEP,YDSPPT_CONFIG%RESETSEEDFRQ_SDT)==0.AND.NSTEP>0) THEN
      ...
      !
      IF (YDSPPT_CONFIG%LWRITE_ARP) THEN
        CALL WRITE_SPEC_GRIB(YDGEOMETRY,YDRIP,YDSPPT_CONFIG%COPATSP_SDT(J2D),YDSPPT%YSPSDT_AR1(J2D)%SF,CDMODE='a',CDLEVTYPE="AL")
      ENDIF
    ENDDO
  ENDIF !LSPSDT
  IF (YDSPP_CONFIG%LSPP.AND.CDCONF(9:9) == 'A'.AND.LLLCOR) THEN
    IF (YDSPP_CONFIG%LRESETSEED.AND.MOD(NSTEP,YDSPP_CONFIG%RESETSEEDFRQ)==0.AND.NSTEP>0) THEN
      DO JPERT=1, YDSPP_CONFIG%SM%NACT
        ! 
        !   loop over perturbations
        !
        ...
        DO JRF=1, YDSPP_CONFIG%SM%PN(JPERT)%NRF
          ...
          ISEED= KGET_SEED_SPP(YDSPP_CONFIG, JPERT,JRF, KMEMBER=NENSFNB, KSHIFT=YDSPP_CONFIG%SHIFTSEED, &
               & LRESEED=.TRUE., LABSTIME=YDSPP_CONFIG%LABSTIMSEED, LDVERBOSE=.TRUE., PTSTEP=TSTEP)
