...
USE ALGORITHM_STATE_MOD,ONLY : GET_NUPTRA, L_IN_WRITE_INCREMENT, L_4V_ECV
...
IF ((NCONF==131 .AND. LTWINC) .OR. (L_OOPS .AND. L_IN_WRITE_INCREMENT())) THEN
  ...
  ELSE
    ...
    CALL IGRIB_SET_VALUE(KGRIB_HANDLE,'totalNumberOfIterations',MUPTRA)
  ENDIF
ELSEIF (L_OOPS .AND. L_4V_ECV()) THEN
  CALL IGRIB_SET_VALUE(KGRIB_HANDLE,'localDefinitionNumber',36)
  CALL IGRIB_SET_VALUE(KGRIB_HANDLE,'anoffset',NWINOFF_4V)
ENDIF
IF (.NOT.LECMWF) THEN
