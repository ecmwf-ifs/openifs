...
SUBROUTINE SUNDDH(YDGEOMETRY,YDMODEL,LDONLY_SWITCHES)
!****
!     ------------------------------------------------------------------
...
!     D. Nemec (Jul 2021): added graupel 
!------------------------------------------------------------------------------
...
LOGICAL,INTENT(IN),OPTIONAL :: LDONLY_SWITCHES
INTEGER(KIND=JPIM) :: IMASK, JBOX, JJ, JMASK
INTEGER(KIND=JPIM) :: IPT
LOGICAL :: LLCUMUL, LLERR,LLONLY_SWITCHES
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, YDMP=>YDGEOMETRY%YRMP, &
 ...
 & YDDPHY=>YDMODEL%YRML_PHY_G%YRDPHY,YDARPHY=>YDMODEL%YRML_PHY_MF%YRARPHY, &
  ...
  & YDDIMF=>YDMODEL%YRML_GCONF%YRDIMF,YDCOMPO=>YDMODEL%YRML_CHEM%YRCOMPO, &
...
LLONLY_SWITCHES = .FALSE.
IF(PRESENT(LDONLY_SWITCHES)) LLONLY_SWITCHES = LDONLY_SWITCHES
...
WRITE (NULOUT,*) ' DIAGNOSTICS PAR DOMAINES HORIZONTAUX,LLONLY_SWITCHES=',LLONLY_SWITCHES
...
LFLEXDIA=LFLEXDIA.OR.(LSDDH .AND. NPHY==JPHYARO)
...
IF(L_OOPS .AND.LFLEXDIA) THEN
  WRITE(NULERR,*) ' ERREUR DDH *** '
  WRITE(NULERR,*) ' CODE UNDER LFLEXDIA BREAKS MODEL ENCAPSULATION'
  LLERR = .TRUE.
ENDIF
...
!             - ARPEGE/ALADIN -
    IF ((LECMWF.AND.LEPCLD).OR.(.NOT.LECMWF.AND.(.NOT.LMPA))) THEN
      ...
      IF ( L3MT.OR.LSTRAPRO )THEN
        NHDQLNVA= 5  ! total number of variables for water
        NHDQLNTD= 5  ! number of dyn. tendencies for water
        NHDQLNFD= 5  ! number of dyn. fluxes for water
      ...
      ELSE
        NHDQLNVA=5  ! total number of variables for water
        NHDQLNTD=5 !number of dynamical tendendies for water
        NHDQLNFD=5 !number of dynamical fluxes for water
