...
!   R. El Khatib 21-Jan-2020 Bugfix on Ws, Wp, Fs, Fp for interoperability
...
LOGICAL :: LLINC, LLBIP(YDQTYPE%NFPDYNB), LLMFPHYS
...
ASSOCIATE(YDCST=>YDMODEL%YRCST, YDFPGEO_DEP=>YDFPGEOMETRY%YFPGEO_DEP, YDFPGEO=>YDFPGEOMETRY%YFPGEO, YDFPGIND=>YDFPGEOMETRY%YFPGIND, &
...
!$OMP DO SCHEDULE(DYNAMIC,1)
  DO JKGLO=1,NGPTOT,NPROMA
    ...
    ELSE
      CALL VPOS(YDCST,YDQTYPE,YDNAMFPSCI,YDAFN,LDHPOS,YDGEOMETRY%YRVAB,YDGEOMETRY,YDSURF,YDXFU,KCUFNR,PMCUFGP(:,:,IBL),YDMODEL%YRML_GCONF, &
       & YDMODEL%YRML_DYN%YRDYN,YDMODEL%YRML_DYN%YRDYNA,YDMODEL%YRML_PHY_MF,1,IEND, &
...
IF (LFPOSBUF(YDFPGEOMETRY)) THEN
  IF (LDEZO.AND. NFPBOYD == 0) THEN
    ...
    CALL FPEZO2H(YDQTYPE,NFPDYNB,ICOD,LLBIP,YDCLIMO,ZWSXI,ZWDXI,YDNAMFPSCI,YDAFN,KFPXFLD,YDFPGEO,YDMODEL%YRML_PHY_EC%YREPHY, &
     & YDMODEL%YRML_PHY_MF,YDFPUSERGEO,CLBIP,LFPOSHOR,LLMFPHYS,PBUF)
  ENDIF
ENDIF
!     ------------------------------------------------------------------
END ASSOCIATE
