...
SUBROUTINE ENDPOS(YDCST,YDQTYPE,YDNAMFPSCI,YDAFN,KLMOD,YDFPVAB,YDGEOMETRY,YDML_GCONF,YDDYNA,YDML_PHY_MF,LDZ0H,KPROMA,KST,KND,KAUX,KOPLEV,CDCONF, &
...
!      I. Polichtchouk 2021: added geop. height calc for shallow-atm complete-Coriolis eqns.
...
LOGICAL :: LLICV,LLICV1,LLICV2 ! to activate the computation of CAPE & CIEN & TCVS
LOGICAL :: LLCLD1, LLCLD2 ! to activate the computation of base and top of clouds
LOGICAL :: LL1,LL2 
...
REAL(KIND=JPRB) , TARGET:: ZRHCLS(KPROMA)! RH of pbl
REAL(KIND=JPRB), POINTER :: ZRHCLSP(:)   ! pointer for RH of pbl
REAL(KIND=JPRB), TARGET :: ZTCLS(KPROMA) ! T of pbl
REAL(KIND=JPRB), POINTER :: ZTCLSP(:)    ! pointer for T of PBL
...
REAL(KIND=JPRB) :: ZLCL(KPROMA) ! Lifting condensation level (geopotential height)
REAL(KIND=JPRB) :: ZFCL(KPROMA) ! Free convection level (geopotential height)
REAL(KIND=JPRB) :: ZEL(KPROMA)  ! Equilibrium level (geopotential height)
...
ASSOCIATE(TFP=>YDAFN%TFP, TFP_DYNDS=>YDAFN%TFP_DYNDS,YDCVER=>YDGEOMETRY%YRCVER, &
...
ASSOCIATE(LFPRH100=>YDNAMFPSCI%LFPRH100, LFPQ=>YDNAMFPSCI%LFPQ, NFPCAPE=>YDNAMFPSCI%NFPCAPE, &
 ...
 & RETV=>YDCST%RETV, RG=>YDCST%RG, RTT=>YDCST%RTT, &
 & NAERO=>YGFL%NAERO, &
...
LLICV1 = YDQTYPE%LL(TFP%CAPE%ICOD) .OR. YDQTYPE%LL(TFP%CIEN%ICOD)&
 & .OR. YDQTYPE%LL(TFP%LCL%ICOD)  .OR. YDQTYPE%LL(TFP%FCL%ICOD)&
 & .OR. YDQTYPE%LL(TFP%EL%ICOD)   .OR. YDQTYPE%LL(TFP%TCVS%ICOD)
LLICV2 = YDQTYPE%LL(TFP%MUMLCAPE%ICOD).OR.YDQTYPE%LL(TFP%MLCAPE%ICOD)
LLICV=LLICV1.OR.LLICV2
...
LLCLD1=YDQTYPE%LL(TFP%PCLDCEIL%ICOD) .OR. YDQTYPE%LL(TFP%HCLDCEIL%ICOD).OR.&
  &   YDQTYPE%LL(TFP%PCLDTOP %ICOD)
LLCLD2= YDQTYPE%LL(TFP%PCLDBASE%ICOD) .OR. YDQTYPE%LL(TFP%HCLDBASE%ICOD)
...
CALL GPHPRE(KPROMA,NFLEVG,KST,KND,YDVAB,YDCVER,ZI_PRESH,PXYB=ZI_XYB,PRESF=ZI_PRESF,&
...
LLAPA(2)=LL_PTB.OR.LL_HTB.OR.LL_HTPW.OR.LLAERO.OR.LL_HTPW1.OR.LL_HTPW2.OR.LL_TPW.OR. &
 & LLCLD1.OR.LLCLD2
...
IF (LLAPA(1).OR.LLAPA(2)) THEN
  ...
  ENDDO
  CALL PPLETA(YDGEOMETRY%YRVAB,YDGEOMETRY%YRCVER,KPROMA,KST,KND,NFLEVG,ZXLEVG,ZSPP,ZPREPP,&
...
IF (LLAPA(1)) THEN
  ...
  IF (LLPBL) THEN
    ...
    CALL GPHPRE(KPROMA,NFLEVG,KST,KND,YDVAB,YDCVER,ZPRES_PPH,PXYB=ZXYB_PP,PRESF=ZPRES_PPF)
    ...
    IF (YDDYNA%LSACC) THEN
      CALL GPGEO_SACC(KPROMA,KST,KND,NFLEVG,&
       & ZGEO_PPH,ZGEO_PPF,ZTPP,ZUPP,ZRRED_PP,ZXYB_PP(:,:,YYTXYB%M_LNPR),ZXYB_PP(:,:,YYTXYB%M_ALPH),&
       & YDGEOMETRY%YRVERT_GEOM)
    ELSE
  ...
  IF (LLICV) THEN
    ...
    ENDIF  
    IF (LLICV1) THEN
      CALL FPCICA(YDML_PHY_MF,KST,KND,KPROMA,NFLEVG,ICAPE,.TRUE.,RENTRA,RMLDEP,ZTCLSP,ZPCLS,ZRHCLSP,ZTPP,&
       & ZPREPP,ZGFLPP(1,1,YLPPIC%IQ),ZCAPE,ZTCVS,PCIN=ZCIN,KLCL=ILCL,KFCL=IFCL,KEL=IEL)
    ENDIF 
    IF (LLICV2) THEN
      ICAPE=6
      CALL FPCICA(YDML_PHY_MF,KST,KND,KPROMA,NFLEVG,ICAPE,YDQTYPE%LL(TFP%MUMLCAPE%ICOD),RENTRA,RMLDEP, &
       & ZTCLSP,ZPCLS,ZRHCLSP,ZTPP,ZPREPP,ZGFLPP(1,1,YLPPIC%IQ),ZMUMLCAPE,ZTCVS,PMLCAPE=ZMLCAPE)
    ENDIF
...
IF (KOPLEV > 0) THEN
  ...
  SELECT CASE (CDCONF)
  CASE ('S')
    CALL PPLETA(YDFPVAB,YDGEOMETRY%YRCVER,KPROMA,KST,KND,KOPLEV,PXLEV,ZSPP,ZPREPP,ZPREH2(1,1,0),ZPREH2(1,1,1))
  CASE ('H')
    DO JL=1,KOPLEV
      DO JI=KST,KND
        ZGEOP(JI,JL)=RG*PXLEV(JL)+PCFIS(JI)
      ENDDO
    ENDDO
  ...
  IF (LL_VD) THEN
    DO JL=1,KOPLEV
      DO JI=KST,KND
        ZMGDWPP(JI,JL)=RG*(ZWHPP2(JI,JL,1)-ZWHPP2(JI,JL,0))
      ENDDO
    ENDDO
  ENDIF
...
!   Lowest output model level:
    ...
    DO JI=KST,KND
      ...
      ZGFLPP(JI,ILOC,YLPPIC%IQ)=(PGAUX(JI,KRHCLS)*ZEW(JI)) /(&
       & ZSPP(JI)*(RETV+1.0_JPRB)-RETV*ZEW(JI)*PGAUX(JI,KRHCLS))
    ENDDO
  ENDIF
