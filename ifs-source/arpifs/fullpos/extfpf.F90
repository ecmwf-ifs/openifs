...
USE YOMFPC   , ONLY : NFP_SYNC_LEVEL
...
INTEGER(KIND=JPIM) :: ISYNC_LEVEL ! 1 if non-blocking recv ; 0 if blocking recv
...
IF (NFP_SYNC_LEVEL == 1) THEN
  ISYNC_LEVEL=1
ELSEIF (NFP_SYNC_LEVEL == 0) THEN
  IF (KNFG > NPROC) THEN
    ISYNC_LEVEL=1 ! well-balanced distribution => non-blocking recv is safe
  ELSE
    ISYNC_LEVEL=0 ! poorly-balanced distribution => use blocking recv to avoid a data flood
  ENDIF
ELSE
  ISYNC_LEVEL=0
ENDIF
