...
ASSOCIATE(LFPRH100=>YDNAMFPSCI%LFPRH100)
...
!   Cloud water AROME > 0.
    ZMIN(KGPST:KGPEND)=0.0_JPRB
    ICOR=4
  ELSEIF(IJ==YDTFP%LRAD%ICOD) THEN
!   Cloud water ARPEGE > 0.
    ZMIN(KGPST:KGPEND)=0.0_JPRB
    ICOR=4
...
!   Ice > 0.
    ZMIN(KGPST:KGPEND)=0.0_JPRB
    ICOR=4
  ELSEIF(IJ==YDTFP%IRAD%ICOD) THEN
!   Ice ARPEGE > 0.
    ZMIN(KGPST:KGPEND)=0.0_JPRB
    ICOR=4
...
!     EDR > 1E-06
      ZMIN(KGPST:KGPEND)=0.000001_JPRB
      ICOR=4
  ELSEIF(IJ==YDTFP%ICING%ICOD.OR.IJ==YDTFP%ICINGX%ICOD) THEN
      ZMIN(KGPST:KGPEND)=0.0_JPRB
      ZMAX(KGPST:KGPEND)=100.0_JPRB
      ICOR=6
  ELSEIF(IJ==YDTFP%SP%ICOD) THEN
!   After interpolation of log(Ps) for better consistency with PHIs, recover Ps :
