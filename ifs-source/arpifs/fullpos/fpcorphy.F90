...
ASSOCIATE(NFPMASK=>YDNAMFPSCI%NFPMASK, LCRITSNOWTEMP=>YDNAMFPSCI%LCRITSNOWTEMP, TSRESERV1=>YDNAMFPSCI%TSRESERV1, &
 ...
 & NTPDES=>YRCLI%NTPDES, NTPMER=>YRCLI%NTPMER, SALBN=>YRCLI%SALBN, SALBX=>YRCLI%SALBX, &
 & SARGN=>YRCLI%SARGN, SARGX=>YRCLI%SARGX, SDEPN=>YRCLI%SDEPN, SDEPX=>YRCLI%SDEPX, &
 & SEMIN=>YRCLI%SEMIN, SEMIX=>YRCLI%SEMIX, SMASK=>YRCLI%SMASK, SRSMN=>YRCLI%SRSMN, &
 & SRSMX=>YRCLI%SRSMX, SSABN=>YRCLI%SSABN, SSABX=>YRCLI%SSABX, STHER=>YRCLI%STHER, &
 & SZZ0M=>YRCLI%SZZ0M, SZZ0N=>YRCLI%SZZ0N, &
...
IF (NFPCLI > 0) THEN
  ...
  ICTS =IFPSEARCH(YDRQCLI,GFP%CST%ICOD)
  ICDT =IFPSEARCH(YDRQCLI,GFP%CDST%ICOD)
  ICSD =IFPSEARCH(YDRQCLI,GFP%CSD%ICOD)
ELSE
  ICARG=0
  ICD2 =0
  ICDSW=0
...
CALL SURF_INQ(YSURF,PRWSAT=ZRWSAT,PRWLMAX=ZRWLMAX,PRALFMINSN=ZRALFMINSN,&
 & PRALFMAXSN=ZRALFMAXSN,PRHOMINSND=ZRHOMINSN,PRHOMAXSN_NEW=ZRHOMAXSN,&
...
!     4.13 Snow cover
  ...
  ELSEIF ((ICOD == GFP%SDSL%ICOD .AND. .NOT. LECMWF) .OR. &
          (ICOD == GFP%SD%ICOD .AND. LECMWF)) THEN
    IF (LLSM) THEN
      IF (LCRITSNOWTEMP) THEN
        IF (LLTS) THEN
          ICOR=2
...
!     4.17 Climatology Roughness lenth * g
  ...
  ELSEIF (ICOD == GFP%SR%ICOD .OR. ICOD == GFP%BSR%ICOD) THEN
    IF (LMPHYS) THEN
      IF (LLSM) THEN
        ICOR=1
        ZMIN(KST:KEND)=SZZ0N*RG
        ZSEA(KST:KEND)=SZZ0M*RG
      ELSE
        CALL ABOR1('FPCORPHY : CANNOT FIND LAND-SEA MASK')
      ENDIF
    ELSE
