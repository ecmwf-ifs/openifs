...
SUBROUTINE PHYMFPOS(YDCST,YDQTYPE,YDNAMFPSCI,YDAFN,LDHPOS,YDGEOMETRY,YDDYNA,&
 & YDSURF,YDRIP,YDML_PHY_MF,KPROMA,KST,KND,KFLDAUX,KFLDSPP,KGFL, &
...
!   03-2020 O. Jaron: EL, FLC, LCL in geopotential height, add MLCAPE and MUMLCAPE
...
LOGICAL :: LLICV1 ! to activate the computation of CAPE & CIEN & TCVS
LOGICAL :: LLICV2 ! to activate the computation of MLCAPE and MUMLCAPE
LOGICAL :: LLICV3 ! to activate the computation of EL, LFL, LCL
LOGICAL :: LLICV
...
LOGICAL :: LLCLD,LLCLD1,LLCLD2 ! to activate the computation of base and top of clouds 
LOGICAL :: LLZ0H
...
REAL(KIND=JPRB) :: ZUCLS(KPROMA),ZVCLS(KPROMA),ZFCLS(KPROMA)
REAL(KIND=JPRB), TARGET :: ZTCLS(KPROMA), ZRHCLS(KPROMA)
REAL(KIND=JPRB) :: ZNUCLS(KPROMA),ZNVCLS(KPROMA)
REAL(KIND=JPRB) :: ZUGST(KPROMA),ZVGST(KPROMA),ZFGST(KPROMA),ZUSMC(KPROMA)
REAL(KIND=JPRB) :: ZQCLS(KPROMA),ZPCLS(KPROMA),ZVSMC(KPROMA)
REAL(KIND=JPRB), POINTER :: ZRHCLSP(:),ZTCLSP(:)
...
ASSOCIATE(NFPCAPE=>YDNAMFPSCI%NFPCAPE, LFPCAPEX=>YDNAMFPSCI%LFPCAPEX, RENTRA=>YDNAMFPSCI%RENTRA,   RMLDEP=>YDNAMFPSCI%RMLDEP,   &
 ...
 & LUBIQUITAIRE=>YDNAMFPSCI%LUBIQUITAIRE,LISP_HYBRID=>YDNAMFPSCI%LISP_HYBRID, &
 & RATM=>YDCST%RATM, RETV=>YDCST%RETV, RKAPPA=>YDCST%RKAPPA, RTT=>YDCST%RTT)
!     ------------------------------------------------------------------
...
LLCLD1=(YDQTYPE%LL(TFP%PCLDCEIL%ICOD) .OR. &
  & YDQTYPE%LL(TFP%HCLDCEIL%ICOD) .OR. &
  & YDQTYPE%LL(TFP%PCLDTOP %ICOD))
LLCLD2=(YDQTYPE%LL(TFP%PCLDBASE%ICOD) .OR. &
  & YDQTYPE%LL(TFP%HCLDBASE%ICOD))
LLCLD=(.NOT.LDHPOS) .AND. (LLCLD1.OR.LLCLD2)
LLICV1 = (.NOT.LDHPOS) .AND. (     &
...
LLICV2 = (.NOT.LDHPOS) .AND. (     &
 & YDQTYPE%LL(TFP%MUMLCAPE%ICOD) .OR. &
 & YDQTYPE%LL(TFP%MLCAPE%ICOD) )
LLICV3 = (.NOT.LDHPOS) .AND. (     & 
 & YDQTYPE%LL(TFP%LCL%ICOD)  .OR. &
 & YDQTYPE%LL(TFP%FCL%ICOD)  .OR. &
 & YDQTYPE%LL(TFP%EL%ICOD ) )
LLICV=LLICV1.OR.LLICV2.OR.LLICV3
...
LLPBL  =LLSMC.OR.LLASMC.OR.LL_MOCO.OR.&
...
IF (LLSMC.OR.LLASMC.OR.LLVSMC.OR.LLICV.OR.LLMTS.OR.LLPBL.OR.LLSRH.OR.LLCLD) THEN
  ...
  IF (LLPBL .OR. LLMTS) THEN
    ...
    ENDDO
  ENDIF
  IF (LLPBL .OR. LLICV .OR. LLMTS .OR. LLCLD) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KND
        ZTT0(JROF,JLEV)=PTT0(JROF,JLEV)
        IF (YDDYNA%LNHDYN) ZPDT0(JROF,JLEV)=PSPDT0(JROF,JLEV)
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LLPBL.OR.LLICV.OR.LLMTS.OR.LLSRH.OR.LLCLD) THEN
    ...
    CALL GPHPRE(KPROMA,NFLEVG,KST,KND,YDVAB,YDGEOMETRY%YRCVER,ZPRESH,PXYB=ZXYB,PRESF=ZPRESF,LDELP=LL_MOCO,&
     & LRTGR=.FALSE.,LRPP=.FALSE.)
    ! Compute NH pressure departure, and ZRRED=(pre/prehyd)*R
    IF (LLPBL.OR.LLCLD.OR.LLICV3.OR.LLSRH) THEN
      IF (YDDYNA%LNHDYN) THEN
        ! ZPDT0 contains pressure departure prognostic variable.
        CALL GNHPRE(YDGEOMETRY,YDDYNA%NPDVAR,KPROMA,NFLEVG,KST,KND,ZPDT0,ZPRESF,&
    ...
    IF (LLPBL.OR.LLSRH.OR.LLCLD.OR.LLICV3) THEN
  ...
  IF (LLICV) THEN
    ...
    ELSE
      ...
      ZRHCLSP=>ZRHCLS
    ENDIF
    IF (LLICV1.OR.LLICV3) THEN
      CALL FPCICA(YDML_PHY_MF,KST,KND,KPROMA,NFLEVG,ICAPE,.TRUE.,RENTRA,RMLDEP,ZTCLSP,ZPCLS,ZRHCLSP,ZTT0,ZPRESF,&
       & ZQT0,ZCAPE,ZTCVS,PCIN=ZCIN,KLCL=ILCL,KFCL=IFCL,KEL=IEL)
      CALL STORE_DATA(TFP%CAPE%ICOD,1,ZCAPE)
      CALL STORE_DATA(TFP%CIEN%ICOD,1,ZCIN)
      CALL STORE_DATA(TFP%TCVS%ICOD,1,ZTCVS)
      IF(LLICV3) THEN
        DO JROF=KST,KND
          IF (ILCL(JROF)<0_JPIM.OR.ILCL(JROF)>NFLEVG) THEN
            ZLCL(JROF)=0._JPRB
          ELSE  
            ZLCL(JROF)=ZGEOPF(JROF,ILCL(JROF))
          ENDIF  
          IF (IFCL(JROF)<0_JPIM.OR.IFCL(JROF)>NFLEVG) THEN
            ZFCL(JROF)=0._JPRB
          ELSE  
            ZFCL(JROF)=ZGEOPF(JROF,IFCL(JROF))
          ENDIF  
          IF (IEL(JROF)<0_JPIM.OR.IEL(JROF)>NFLEVG) THEN
            ZEL(JROF)=0._JPRB
          ELSE  
            ZEL(JROF)=ZGEOPF(JROF,IEL(JROF))
          ENDIF  
        ENDDO
      ENDIF  
    ...
    IF (LLICV2) THEN
      ...
      CALL FPCICA(YDML_PHY_MF,KST,KND,KPROMA,NFLEVG,ICAPE,YDQTYPE%LL(TFP%MUMLCAPE%ICOD),RENTRA,RMLDEP,&
       & ZTCLSP,ZPCLS,ZRHCLSP,ZTT0,ZPRESF,ZQT0,ZMUMLCAPE,ZTCVS,PMLCAPE=ZMLCAPE)
  ...
  IF (LL_MOCO) THEN
    ...
    DO JLEV=ITDIA,NFLEVG
      DO JROF=KST,KND
        ZTHETAV(JROF,JLEV)=ZTT0(JROF,JLEV)*(1.0_JPRB+RETV*ZQT0(JROF,JLEV)) &
         & *(RATM/ZPRESF(JROF,JLEV))**RKAPPA  
      ENDDO
    ENDDO
    DO JROF=KST,KND
      ZTHETAVS(JROF)=PSP_RR(JROF,YSP_RR%YT%MP0)*(1.0_JPRB+RETV*ZQS(JROF)) &
       & *(RATM/ZPRESH(JROF,NFLEVG))**RKAPPA  
    ENDDO
  ...
  IF (LLASMC) THEN
    DO JROF=KST,KND
      ...
      IF(LNEIGE) THEN
        ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-PXTCLS(JROF)))
      ELSE
        ZDELTA=0.0_JPRB
      ENDIF
      ...
      ZXQCLS(JROF)=-PXRHCLS(JROF)*ZES /(&
       & RETV*PXRHCLS(JROF)*ZES-ZPCLS(JROF)*(RETV+1.0_JPRB))  
  ...
  IF (LLMTS) THEN
    ...
    CALL MTS_PHYS(YDDIMV,YDRIP,KST,KND,KPROMA,NFLEVG,LUBIQUITAIRE,LISP_HYBRID,PGEMU,PGELAM,POROG,ZPRESH,ZPRESF,&
