...
!      R. El Khatib 23-Sep-2021 Bugfix for LSPRT=T + LDPACK=T
!     ------------------------------------------------------------------
...
IF (LDPACK) THEN
  ...
  ! Inverse transforms
  IF (LELAM) THEN
    CALL ETRANSINV_MDL(YDGEOMETRY,YDGMV,YDMDDH,YDML_GCONF,YDDYNA,YLSPEC%VOR,YLSPEC%DIV,&
     & YLSPEC%UB,YLSPEC%VB,&
...
ELSE
  ...
  IF (LELAM) THEN
    CALL ETRANSINV_MDL(YDGEOMETRY,YDGMV,YDMDDH,YDML_GCONF,YDDYNA,YDSPEC%VOR,YDSPEC%DIV,&
     & YDSPEC%UB,YDSPEC%VB,&
