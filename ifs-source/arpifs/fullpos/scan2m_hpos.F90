...
!   R. El Khatib 11-Mar-2021 (indirect) Support for LDIRCLSMOD
!-----------------------------------------------------------------------------
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV, YDGEM=>YDGEOMETRY%YRGEM, &
 & YDML_PHY_G=>YDMODEL%YRML_PHY_G, YDML_PHY_MF=>YDMODEL%YRML_PHY_MF, YDPHY1=>YDMODEL%YRML_PHY_MF%YRPHY1)
ASSOCIATE(NFIELDG=>YDRQPHY%NFIELDG,GFP_PHYDS=>YDAFN%GFP_PHYDS,NPROMA=>YDDIM%NPROMA, NGPTOT=>YDGEM%NGPTOT, NGPBLKS=>YDDIM%NGPBLKS, &
 & NFPCLI=>YDNAMFPSCI%NFPCLI, NFLEVG=>YDDIMV%NFLEVG, LMPHYS=>YDML_PHY_MF%YRPHY%LMPHYS, &
 ...
 & SD_OC=>YDSURF%SD_OC, SP_CL=>YDSURF%SP_CL, &
...
IF (LMPHYS) THEN
  DO JFLD=1,NFIELDG
    CALL HPOS_XFU(YDAFN,LDFPOSHOR,YDXFU,YDML_PHY_G,YDML_PHY_MF,NPROMA,NFLEVG,YDSURF,KEND,YDRQPHY%ICOD(JFLD),NFPCLI, &
     & YDGEOMETRY%YRGSGEOM(KBL)%GM,PWSXI,SP_RR(:,:,KBL),SP_CL(:,:,KBL),SD_VX(:,:,KBL),SD_VF(:,:,KBL),SD_VV(:,:,KBL), &
