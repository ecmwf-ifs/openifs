...
CHARACTER(LEN=64), ALLOCATABLE :: CLMODEL(:)
LOGICAL, ALLOCATABLE :: LLEXTERN(:)
...
CALL SUFPC(LDPRINT=.TRUE.,YDNAMFPOBJ=YLNAMFPOBJ,YDNAMFPSCI=YDFPOS%YNAMFPSCI,YDNAMFPINT=YDFPOS%YNAMFPINT, &
 & YDNAMFPL=YDFPOS%YNAMFPL,KFPCONF=KFPOS)
...
ALLOCATE(CLMODEL(IFPDOM))
ALLOCATE(LLEXTERN(IFPDOM))
WRITE(UNIT=NULOUT,FMT='('' CFPDIR = '',A)') TRIM(CFPDIR)
CALL SUFPIOS(NFPGRIB,YDFPOS%YNAMFPSCI%NFPSURFEX,YDMODEL%YRML_GCONF%YRRIP%TSTEP,YDMODEL%YRML_GCONF%YRRIP%NSTOP, &
 & CFPDIR,YDFPOS%YFPGEOMETRY%YFPUSERGEO(:)%CFPDOM,CLFPFN,CLFPCLIFNAME,CLFPSFXFNAME,CLMODEL,LLEXTERN,YLNAMFPIOS)
WRITE(NULOUT,'('' == Full-Pos constructor : setup I/O handlings == '')')
CALL SUFPIOH(YDFPOS%YNAMFPSCI%NFPSURFEX,YLNAMFPIOS,CLFPFN,CLFPCLIFNAME,CLFPSFXFNAME, &
 & CLMODEL,LLEXTERN,YDFPOS%YFPGEOMETRY,YDFPOS%YFPVAB,YDFPOS%YFPIOH)
...
CALL SUAFN(YDFPOS%YNAMFPSCI,YDFPOS%YAFN,LLNEWCL,YDMODEL%YRML_GCONF%YGFL,YDMODEL%YRML_DYN%YRDYNA%LNHDYN, &
  &        IFPCONF,YDMODEL%YRML_PHY_G%YRDPHY%LDIRCLSMOD,YDMODEL%YRML_PHY_G%YRDPHY%LDIRSICMOD,&
  &        YDMODEL%YRML_CHEM%YRCOMPO,YDMODEL%YRML_PHY_RAD%YREAERATM,CLFPSFXFNAME)
...
CALL SUFPFILTERS(YDFPOS%YNAMFPSCI,YLNAMFPF,YDFPOS%YNAMFPL,YDHOSTGEOMETRY,YDMODEL%YRML_PHY_G%YRDPHY%NCSNEC,&
 & IFPDOM,IFPCONF,YDFPOS%YAFN%TFP_DYNDS(:)%ISF, &
