...
!     MODIFICATIONS.
!     --------------
!     R. El Khatib 09-Feb-2021 Multi-instanciable NPDIRL
!     ------------------------------------------------------------------

USE PARKIND1  ,ONLY : JPIM     ,JPRB
USE YOMHOOK   ,ONLY : LHOOK,   DR_HOOK, JPHOOK

USE YOMLUN   , ONLY : NULOUT  ,NPPPSH, FOPEN
...
ASSOCIATE(LFPCNT=>YDFPCNT%LFPCNT, LFPNAMELIST=>YDFPCNT%LFPNAMELIST, NFPCONF=>YDFPCNT%NFPCONF, CFPNCF=>YDFPCNT%CFPNCF, NPDIRL=>YDFPCNT%NPDIRL)
...
IF(.NOT.LLEXIST .AND. CDPATH /= ' ') THEN
  ...
  ENDIF
  IF (MYPROC==1) THEN
    CALL EXECUTE_COMMAND_LINE('/bin/ls -d '//TRIM(CLDIR)//'/* > '//TRIM(CLIST),EXITSTAT=IERR)
    !CALL EXECUTE_COMMAND_LINE('\find '//TRIM(CLDIR)//' -type f -depth 2 > '//TRIM(CLIST),EXITSTAT=IERR)
    IF (IERR /= 0) THEN
      WRITE(CIERR,'(A4)') IERR
...
IF (LFPCNT) THEN
  ...
  CALL FOPEN(NPDIRL,CLIST,CDTEXT='DIRECTORY LISTING FILE')
  IF (LECMWF) THEN
    IF (CDMONIPATH_IN /= ' ' .AND. CDMONIPATH_IN /= '.') THEN
