...
SUBROUTINE SUFPFILTERS(YDNAMFPSCI,YDNAMFPF,YDNAMFPL,YDGEOMETRY,KLEVSNOW,KFPDOM,KFPCONF,KSF,CDNAME,YDFPFILTERS)
...
!    R. El Khatib : 20-Aug-2020 Fix uninitialized variable, fix if no stretching
!    R. El Khatib : 08-Feb-2021 Proper deallocation of ylfpfields
!     ------------------------------------------------------------------
...
!*       1.    COMPUTE FILTERS, FIND OUT IF FILTERS ARE ACTIVE OR NOT, THEN COMPUTE
!              --------------------------------------------------------------------
...
ELSE
  ...
  CALL SUFPFIELDS(YDNAMFPL,NFLEVG,KLEVSNOW,YLFPFIELDS,LDPRINT=.FALSE.)
  LLFILTER_ACTIVE = &
  ...
  IF (.NOT.LLFILTER_ACTIVE) THEN
    ...
    ENDIF
  ENDIF
  IF (RSTRET > 1._JPRB+100_JPRB*EPSILON(1._JPRB)) THEN
  ...
  ! in principle, after each call to sufpfields, where the content of the structure is allocated :
