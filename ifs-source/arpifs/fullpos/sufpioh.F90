...
YDFPIOH%YNAMFPIOS%NFPWRITE=MAX(-1,MIN(2,YDFPIOH%YNAMFPIOS%NFPWRITE))
...
WRITE(UNIT=NULOUT,FMT='('' NFRFPDI = '',I4,6('' NFPDITS('',I3,'') = '',I4))') &
 & YDNAMFPIOS%NFRFPDI,(J,YDNAMFPIOS%NFPDITS(J),J=0,ABS(YDNAMFPIOS%NFPDITS(0)))
IF (YDNAMFPIOS%NFPDITSMIN(0) /= 0) THEN
  WRITE(UNIT=NULOUT,FMT='(5('' NFPDITSMIN('',I3,'') = '',I4))') (J,YDNAMFPIOS%NFPDITSMIN(J),J=1,ABS(YDNAMFPIOS%NFPDITSMIN(0)))
ENDIF
IF (KFPSURFEX == 1) THEN
...
DO J=1,IFPDOM
  IF (LARPEGEF) THEN
    ...
    YDFPIOH%YFPOPH(J)%CFAMODEL=CDMODEL(J)
    YDFPIOH%YFPOPH(J)%LFAEXTERN=LDEXTERN(J)
    WRITE(NULOUT,'('' LFAEXTERN('',I2.2,'') = '',L2,'' CFAMODEL('',I2.2,'') = '',A64)') &
     & J, YDFPIOH%YFPOPH(J)%LFAEXTERN, J, YDFPIOH%YFPOPH(J)%CFAMODEL
  ELSE
    ! INITIALIZE PARAMETERS FOR GRIB ENCODING 
    IF (J > 1) THEN
