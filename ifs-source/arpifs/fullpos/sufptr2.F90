...
!      R. El Khatib 08-03-2021 Bugfix for interpoperability Tessel->ISBA
!     ------------------------------------------------------------------
...
IF (PRESENT(YDRQDYN)) THEN
  DO JFLD=1,YDRQDYN%Y2D%NPPFIELDG
    IF (YDRQDYN%Y2D%ICOD(JFLD)==TFP%CAPE%ICOD.OR. &
     ...
     &  YDRQDYN%Y2D%ICOD(JFLD)==TFP%TCVS%ICOD.OR. &
     &  YDRQDYN%Y2D%ICOD(JFLD)==TFP%LCL%ICOD.OR. &
     &  YDRQDYN%Y2D%ICOD(JFLD)==TFP%FCL%ICOD.OR. &
     &  YDRQDYN%Y2D%ICOD(JFLD)==TFP%EL%ICOD.OR. &
     &  YDRQDYN%Y2D%ICOD(JFLD)==TFP%MLCAPE%ICOD.OR.&
     &  YDRQDYN%Y2D%ICOD(JFLD)==TFP%MUMLCAPE%ICOD)  THEN
      LLCAPE=.TRUE.
      EXIT
    ENDIF
  ENDDO
...
! Output surface temperature is needed for surface-dependent vertical interpolations, Tcls, snow depth, Tp and Surfex :
  ...
  IF (NFPCLI > 0 .OR. LDFPOSHOR) THEN
    IF (IFPSEARCH(YDRQPHY,GFP%SDSL%ICOD) /= 0 .OR. IFPSEARCH(YDRQPHY,GFP%DST%ICOD) /= 0 &
...
! Deep soil temperature needed for interoperability ISBA/TESSEL->Surfex or TESSEL->ISBA
  IF (LLFPSX1 .OR. &
   &  (NFPSWI==0 .AND. (IFPSEARCH(YDRQPHY,GFP%DSW%ICOD) /= 0 .OR. IFPSEARCH(YDRQPHY,GFP%FDSW%ICOD) /= 0)) ) THEN
    IF (LLFPSX1 .OR. IFPSEARCH(YDRQPHY,GFP%DST%ICOD) == 0) CALL SET_NEXT(GFP%DST%ICOD)
  ENDIF
