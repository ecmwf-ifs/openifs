...
@PROCESS NOCHECK
#endif
SUBROUTINE VPOS(YDCST,YDQTYPE,YDNAMFPSCI,YDAFN,LDHPOS,YDFPVAB,YDGEOMETRY,YDSURF,YDXFU,KCUFNR,PMCUF,YDML_GCONF,YDDYN,YDDYNA,YDML_PHY_MF, &
...
!      R. El Khatib 31-Jan-2022 fix a memory leak if LNHQE=.TRUE. Do not use allocated pointers.
!     ------------------------------------------------------------------
...
REAL(KIND=JPRB), ALLOCATABLE, TARGET :: ZGPGRHL(:,:), ZGPT(:,:), ZGPTL(:,:), ZGPTM(:,:)
...
ASSOCIATE(YDTFP=>YDAFN%TFP,YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV, &
 ...
 & YDDIMF=>YDML_GCONF%YRDIMF,YDCVER=>YDGEOMETRY%YRCVER)
...
IF (YDDYNA%LNHQE) THEN
  ...
  ALLOCATE(ZGPT(NPROMA,NFLEVG))
  ALLOCATE(ZGPTL(NPROMA,NFLEVG))
  ALLOCATE(ZGPTM(NPROMA,NFLEVG))
  DO JLEV=1,NFLEVG
    ZGPT (:,JLEV) = PGMV(:,JLEV,YDIN_GMV%MT)
    ZGPTL(:,JLEV) = PGMV(:,JLEV,YDIN_GMV%MTL)
    ZGPTM(:,JLEV) = PGMV(:,JLEV,YDIN_GMV%MTM)
  ENDDO
  CALL GNHQE_CONV_TEMPE(YDGEOMETRY,YDDYNA%NPDVAR,.TRUE.,1,KST,KEND,PGMV(1,1,YDIN_GMV%MSPD),PGMVS(1,YDIN_GMV%MSP),ZGPT,&
   & KDDER=2,PQCHAL=PGMV(1,1,YDIN_GMV%MSPDL),PQCHAM=PGMV(1,1,YDIN_GMV%MSPDM),PTL=ZGPTL,PTM=ZGPTM)
  ZT  => ZGPT (:,1:NFLEVG)
  ZTL => ZGPTL(:,1:NFLEVG)
  ZTM => ZGPTM(:,1:NFLEVG)
ELSE
...
ELSE
  ...
  ENDDO
  CALL GPHPRE(NPROMA,NFLEVG,KST,KEND,YDVAB,YDCVER,ZIPH,PRESF=ZIPF)
...
CALL POS(YDCST,YDQTYPE,YDNAMFPSCI,YDTFP,LDHPOS,YDFPVAB,YDGEOMETRY,YDSURF,YDML_GCONF,YDDYN,YDDYNA,YDPHY,NPROMA,KST,KEND,&
...
IF(YDGFL%LLG) THEN
  IF (YDGFL%LLH) THEN
    ALLOCATE(ZGPGRHL(NPROMA,NFLEVG))
    DO JLEV=1,NFLEVG
      DO JROF=KST,KEND
        ZGPGRHL(JROF,JLEV)=PGFL(JROF,JLEV,YDIN_GFL%IG)+PGFL(JROF,JLEV,YDIN_GFL%IH)
      ENDDO
    ENDDO
    ZGRHL => ZGPGRHL(:,1:NFLEVG)
  ELSE
...
CALL PHYMFPOS(YDCST,YDQTYPE,YDNAMFPSCI,YDAFN,LDHPOS,YDGEOMETRY,YDDYNA,YDSURF,YDML_GCONF%YRRIP,YDML_PHY_MF,NPROMA,KST,KEND,KAUX, &
