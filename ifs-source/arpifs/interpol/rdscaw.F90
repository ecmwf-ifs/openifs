...
INTEGER(KIND=JPIM) :: ILA, ILO, ILO1, ILO2, ILO3, IZLAT, JROF
...
! Despite the removal of JPDUP, I keep these arrays because of their odd type JPRD. REK. 
REAL(KIND=JPRD) :: ZRIPI0_(YDSL%NDGSAH:YDSL%NDGENH)
REAL(KIND=JPRD) :: ZRIPI1_(YDSL%NDGSAH:YDSL%NDGENH)
REAL(KIND=JPRD) :: ZRIPI2_(YDSL%NDGSAH:YDSL%NDGENH)
...
ZRIPI0_(YDSL%NDGSAH:YDSL%NDGENH)=PIPI(YDSL%NDGSAH:YDSL%NDGENH,1)
ZRIPI1_(YDSL%NDGSAH:YDSL%NDGENH)=PIPI(YDSL%NDGSAH:YDSL%NDGENH,2)
ZRIPI2_(YDSL%NDGSAH:YDSL%NDGENH)=PIPI(YDSL%NDGSAH:YDSL%NDGENH,3)
...
!CDIR NODEP
  DO JROF=KSTART,KPROF
    ...
    ILA   =IZLAT+NINT(SIGN(0.5_JPRB,PLATI(IZLAT)-PLAT(JROF)+ZEPS)-1.5_JPRB)

    PDLAT(JROF)=(PLAT(JROF)-PLATI(ILA+1))&
     & /(PLATI(ILA+2)-PLATI(ILA+1)+ZEPS)  

    ZLO1  =PLON(JROF)*PLSDEPI(ILA+1)
    ILO1  =INT(ZLO1)
    PDLO(JROF,1)=ZLO1-REAL(ILO1,JPRB)
    ZLO2  =PLON(JROF)*PLSDEPI(ILA+2)
    ILO2  =INT(ZLO2)
    PDLO(JROF,2)=ZLO2-REAL(ILO2,JPRB)

    KL0(JROF,1)=KSTABUF(ILA+1)+YDSL%NSLEXT(ILO1,ILA+1)
    KL0(JROF,2)=KSTABUF(ILA+2)+YDSL%NSLEXT(ILO2,ILA+2)
...
!CDIR NODEP
  DO JROF=KSTART,KPROF
    ...
    ZDA   =PLAT(JROF)-PLATI(ILA)
    ZDB   =PLAT(JROF)-PLATI(ILA+1)
    ZDC   =PLAT(JROF)-PLATI(ILA+2)
    ZDD   =PLAT(JROF)-PLATI(ILA+3)
    
    PCLA(JROF,1)=REAL((ZDA*ZDC)*ZDD,JPRD)*ZRIPI0_(ILA+1)
    PCLA(JROF,2)=REAL((ZDA*ZDB)*ZDD,JPRD)*ZRIPI1_(ILA+1)
    PCLA(JROF,3)=REAL((ZDA*ZDB)*ZDC,JPRD)*ZRIPI2_(ILA+1)

    ZLO   =PLON(JROF)*PLSDEPI(ILA  )
    ILO   =INT(ZLO )
    ...
    KL0(JROF,0)=KSTABUF(ILA  )+YDSL%NSLEXT(ILO ,ILA  )
    KL0(JROF,1)=KSTABUF(ILA+1)+YDSL%NSLEXT(ILO1,ILA+1)
    KL0(JROF,2)=KSTABUF(ILA+2)+YDSL%NSLEXT(ILO2,ILA+2)
    KL0(JROF,3)=KSTABUF(ILA+3)+YDSL%NSLEXT(ILO3,ILA+3)
