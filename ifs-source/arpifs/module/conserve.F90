...
#include "dir_trans.h"
contains
  subroutine conservini(geo,ygfl,phy,sp,pgfl,ldcorm)
    type(geometry),intent(in) :: geo
    type(type_gfld),intent(in) :: ygfl
    type(tphy),intent(in) :: phy
    type(spectral_field),intent(in) :: sp
    real(kind=jprb),intent(in) :: pgfl(geo%yrdim%nproma,geo%yrdimv%nflevg,ygfl%ndim,&
      geo%yrdim%ngpblks)
    logical,intent(in) :: ldcorm

    integer(kind=jpim) :: jp,ifld,irw,igm,ipsf,ipdryf,ipvf,iener,liquid,ice,rain,snow
    real(kind=jprb),allocatable :: zweight(:),zphis(:)
    real(kind=jprb),allocatable :: zsendg(:,:),zh(:,:),zrr(:,:)
    real(kind=jprb),allocatable :: zpq(:,:,:)
    real(kind=jprb),allocatable,target :: zsend(:,:)
    real(kind=jprb),contiguous,pointer :: zpsf(:),zpvf(:)
    real(kind=jprb) :: ztotmass
    ...
    if (ldcorm) then
      ipsf = addindex(ifld,1)
      ipdryf = addindex(ifld,1)
    end if

    if (lcorwat) ipvf = addindex(ifld,1)

    allocate(zsend(ngptot,ifld))

    call setprocfield(mp,ngptot,rw,zsend(:,irw))

    ! optim: no way to avoid copying
    !$omp parallel do
    !dir$ ivdep
    !cdir nodep
    do jp=1,ngptot
    ...
    allocate(zh(ngptot,nflevg))

    allocate(zpq(nproma,nflevg,ngpblks))
    ...
    zpvf(:) = 0
    call blockvsum(dimg,ngptot,nflevg,vab,zpsf,zh,zpvf)
    ...
    if (ldcorm) then
      ...
      !cdir nodep
      do jp=1,ngptot
        zsend(jp,ipdryf) = zpsf(jp)-zpvf(jp)
      end do
      !$omp end parallel do
    end if

    if (lcorwat) then
      allocate(zrr(ngptot,nflevg))

      if (lcondwt) then
        call condensatesind(ygfl,liquid,ice,rain,snow)
        call condpre(dimg,dimv,gem,ygfl,pgfl,liquid,ice,rain,snow,zrr)
        call blockvsum(dimg,ngptot,nflevg,vab,zpsf,zrr,zpvf)
      end if
    ...
    if (myproc == proc0) then
      ...
      if (ldcorm) then
        ...
        ztotmass = sum(zsendg(:,ipsf)*zweight(:))
        write(nulout,"('conservini: total and dry masses',2(x,f15.7))")&
          ztotmass,vdrymass
      end if

      if (lcorwat) then
        vtotwat = sum(zsendg(:,ipvf)*zweight(:))/rg
        write(nulout,"('conservini: total atm. water mass ',f15.7)") vtotwat
      end if
  ...
  subroutine conserv(geo,rip,ygfl,phy,parar,sp,surf,ydcfu,pgfl,ldcorm)
    type(geometry),intent(in) :: geo
    ...
    integer(kind=jpim) :: ifld,jfl,jp,jf,ibl,joff,k1,k2,np,irw,igm,ipsf,ipdryf,ievasf,&
      iprpar,ipvf,ispvf,iprparl,iprparn,iphis,iflxt,ifts,ifsols,liquid,ice,rain,snow
    real(kind=jprb) :: zapcorm,zdrymass,ztotwat,zda,zdb,zesuf,zadd,zpr,zratio(1),zz
    real(kind=jphook) :: zhook_handle
    real(kind=jprb),allocatable :: zweight(:),zsendg(:,:),zh(:,:),zrr(:,:),zrrcv(:,:)
    real(kind=jprb),allocatable,target :: zsend(:,:)
    real(kind=jprb),allocatable :: zpq(:,:,:)
    real(kind=jprb),contiguous,pointer :: zpsf(:),zpvf(:),zspvf(:),zpdryf(:),&
      zevasf(:),zprpar(:),zprparl(:),zprparn(:)
    ...
    if (ldcorm) ipdryf = addindex(ifld,1)

    if (lcorwat) then
      ievasf = addindex(ifld,1)
      ipvf = addindex(ifld,1)
      iprpar = addindex(ifld,1)
      iprparl = addindex(ifld,1)
      iprparn = addindex(ifld,1)
    end if
    ...
    zpvf(:) = 0
    zspvf(:) = 0
    call blockvsum(dimg,ngptot,nflevg,vab,zpsf,zh,zpvf,zspvf)
    ...
    if (lcorwat) then
      ...
      if (lcondwt) then
        call condensatesind(ygfl,liquid,ice,rain,snow)
        call condpre(dimg,dimv,gem,ygfl,pgfl,liquid,ice,rain,snow,zrr)
        call blockvsum(dimg,ngptot,nflevg,vab,zpsf,zrr,zpvf,zspvf)
      end if
      ...
      !cdir nodep
      do jp=1,ngptot
        zprpar(jp) = zprparl(jp)+zprparn(jp)
      end do
      !$omp end parallel do
    end if
    ...
    if (myproc == proc0) then
      ...
      if (ldcorm) then
        ...
        write(nulout,"('conserv: current and former total dry masses + correction',&
          &3(x,f15.7))") zdrymass,vdrymass,zapcorm
      ...
      if (lcorwat) then
        ...
        write(nulout,"('conserv: correction ratio on water ',f15.12)") zratio
    ...
    if (lcorwat) then
      !$omp parallel do
      do ibl=1,ngpblks
        gparbuf(:,parar%mrain,ibl) = gparbuf(:,parar%mrain,ibl)*zratio(1)
        gparbuf(:,parar%msnow,ibl) = gparbuf(:,parar%msnow,ibl)*zratio(1)
      end do
      !$omp end parallel do
      ...
      if (ydcfu%nfdcfu > 0) then
        ...
        !$omp parallel do private(np,k1,k2,jfl,jp)
        do ibl=1,ngpblks
          np = blocksize(ibl,nproma,ngptot)

          k1 = 1
          do jf=1,ydcfu%ntypcfu
            k2 = k1+ydcfu%type_cfu(jf)%ilev-1
            ...
            if (k1 == ycfupt%mfplcl) then
              do jfl=k1,k2
                ...
                !cdir nodep
                do jp=1,np
                  gfubuf(jp,jfl,ibl) = gfubuf(jp,jfl,ibl)+sd_vh(jp,ysd_vh%ypcl%mp,ibl)*zz
                end do
              end do
              ...
              do jfl=k1,k2
                ...
                !cdir nodep
                do jp=1,np
                  gfubuf(jp,jfl,ibl) = gfubuf(jp,jfl,ibl)+sd_vh(jp,ysd_vh%ypsl%mp,ibl)*zz
                end do
              end do
              ...
              do jfl=k1,k2
                ...
                !cdir nodep
                do jp=1,np
                  gfubuf(jp,jfl,ibl) = gfubuf(jp,jfl,ibl)+sd_vh(jp,ysd_vh%ypcn%mp,ibl)*zz
                end do
              end do
              ...
              do jfl=k1,k2
                ...
                !cdir nodep
                do jp=1,np
                  gfubuf(jp,jfl,ibl) = gfubuf(jp,jfl,ibl)+sd_vh(jp,ysd_vh%ypsn%mp,ibl)*zz
                end do
              end do
            end if
  ...
  subroutine setprocfield(mp,ngptot,xgeo,z)
    ...
    real(kind=jprb),intent(out) :: z(ngptot)
  ...
  subroutine sptrans_spsc2(nproma,ngpblks,nspec2,ngptot,nbsetsp,zsps,zpsf)
    ...
    real(kind=jprb),intent(out) :: zpsf(ngptot)
  ...
  subroutine sptrans_spvordiv(dimg,dimv,sp,nbsetlev,zu,zv)
    ...
    real(kind=jprb),intent(out) :: zu(dimg%nproma,dimv%nflevg,dimg%ngpblks)
    real(kind=jprb),intent(out) :: zv(dimg%nproma,dimv%nflevg,dimg%ngpblks)

    integer(kind=jpim) :: ibl
    real(kind=jprb),allocatable :: zuv(:,:,:)
    ...
    !$omp parallel do
    do ibl=1,dimg%ngpblks
      zu(:,:,ibl) = zuv(:,1:dimv%nflevg,ibl)
      zv(:,:,ibl) = zuv(:,dimv%nflevg+1:2*dimv%nflevg,ibl)
    end do
    !$omp end parallel do
  ...
  subroutine gptrans_spsc2(nproma,ngpblks,nspec2,ngptot,nbsetsp,zpsf,zsps)
    integer(kind=jpim),intent(in) :: nproma,ngpblks,nspec2,ngptot,nbsetsp
    real(kind=jprb),intent(in) :: zpsf(ngptot)
  ...
  subroutine energy(lcondwt,lgpcmt,dimg,dimv,gem,vab,sp,nbsetlev,zh,zrr,zrrcv,zpsf,&
    ...
    call blockvsum(dimg,ngptot,nflevg,vab,zpsf,ze,zener,zsener)
  ...
  subroutine gpweights(dimg,gem,zrw,zgm,zweight)
    type(tdim),intent(in) :: dimg
    type(tgem),intent(in) :: gem
    real(kind=jprb),intent(in) :: zrw(gem%ngptotg),zgm(gem%ngptotg)
    real(kind=jprb),intent(out) :: zweight(gem%ngptotg)
  ...
  subroutine blockvsum(dimg,ngptot,nflevg,vab,zpsf,zmult,z,zs)
    type(tdim),intent(in) :: dimg
    type(tvab),intent(in) :: vab
    integer(kind=jpim),intent(in) :: ngptot,nflevg
    real(kind=jprb),intent(in) :: zpsf(ngptot),zmult(ngptot,nflevg)
    real(kind=jprb),intent(out) :: z(ngptot)
    real(kind=jprb),optional,intent(out) :: zs(ngptot)

    integer(kind=jpim) :: jl,jp,ibl,k1,k2
    real(kind=jprb) :: zda,zdb

    associate(nproma=>dimg%nproma,ngpblks=>dimg%ngpblks,vah=>vab%vah,vbh=>vab%vbh)

    if (present(zs)) then
      !$omp parallel do private(k1,k2,jp,jl,zda,zdb)
      do ibl=1,ngpblks
        k1 = 1+(ibl-1)*nproma
        k2 = k1+blocksize(ibl,nproma,ngptot)-1
        ...
        do jl=1,nflevg
          ...
          do jp=k1,k2
            z(jp) = z(jp)+zmult(jp,jl)*(zda+zdb*zpsf(jp))
            zs(jp) = zs(jp)+zmult(jp,jl)*zdb
          end do
        end do
      end do
      !$omp end parallel do
  ...
  subroutine condpre(dimg,dimv,gem,ygfl,pgfl,liquid,ice,rain,snow,z)
    type(tdim),intent(in) :: dimg
    type(tdimv),intent(in) :: dimv
    type(tgem),intent(in) :: gem
    type(type_gfld),intent(in) :: ygfl
    integer(kind=jpim),intent(in) :: liquid,ice,rain,snow
    real(kind=jprb),intent(in) :: pgfl(dimg%nproma,dimv%nflevg,ygfl%ndim,dimg%ngpblks)
    real(kind=jprb),intent(out) :: z(gem%ngptot,dimv%nflevg)

    integer(kind=jpim) :: ibl,joff,np,jl

    associate(nproma=>dimg%nproma,nflevg=>dimv%nflevg,ngpblks=>dimg%ngpblks,&
      ngptot=>gem%ngptot)
    ...
    do ibl=1,ngpblks
      ...
      do jl=1,nflevg
        z(joff+1:joff+np,jl) = pgfl(1:np,jl,liquid,ibl)+pgfl(1:np,jl,ice,ibl)+&
          pgfl(1:np,jl,snow,ibl)+pgfl(1:np,jl,rain,ibl)
      end do
