...
TYPE PERTURB_TYPE
  ...
  REAL(KIND=JPRB), DIMENSION(:,:,:), POINTER, CONTIGUOUS :: PGP2DSPP ! SPP random fields
...
TYPE PERTURB_LOCAL_TYPE
  ...
  CLASS(FIELD_2RB), POINTER :: F_ZWMEAN, F_ZCUCONVCA
  CLASS(FIELD_3RB), POINTER :: F_ZDISSCU, F_ZDISSGW
...
CONTAINS
  ...
  SUBROUTINE PERTURB_TYPE_UPDATE_VIEW(THIS, YDMODEL, YDSTOPH, IBL, IOFF, ICEND, NPROMA, NFLEVG, ZSPPTGFIX)
    ...
    INTEGER(KIND=JPIM) :: J2D, JARP, JRF
    ...
    IF (YDMODEL%YRML_GCONF%YRSPPT_CONFIG%LSPSDT) THEN
      ALLOCATE(THIS%PGP2DSDT(NPROMA,YDMODEL%YRML_SPPT%YGPSDT(1)%NG2D,YDMODEL%YRML_SPPT%N2D))
      DO J2D=1,YDMODEL%YRML_SPPT%N2D
        THIS%PGP2DSDT(:,:,J2D) = YDMODEL%YRML_SPPT%YGPSDT(J2D)%GP2D(:,:,IBL)
      ENDDO
    ELSE
      THIS%PGP2DSDT => NULL()
    ENDIF
    IF (YDMODEL%YRML_GCONF%YRSPP_CONFIG%LSPP) THEN
      ALLOCATE(THIS%PGP2DSPP(NPROMA,YDMODEL%YRML_SPP%GP_ARP(1)%NG2D,YDMODEL%YRML_GCONF%YRSPP_CONFIG%SM%NRFTOTAL))
      DO JRF=1,YDMODEL%YRML_GCONF%YRSPP_CONFIG%SM%NRFTOTAL
        THIS%PGP2DSPP(:,:,JRF) = YDMODEL%YRML_SPP%GP_ARP(JRF)%GP2D(:,:,IBL)
      ENDDO
    ELSE
      THIS%PGP2DSPP=> NULL()
    ENDIF
    ...
    ENDIF
    IF (YDMODEL%YRML_GCONF%YRSPPT_CONFIG%LSPSDT.AND.YDMODEL%YRML_GCONF%YRSPPT_CONFIG%LSPPTGFIX) THEN
  ...
  SUBROUTINE PERTURB_TYPE_FINAL(THIS, YDMODEL, YDSTOPH)
    CLASS(PERTURB_TYPE) :: THIS
    TYPE(MODEL),          INTENT(IN)    :: YDMODEL
    TYPE(TSTOPH), TARGET, INTENT(INOUT) :: YDSTOPH

    ! bit of cleaning
    IF (YDMODEL%YRML_GCONF%YRSPPT_CONFIG%LSPSDT) DEALLOCATE(THIS%PGP2DSDT)
    IF (YDMODEL%YRML_GCONF%YRSPP_CONFIG%LSPP) DEALLOCATE(THIS%PGP2DSPP)
