...
#endif
USE FIELD_CONTAINER_BASE_MOD, ONLY : TYPE_FIELD_INDEX_BASE, FIELD_CONTAINER_BASE, &
                                   & JP_STORAGE_ARR, JP_STORAGE1D, JP_STORAGEOLD
...
INTEGER(KIND=JPIM), PARAMETER :: JP_FADD=1,JP_FCOPY=2,JP_FAXPY=3,JP_FSET=4, JP_FMULT=5, JP_SCHUR_PROD=6,JP_DOT_PROD=7,JP_FRANDOM=8
! 5) Derived types, constants
PUBLIC :: FIELD_CONTAINER_GP, FIELD_CONTAINER_GP_CONTAINER
...
! 3) Field operators (indirect access)
  ...
  PROCEDURE :: FIELD_AXPY
  PROCEDURE :: FIELD_NORM
  PROCEDURE :: FIELD_SETVALUE
  PROCEDURE :: FIELD_ZERO
  PROCEDURE :: FIELD_MULT
  PROCEDURE :: FIELD_SCHUR_PROD
  PROCEDURE :: FIELD_DOT_PROD
  PROCEDURE :: FIELD_RANDOM
...
TYPE FIELD_CONTAINER_GP_CONTAINER
  CLASS(FIELD_CONTAINER_GP), POINTER :: OBJ
END TYPE FIELD_CONTAINER_GP_CONTAINER    
...
!=======================================================================================
SUBROUTINE FIELD_SETVALUE(F1, PZ, KFIDS, KBLOCK)
! -------------------------------------------------
! Add fields: f1=f1+f2
! -------------------------------------------------
...
CALL ONE_FIELD_ACTION(F1, JP_FSET, KFIDS, KBLOCK,PZ=PZ)

END SUBROUTINE FIELD_SETVALUE
!=======================================================================================
!=======================================================================================
SUBROUTINE FIELD_MULT(F1, PZ, KFIDS, KBLOCK)
! -------------------------------------------------
! Add fields: f1=f1+f2
! -------------------------------------------------
...
CALL ONE_FIELD_ACTION(F1, JP_FMULT, KFIDS=KFIDS, KBLOCK=KBLOCK, PZ=PZ)

END SUBROUTINE FIELD_MULT
!=======================================================================================
SUBROUTINE FIELD_ZERO(F1, KFIDS, KBLOCK)
! -------------------------------------------------
! Zero field: f1=0
! -------------------------------------------------
...
CALL ONE_FIELD_ACTION(F1, JP_FSET, KFIDS, KBLOCK,PZ=0.0_JPRB)

END SUBROUTINE FIELD_ZERO
!=======================================================================================
SUBROUTINE FIELD_RANDOM(F1, KFIDS, KBLOCK)
! -------------------------------------------------
! Create random fiels: f1=random number
! -------------------------------------------------
...
CALL ONE_FIELD_ACTION(F1, JP_FRANDOM, KFIDS, KBLOCK,PZ=0.0_JPRB)

END SUBROUTINE FIELD_RANDOM
!=======================================================================================
...
SUBROUTINE FIELD_SCHUR_PROD(F1, F2, KFIDS, KBLOCK)
! -------------------------------------------------
! Copy fields: f1=f2
! -------------------------------------------------
...
CALL TWO_FIELD_ACTION(F1, F2, JP_SCHUR_PROD, KFIDS, KBLOCK)

END SUBROUTINE FIELD_SCHUR_PROD
!=======================================================================================
SUBROUTINE FIELD_DOT_PROD(F1, F2,PDOTP ,KFIDS, KBLOCK)
! -------------------------------------------------
! Copy fields: f1=f2
! -------------------------------------------------
...
CALL TWO_FIELD_ACTION(F1, F2, JP_DOT_PROD, KFIDS, KBLOCK,POUT=PDOTP)

END SUBROUTINE FIELD_DOT_PROD

!=======================================================================================
SUBROUTINE FIELD_AXPY(F1, F2, PZ,KFIDS, KBLOCK)
! -------------------------------------------------
! Copy fields: f1=f2
! -------------------------------------------------
...
CALL TWO_FIELD_ACTION(F1, F2, JP_FAXPY, KFIDS, KBLOCK,PZ=PZ)

END SUBROUTINE FIELD_AXPY
!=======================================================================================

SUBROUTINE TWO_FIELD_ACTION(F1, F2, KACTION, KFIDS, KBLOCK, PZ,POUT)
! -------------------------------------------------
! Two-field actions:
!
!    add:   f1 = f1+f2
...
INTEGER(KIND=JPIM), OPTIONAL, TARGET, INTENT(IN)    :: KFIDS(:)
INTEGER(KIND=JPIM), OPTIONAL,         INTENT(IN)    :: KBLOCK
REAL(KIND=JPRB),    OPTIONAL,         INTENT(IN)    :: PZ
REAL(KIND=JPRB),    OPTIONAL,         INTENT(OUT)   :: POUT

INTEGER(KIND=JPIM) :: ID
INTEGER(KIND=JPIM) :: IUSE_BLOCK, IBLOCK, IBLOCK_START, IBLOCK_END,JCOL,IST,IEND
INTEGER(KIND=JPIM), POINTER :: IUSE_FIDS(:)
...
ENDIF
IF(KACTION == JP_DOT_PROD) POUT=0.0_JPRB

! NFTBC CAN BE OMP PARALLEL EVENTUALLY
DO IBLOCK = IBLOCK_START, IBLOCK_END
  IEND = F1%NPOINTS_BLOCK(IBLOCK)
  DO ID = 1, F1%NFIELDS
    IF (F1%FIELD_INDEX(ID)%L_ACTIVE .AND. F2%FIELD_INDEX(ID)%L_ACTIVE .AND. ANY(ID == IUSE_FIDS)) THEN
      IF (F1%METADATA(ID)%NDIMS == 1) THEN
        ...
        IF     (KACTION==JP_FADD)  THEN
          F1_1D(1:IEND) = F1_1D(1:IEND) + F2_1D(1:IEND)
        ELSEIF (KACTION==JP_FCOPY) THEN
          F1_1D(1:IEND) = F2_1D(1:IEND)
        ELSEIF (KACTION==JP_FAXPY) THEN
          F1_1D(1:IEND) = F1_1D(1:IEND) + PZ*F2_1D(1:IEND)
        ELSEIF (KACTION==JP_SCHUR_PROD) THEN
          F1_1D(1:IEND) = F1_1D(1:IEND) * F2_1D(1:IEND)
        ELSEIF (KACTION==JP_DOT_PROD) THEN
          POUT = POUT + DOT_PRODUCT(F1_1D(1:IEND),F2_1D(1:IEND))
        ENDIF
      ...
      ELSEIF (F1%METADATA(ID)%NDIMS == 2) THEN
        ...
        IF     (KACTION==JP_FADD)  THEN
          F1_2D(1:IEND,:) = F1_2D(1:IEND,:) + F2_2D(1:IEND,:)
        ELSEIF (KACTION==JP_FCOPY) THEN
          F1_2D(1:IEND,:) = F2_2D(1:IEND,:)
        ELSEIF (KACTION==JP_FAXPY) THEN
          F1_2D(1:IEND,:) = F1_2D(1:IEND,:) + PZ*F2_2D(1:IEND,:)
        ELSEIF (KACTION==JP_SCHUR_PROD) THEN
          F1_2D(1:IEND,:) = F1_2D(1:IEND,:) * F2_2D(1:IEND,:)
        ELSEIF (KACTION==JP_DOT_PROD) THEN
          DO JCOL=1,SIZE(F1_2D(1,:))
            POUT  = POUT + DOT_PRODUCT(F1_2D(1:IEND,JCOL),F2_2D(1:IEND,JCOL))
          ENDDO
       ENDIF
...
!=======================================================================================
SUBROUTINE ONE_FIELD_ACTION(F1, KACTION, KFIDS, KBLOCK, PZ)
! -------------------------------------------------
! Two-field actions: 
!
!    add:   f1 = f1+f2 
...
INTEGER(KIND=JPIM) :: ID, IPOINTS, INLEVS
INTEGER(KIND=JPIM) :: IUSE_BLOCK, IBLOCK, IBLOCK_START, IBLOCK_END,ISEED
INTEGER(KIND=JPIM), POINTER :: IUSE_FIDS(:)
REAL(KIND=JPRB), POINTER :: F1_1D(:), F1_2D(:,:), F2_1D(:), F2_2D(:,:)
...
ELSE
  IUSE_FIDS => F1%MALL_FIELDS
ENDIF
IF(KACTION == JP_FRANDOM)  THEN
  ISEED=1
  CALL INITIALIZE_RANDOM_NUMBERS (ISEED,YL_RANDOM_STREAM)
ENDIF

! NFTBC CAN BE OMP PARALLEL EVENTUALLY
DO IBLOCK = IBLOCK_START, IBLOCK_END
  DO ID = 1, F1%NFIELDS
    IF (F1%FIELD_INDEX(ID)%L_ON .AND. ANY(ID == IUSE_FIDS)) THEN
      IPOINTS = F1%NPOINTS_BLOCK(IBLOCK)
      IF (F1%METADATA(ID)%NDIMS == 1) THEN

        CALL FMAP_1D(F1, IBLOCK, ID, F1_1D)
        IF     (KACTION==JP_FSET)  THEN
          F1_1D = PZ
        ELSEIF (KACTION==JP_FMULT)  THEN
          F1_1D(1:IPOINTS) = PZ*F1_1D(1:IPOINTS)
        ELSEIF (KACTION==JP_FRANDOM)  THEN
          CALL GAUSSIAN_DISTRIBUTION (F1_1D,YL_RANDOM_STREAM)
        ENDIF

      ELSEIF (F1%METADATA(ID)%NDIMS == 2) THEN

        INLEVS = F1%NLEVELS(F1%METADATA(ID)%D2TYPE)
        CALL FMAP_2D(F1, IBLOCK, ID, F1_2D)
        IF     (KACTION==JP_FSET)  THEN
          F1_2D(1:IPOINTS,1:INLEVS) = PZ
        ELSEIF (KACTION==JP_FMULT)  THEN
          F1_2D(1:IPOINTS,1:INLEVS) = PZ*F1_2D(1:IPOINTS,1:INLEVS)
        ELSEIF (KACTION==JP_FRANDOM)  THEN
          ALLOCATE(ZTMP(SIZE(F1_2D)))
          CALL GAUSSIAN_DISTRIBUTION (ZTMP,YL_RANDOM_STREAM)
          F1_2D = RESHAPE(ZTMP,SHAPE(F1_2D))
          DEALLOCATE(ZTMP)
        ENDIF
...
END SUBROUTINE ONE_FIELD_ACTION
!=======================================================================================
