...
TYPE, PUBLIC :: FIELDS_BASE
  ...
  TYPE(FIELD_CONTAINER_GP), POINTER :: FIELD_ECV   => NULL()
...
IF( SELF%LHAS_ECV) CALL SELF%FIELD_ECV%FIELD_DESTROY()
SELF%FIELD => NULL()
SELF%FIELD_ECV => NULL()
SELF%GEOM => NULL()
...
ELSE
  ...
  ENDIF
  IF( SELF%LHAS_ECV) THEN
    RHS_SELFRES%FIELD_ECV => RHS_SELFRES%FIELDSTORE_ECV
    CALL RHS_SELFRES%FIELD_ECV%FIELD_CREATE(ECV_FIELD_METADATA,RHS_SELFRES%GEOM%YRGEM%NGPTOT, &
     & (/ RHS_SELFRES%GEOM%YRDIMV%NFLEVG,YRJBALPHACV%NDIM2_ALPHACV,&
     & YRSKTECV%NSFCECV*YRSKTECV%NDIM2_SFCECV,&
     & YRSSHECV%NSFCECV*YRSSHECV%NDIM2_SFCECV,&
     & YRTSLECV%NSFCECV*YRTSLECV%NDIM2_SFCECV/),&
     & RHS_SELFRES%GEOM%YRDIM%NPROMA,KFIDS=SELF%VARS%FIELDIDS_ECV)
    CALL FIELD_CONTAINER_INTERP(RHS%GEOM,RHS_SELFRES%GEOM,RHS%FIELD_ECV,RHS_SELFRES%FIELD_ECV)
  ENDIF
...
CALL SELF%FIELD%FIELD_COPY(RHS_SELFRES_PTR%FIELD,KFIDS=IFIDS)

IF( SELF%LHAS_ECV) THEN
  CALL SELF%FIELD_ECV%FIELD_COPY(RHS_SELFRES_PTR%FIELD_ECV)
ENDIF
...
SUBROUTINE FIELDS_AXPY(SELF,PZ,RHS,KFIDS,KFIDS_POSITIVE,POSLIMIT,LDSPEC_INT)
...
REAL(KIND=JPRB),OPTIONAL,          INTENT(IN) :: POSLIMIT
...
ELSE
  IFIDS => SELF%VARS%FIELDIDS
ENDIF
IF(PRESENT(POSLIMIT)) THEN
  ZLIMIT=POSLIMIT
ELSE
  ZLIMIT=0.0_JPRB
ENDIF
LLSAME_RESOL = GEOMETRY_SAME(SELF%GEOM,RHS%GEOM)
WRITE(NULOUT,*) 'FIELDS_BASE_MOD: FIELDS_AXPY ',LLSAME_RESOL,SELF%LHAS_ECV,RHS%LHAS_ECV
IF(LLSAME_RESOL) THEN
  RHS_SELFRES_PTR => RHS
  ALLOCATE(IFIDS_GP(SIZE(IFIDS)))
  IFIDS_GP(:) = IFIDS(:)
ELSE
  WRITE(NULOUT,*) 'FIELDS_BASE_MOD: FIELDS_AXPY INTERP',SELF%GEOM%YRGEM%NGPTOTG,RHS%GEOM%YRGEM%NGPTOTG,&
    & LLSPEC_INT, NINTERPINCR
  IF(LLSPEC_INT) THEN
    CALL SPLIT_SPEC_GP(IFIDS,IFIDS_SPEC,IFIDS_GP)
  ...
  ENDIF
  IF( SELF%LHAS_ECV.AND.RHS%LHAS_ECV) THEN
    RHS_SELFRES%FIELD_ECV => RHS_SELFRES%FIELDSTORE_ECV
    CALL RHS_SELFRES%FIELD_ECV%FIELD_CREATE(ECV_FIELD_METADATA,RHS_SELFRES%GEOM%YRGEM%NGPTOT, &
     & (/ RHS_SELFRES%GEOM%YRDIMV%NFLEVG,YRJBALPHACV%NDIM2_ALPHACV,&
     & YRSKTECV%NSFCECV*YRSKTECV%NDIM2_SFCECV,&
...
CALL SELF%FIELD%FIELD_AXPY(RHS_SELFRES_PTR%FIELD,PZ,KFIDS=IFIDS_GP)

IF( SELF%LHAS_ECV.AND.RHS%LHAS_ECV) THEN
  CALL SELF%FIELD_ECV%FIELD_AXPY(RHS_SELFRES_PTR%FIELD_ECV,PZ)
ENDIF
    ...
    DO WHILE (SELF%FIELD%FIELD_ITERATED(KFIDS=KFIDS_POSITIVE,KBLOCK=JBLK,FAC=YLFAC) + &
     & RHS_SELFRES_PTR%FIELD%FIELD_ITERATED(KFIDS=KFIDS_POSITIVE,KBLOCK=JBLK,FAC=YLRHS)  == 2)
      IF(ASSOCIATED(YLFAC%RR1)) THEN
        YLFAC%RR1(IST:IEND) = MAX(YLFAC%RR1(IST:IEND),ZLIMIT)
      ELSE
        YLFAC%RR2(IST:IEND,:) = MAX(YLFAC%RR2(IST:IEND,:),ZLIMIT)
      ENDIF
    ENDDO
  ENDDO
  !$OMP END DO
...
CALL SELF%FIELD%FIELD_DOT_PROD(RHS%FIELD,ZZ(1))
IF( SELF%LHAS_ECV) THEN
  IF(.NOT.RHS%LHAS_ECV) CALL ABOR1('FIELDS_BASE_MOD:FIELDS_DOT_PROD - SELF HAS ECV BUT NOT RHS')
  CALL SELF%FIELD_ECV%FIELD_DOT_PROD(RHS%FIELD_ECV,Z_ECV)
  ZZ(1) = ZZ(1)+Z_ECV
ENDIF
...
IF( SELF%LHAS_ECV) THEN
  DO JBLK=1,SELF%GEOM%YRDIM%NGPBLKS
    ...
    IST = 1
    IEND = MIN(NPROMA,NGPTOT-IKGLO+1)
    DO WHILE (SELF%FIELD_ECV%FIELD_ITERATED(KFIDS=SELF%VARS%FIELDIDS_ECV,KBLOCK=JBLK,FAC=YLFAC) == 1) 
      IF(ASSOCIATED(YLFAC%RR1)) THEN
        ZZ(1) = ZZ(1)+DOT_PRODUCT(YLFAC%RR1(IST:IEND),YLFAC%RR1(IST:IEND))
      ELSE
        DO JCOL=1,SIZE(YLFAC%RR2(1,:))
...
CALL SELF%FIELD%FIELD_SCHUR_PROD(RHS%FIELD)
IF( SELF%LHAS_ECV) THEN
  IF(.NOT.RHS%LHAS_ECV) CALL ABOR1('FIELDS_BASE_MOD:FIELDS_SCHUR_PROD - SELF HAS ECV BUT NOT RHS')
  CALL SELF%FIELD_ECV%FIELD_SCHUR_PROD(RHS%FIELD_ECV)
ENDIF
...
ELSE
  CALL SPLIT_SPEC_GP(IFIDS,IFIDS_SPEC,IFIDS_GP)
  WRITE(NULOUT,*) 'FIELDS_BASE_MOD:FIELDS_ADD_INCR - LIMIT ',IFIDS_GP
  DEALLOCATE(IFIDS_SPEC)
ENDIF
CALL SELF%FIELDS_AXPY(1.0_JPRB,RHS,KFIDS=IFIDS,LDSPEC_INT=.TRUE.,&
  & KFIDS_POSITIVE=IFIDS_GP,POSLIMIT=100.0_JPRB*EPSILON(ZEPS))
!CALL SELF%FIELDS_GPNORM('FIELDS_ADD_INCR : SELF AFTER ',KFIDS=IFIDS)

DEALLOCATE(IFIDS_GP)

IF(SELF%LHAS_ECV) CALL WRJBECPHYSECV(SELF%GEOM, SELF%FIELD, SELF%FIELD_ECV) 
...
USE FIELD_CONTAINER_OPER_MOD,ONLY: FIELD_CONTAINER_WRITE_FILE
CLASS(FIELDS_BASE),       INTENT(INOUT) :: SELF
...
WRITE(NULOUT,*) ' FIELDS_BASE_MOD:FIELDS_WRITE - WRITING ',CLFNAME,' ',LLWRITE_SPEC,' ',IFIDS
...
YLGBH_SAVE=YGBH
CALL SU_GRIB_API(SELF%GEOM,SELF%GEOM%YRVAB,LLMCC04,YGBH)
...
IF( SELF%LHAS_ECV) THEN
  CALL FIELD_CONTAINER_WRITE_FILE(SELF%GEOM,SELF%FIELD_ECV,SELF%VARS%FIELDIDS_ECV,'ECV_'//CLFNAME)
ENDIF
