...
TYPE, PUBLIC,EXTENDS(FIELDS_BASE) :: FIELDS
  ...
  TYPE(FIELD_CONTAINER_GP)     :: FIELD5_ECV
  TYPE(FIELD_CONTAINER_GP)     :: FIELD7_ECV
  REAL(KIND=JPRB), ALLOCATABLE :: YRGFLT5Q(:,:,:)
  REAL(KIND=JPRB), ALLOCATABLE :: YRGFLT5GO3(:,:,:)
...
PUBLIC :: FIELDS_CREATE, FIELDS_DELETE, FIELDS_ZEROS,FIELDS_SET_MODEL,&
 ...
 & FIELDS_RANDOM, FIELDS_CHANGE_RESOL, FIELDS_GPNORM,FIELDS_CONTAIN,FIELDS_BACKSTITCH,&
 & FIELDS_FORCE_WITH, FIELDS_STORE_GFL_Q, FIELDS_LIMIT_Q, FIELDS_LIMIT_GO3,&
 & FIELDS_STORE_GFL_GO3, FIELDS_CONTAINER
...
USE ECV_DEFINITIONS    , ONLY : ECV_FIELD_METADATA
USE YOMJBSFCECV        , ONLY : YRSKTECV, YRSSHECV, YRTSLECV
USE YOMJBALPHACV_DATA  , ONLY : YRJBALPHACV
...
!  CALL SUFPC(SELF%GEOM)
  ...
  CALL SU_SURF_FLDS(SELF%GEOM%YRDIMV,SELF%YRSURF,YDMODEL,KPPVCLIX=KPPVCLIX,KPPEDR=KPPEDR)
!ENDIF
ENDIF
...
CALL SUCFU(SELF%GEOM,SELF%YRCFU,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_PHY_RAD%YRERAD,YDMODEL%YRML_PHY_MF%YRPHY, &
 & NULOUT,YDCFUPP=YDCFUPP)

!*    Initialize instantaneous fluxes requests
WRITE(NULOUT,*) '------ Set up instantaneous fluxes diags '
CALL SUXFU(SELF%GEOM,SELF%YRXFU,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_PHY_MF%YRPHY,NULOUT,YDXFUPP=YDXFUPP)
...
IF ( (YDMODEL%YRML_PHY_EC%YREPHY%LEPHYS) .OR. &
   & (.NOT. LECMWF .AND. YDMODEL%YRML_PHY_EC%YREPHY%LEMWAVE)) CALL SELF%YEC_PHYS_FIELDS%CREATE(SELF%GEOM,YDMODEL%YRML_PHY_G%YRDPHY)
...
IF(SELF%VARS%LHAS_ECV) THEN
!  SELF%FIELD_ECV => SELF%FIELDSTORE_ECV
  ALLOCATE(SELF%FIELD_ECV)

  CALL SELF%FIELD_ECV%FIELD_CREATE(ECV_FIELD_METADATA,SELF%GEOM%YRGEM%NGPTOT, &
   & (/ SELF%GEOM%YRDIMV%NFLEVG,YRJBALPHACV%NDIM2_ALPHACV,&
   & YRSKTECV%NSFCECV*YRSKTECV%NDIM2_SFCECV,&
   & YRSSHECV%NSFCECV*YRSSHECV%NDIM2_SFCECV,&
   & YRTSLECV%NSFCECV*YRTSLECV%NDIM2_SFCECV/),&
   & SELF%GEOM%YRDIM%NPROMA,KFIDS=SELF%VARS%FIELDIDS_ECV)
  IF(.NOT.L_OOPS) THEN
    CALL SELF%FIELD5_ECV%FIELD_CREATE(ECV_FIELD_METADATA,SELF%GEOM%YRGEM%NGPTOT, &
     & (/ SELF%GEOM%YRDIMV%NFLEVG,YRJBALPHACV%NDIM2_ALPHACV,&
     & YRSKTECV%NSFCECV*YRSKTECV%NDIM2_SFCECV,&
     & YRSSHECV%NSFCECV*YRSSHECV%NDIM2_SFCECV,&
     & YRTSLECV%NSFCECV*YRTSLECV%NDIM2_SFCECV/),&
     & SELF%GEOM%YRDIM%NPROMA,KFIDS=SELF%VARS%FIELDIDS_ECV)
    CALL SELF%FIELD7_ECV%FIELD_CREATE(ECV_FIELD_METADATA,SELF%GEOM%YRGEM%NGPTOT, &
     & (/ SELF%GEOM%YRDIMV%NFLEVG,YRJBALPHACV%NDIM2_ALPHACV,&
     & YRSKTECV%NSFCECV*YRSKTECV%NDIM2_SFCECV,&
     & YRSSHECV%NSFCECV*YRSSHECV%NDIM2_SFCECV,&
     & YRTSLECV%NSFCECV*YRTSLECV%NDIM2_SFCECV/),&
...
IF(YGBH%NGRIB_HANDLE_GG < 0) THEN
  ...
  CALL SU_GRIB_API(SELF%GEOM,SELF%GEOM%YRVAB, &
...
! ------------------------------------------------------------------------------
SUBROUTINE FIELDS_BACKSTITCH(SELF,GEOM,YDMODEL,VARS)
USE TYPE_MODEL         , ONLY : MODEL
USE ECV_DEFINITIONS    , ONLY : ECV_FIELD_METADATA
USE YOMJBSFCECV        , ONLY : YRSKTECV, YRSSHECV, YRTSLECV
USE YOMJBALPHACV_DATA  , ONLY : YRJBALPHACV
...
END SUBROUTINE FIELDS_BACKSTITCH
...
IF(SELF%VARS%LHAS_ECV) THEN
  CALL SELF%FIELD_ECV%FIELD_DESTROY()
ENDIF
NULLIFY(SELF%GEOM)
...
ENDIF
IF (ASSOCIATED(SELF%SURFVARS)) THEN
  NULLIFY(SELF%SURFVARS)
ENDIF
...
IF(SELF%VARS%LHAS_ECV) THEN
  CALL SELF%FIELD_ECV%FIELD_ZERO()
ENDIF
...
SELF%LHAS_ECV = RHS%LHAS_ECV
IF( SELF%LHAS_ECV) THEN
  WRITE(NULOUT,*) 'FIELD_COPY ECV ',ASSOCIATED(SELF%FIELD_ECV)
  IF(.NOT.ASSOCIATED(SELF%FIELD_ECV)) ALLOCATE(SELF%FIELD_ECV)
  CALL SELF%FIELD_ECV%FIELD_COPY(RHS%FIELD_ECV)
ENDIF
...
IF ( (SELF%STATE_MODEL%YRML_PHY_EC%YREPHY%LEPHYS) .OR. &
   & (.NOT. LECMWF .AND. SELF%STATE_MODEL%YRML_PHY_EC%YREPHY%LEMWAVE) ) &
...
IF( SELF%LHAS_ECV) THEN
  CALL SELF%FIELD_ECV%FIELD_ADD(RHS%FIELD_ECV)
ENDIF
...
IF( SELF%LHAS_ECV) THEN
  CALL SELF%FIELD_ECV%FIELD_MULT(PZ)
ENDIF
...
IF( SELF%LHAS_ECV) THEN
  CALL SELF%FIELD_ECV%FIELD_AXPY(RHS%FIELD_ECV,PZ)
ENDIF
...
USE PARKIND1, ONLY : JPRB
USE YOMCT0,   ONLY : L_OOPS_VERBOSE_NORMS
USE YOMGFL,   ONLY : AXPBY_YOMGFL
USE YOMGMV,   ONLY : AXPBY_YOMGMV
USE YOMHOOK,  ONLY : LHOOK, DR_HOOK, JPHOOK
IMPLICIT NONE
...
ELSE
  ...
  ENDIF
  IF( SELF%LHAS_ECV) THEN
    CALL FIELD_CONTAINER_INTERP(RHS%GEOM,SELF%GEOM,RHS%FIELD_ECV,SELF%FIELD_ECV)
    CALL FIELD_CONTAINER_GPNORM(SELF%GEOM,SELF%FIELD_ECV)
  ENDIF
ENDIF
...
!---------------------------------
#include "surf_vregrid_snowml.intfb.h"
!---------------------------------

IF (LHOOK) CALL DR_HOOK('FIELDS_MOD:FIELDS_FORCE_WITH',0,ZHOOK_HANDLE)
LLSAME_RESOL = GEOMETRY_SAME(SELF%GEOM,RHS%GEOM)
WRITE(NULOUT,*) 'FIELDS_FORCE_WITH CALLED ', LLSAME_RESOL,SELF%GEOM%YRGEM%NGPTOTG,RHS%GEOM%YRGEM%NGPTOTG,NSTEP,SELF%LHAS_ECV,RHS%LHAS_ECV
CALL FLUSH(NULOUT)
...
IF(SIZE(IFIDS_SPEC) > 0) THEN
  SELF%YRSPEC = RHS%YRSPEC
ENDIF
IF( RHS%LHAS_ECV) THEN
  SELF%LHAS_ECV = .TRUE.
  WRITE(NULOUT,*) 'FIELD_FORCE_WITH ECV ',ASSOCIATED(SELF%FIELD_ECV)
  IF(.NOT.ASSOCIATED(SELF%FIELD_ECV)) ALLOCATE(SELF%FIELD_ECV)
ENDIF
IF(LLSAME_RESOL) THEN
...
#include "symtransin_oops.intfb.h"
...
SUBROUTINE FIELDS_STORE_GFL_GO3(SELF)
IMPLICIT NONE
TYPE(FIELDS), INTENT(INOUT) :: SELF
INTEGER(KIND=JPIM) :: JGFL, JSTGLO, ICEND, IBL
...
!
! Store GEMS ozone (GO3) state in the top model levels

IF (ALLOCATED(SELF%YRGFLT5GO3)) DEALLOCATE(SELF%YRGFLT5GO3)
IF (NFLEVG < 137 ) THEN
  ALLOCATE(SELF%YRGFLT5GO3(NPROMA,5,NGPBLKS))
ELSE
  ALLOCATE(SELF%YRGFLT5GO3(NPROMA,15,NGPBLKS))
ENDIF

DO JGFL=1,NUMFLDS
  IF(YCOMP(JGFL)%LGP .AND. YCOMP(JGFL)%IGRBCODE == 210203) THEN
    WRITE(NULOUT,*) ' FIELDS:FIELDS_STORE_GFL_GO3: CALLED'
    DO JSTGLO=1,NGPTOT,NPROMA
      ICEND=MIN(NPROMA,NGPTOT-JSTGLO+1)
      IBL=(JSTGLO-1)/NPROMA+1
      IF (NFLEVG < 137 ) THEN
        SELF%YRGFLT5GO3(1:ICEND,1:5,IBL) = GFL(1:ICEND,1:5,YCOMP(JGFL)%MP,IBL)
      ELSE
        SELF%YRGFLT5GO3(1:ICEND,1:15,IBL) = GFL(1:ICEND,1:15,YCOMP(JGFL)%MP,IBL)
      ENDIF
    ENDDO
  ENDIF
ENDDO
...
END SUBROUTINE FIELDS_STORE_GFL_GO3
...
DO JGFL=1,NUMFLDS
  IF(YCOMP(JGFL)%LGP .AND. YCOMP(JGFL)%IGRBCODE == 210203) THEN
    ...
    DO JSTGLO=1,NGPTOT,NPROMA
      ...
      IBL=(JSTGLO-1)/NPROMA+1
      IF (NFLEVG < 137 ) THEN
        GFL(1:ICEND,1:5,YCOMP(JGFL)%MP,IBL) = SELF%YRGFLT5GO3(1:ICEND,1:5,IBL)
      ELSE
        GFL(1:ICEND,1:15,YCOMP(JGFL)%MP,IBL) = SELF%YRGFLT5GO3(1:ICEND,1:15,IBL)
      ENDIF
    ENDDO
  ENDIF
ENDDO
