...
!           Original : 2015-01-01
!
!      M. Leutbecher : 2024-08-28 : introduced abstract*Level
!  
!-----------------------------------------------------------------------------
...
USE YOM_GRIB_CODES, ONLY : NGRBMX2T3, NGRBMN2T3, NGRB10FG3, NGRBMXTPR3, &
 ...
 & NGRBCLBT, NGRBCSBT, NGRBCDRFL, NGRBCRRFL, NGRBMXCAP6, NGRBMXCAPS6, &
 ...
 & NGRBVIWVE, NGRBVIWVN,&
 & NGRBMUCAPE, NGRBMUCIN, NGRBMLCAPE50, NGRBMLCAPE100, NGRBMLCIN50, NGRBMLCIN100, NGRBMUDEPL,&
 & NGRBTROPOTP, &
 & NGRBSDSL, NGRBRSN, NGRBTSN, NGRBWSN, NGRBCH4WET, &
...
SELECT CASE(KGRIBCD)
  ...
  CASE(NGRBMXCAP6, NGRBMXCAPS6, NGRBVIWVE, NGRBVIWVN, &
     & NGRBLITOTI, NGRBLITOTA1, NGRBLITOTA3, NGRBLITOTA6, &
     & NGRBLICGI, NGRBLICGA1, NGRBLICGA3, NGRBLICGA6, NGRBMUCIN,&
  ...
  CASE(210170, 210252:210253, 215212:215226, 217057:217206, 217222:217232, &
     ...
     & 222001:222256,223001:223256,228104,229007,229008,228109,229001)
    ! These are newer atmospheric composition parameters that have
    ! only been defined as GRIB-2, under DGOV-83, DGOV-353 and later.
    IEDITION = 2
  CASE DEFAULT
    IEDITION = 1
    IF (ALLOCATED(YRECVDATA%MGRIBS_ECV_FIDS)) THEN
      IF (ANY(YRECVDATA%MGRIBS_ECV_FIDS == KGRIBCD)) IEDITION = 2
    ENDIF
END SELECT
...
! Activate the new encoding of the 14 ERAS model level fc fields.
! via stepType for accumulated fields in GRIB 2
SELECT CASE (IPARAM)
CASE(NGRBMINERA:NGRBMAXERA)
...
IF (IPARAM == NGRBCLBT .OR. IPARAM == NGRBCSBT .OR. IPARAM == NGRBCDRFL .OR. IPARAM == NGRBCRRFL) THEN
...
ELSEIF(CDLTYPE(1:2) == 'AL' .OR. CDLTYPE(1:1) == 'a') THEN
  IF (KLEV==0) THEN
    CALL IGRIB_SET_VALUE(KGRIB_HANDLE,'typeOfLevel','abstractSingleLevel')
  ELSE
    CALL IGRIB_SET_VALUE(KGRIB_HANDLE,'typeOfLevel','abstractMultipleLevels')
    CALL IGRIB_SET_VALUE(KGRIB_HANDLE,'level',KLEV)
  ENDIF
...
ELSE
  ...
  CASE(NGRBPTYPESEVR6)
    ...
    ISECSTART  = ISEC-6*3600
    IF (ISECSTART < ISEC0) LDVALID = .FALSE.
  CASE (222001:222256, 223001:223256,NGRBCH4WET) ! DGOV-353, has to be 'accum' also for SEC=0
    CALL IGRIB_SET_VALUE(KGRIB_HANDLE,'stepType','accum')
