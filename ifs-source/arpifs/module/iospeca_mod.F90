...
SUBROUTINE IOSPECA_START(YDGEOMETRY,YDGFL,YDML_GCONF,YDDYNA,KACTION,YDCTX,YDSPEC)
...
! Compute PD,VD
    ...
    IF (YDDYNA%LNHDYN) THEN
      ...
      IF (YDDYNA%LNHX) ALLOCATE (YDCTX%RSPNHX (YDGEOMETRY%YRDIMV%NFLSUR,YDGEOMETRY%YRDIM%NSPEC2))
 
! Copy to avoid overwriting
      YDCTX%RSPSPD (:,:) = YDSPEC%SPD (:,:)
      YDCTX%RSPSVD (:,:) = YDSPEC%SVD (:,:)
      IF (YDDYNA%LNHX) YDCTX%RSPNHX (:,:) = YDSPEC%NHX (:,:)
...
SUBROUTINE IOSPECA_COUNT(YDGEOMETRY,YDML_GCONF,YDDYNA,KACTION,KFNUM,LDINOR,K3DINI,YDMCUF)
...
SUBROUTINE IOSPECA_FINISH (YDGEOMETRY, YDDYNA,KACTION, YDCTX,YDSPEC)
...
! Restore NH model variables in spectral arrays
    ...
    IF (YDDYNA%LNHDYN) THEN
      ...
      IF (YDDYNA%LNHX) YDSPEC%NHX (:,:) = YDCTX%RSPNHX (:,:)
    
      DEALLOCATE (YDCTX%RSPSPD)
      DEALLOCATE (YDCTX%RSPSVD)
      IF (YDDYNA%LNHX) DEALLOCATE (YDCTX%RSPNHX)
    ENDIF
...
SUBROUTINE IOSPECA_SELECTD(YDGEOMETRY,YDML_GCONF,YDDYNA,KACTION,YDFLDSC,LDINOR,K3DINI,YDMCUF)
