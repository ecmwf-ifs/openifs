...
USE SURFACE_FIELDS_MIX , ONLY : SURF_NORM
...
#include "sumoderrconfptrs.intfb.h"
...
WRITE(NULOUT,*)'MODEL_INIT:FORECAST NORMS AT T0',SELF%COBJECT_ID
CALL SPNORM(YDFIELDS%GEOM,SELF%YRML_GCONF,SELF%YRML_DYN%YRDYNA,YDFIELDS%YRSPEC)
CALL SURF_NORM(YDFIELDS%GEOM,YDFIELDS%YRSURF)

IF (LELAM) THEN
   CALL ETRANSDIRH_FROM_T0(YDFIELDS%GEOM,SELF%YRML_DYN%YRDYNA%LGRADSP, YDFIELDS%YRGFL,YDFIELDS%YRGMV,CL, &
   ...
   CALL ETRANSINVH(YDFIELDS%GEOM,YDFIELDS%YRGFL,YDFIELDS%YRGMV,SELF%YRML_DIAG,SELF%YRML_GCONF, SELF%YRML_DYN%YRDYNA,&
      &   CL,YDFIELDS%YRSPEC)
ELSE
