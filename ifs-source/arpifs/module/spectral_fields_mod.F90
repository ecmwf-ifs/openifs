...
IMPLICIT NONE
PRIVATE
PUBLIC SPECTRAL_FIELD, SPECTRAL_FIELD_CONTAINER, SETUP_SPEC, ALLOCATE_SPEC, DEALLOCATE_SPEC, &
...
! Wrapper around SPECTRAL_FIELD
TYPE :: SPECTRAL_FIELD_CONTAINER
  CLASS(SPECTRAL_FIELD), POINTER :: OBJ
END TYPE SPECTRAL_FIELD_CONTAINER
...
LOGICAL :: LLMISSING, LLSET2D(KS2D)
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
...
ALLOCATE(YDSP%MYMS(KNUMP))
ALLOCATE(YDSP%NALLMS(KSMAX+1))
ALLOCATE(YDSP%NPTRMS(NPRTRW))
ALLOCATE(YDSP%NUMLL(NPRTRV))
ALLOCATE(YDSP%NPTRLL(NPRTRV))
ALLOCATE(YDSP%NPSURF(NPRTRV))
ALLOCATE(YDSP%MYLEVS(KFLEVL))

YDSP%MYMS(:)=KMYMS(:)
IF (SIZE(YDSP%NALLMS(:)) == SIZE(KALLMS(:))) YDSP%NALLMS(:)=KALLMS(:)  ! Only for LELAM ??
YDSP%NPTRMS(:)=KPTRMS(:)
YDSP%NUMLL(:)=KNUMLL(:)
YDSP%NPTRLL(:)=KPTRLL(:)
YDSP%NPSURF(:)=KPSURF(:)
DO JL=1,YDSP%NFLEVL
  YDSP%MYLEVS(JL) = YDSP%NPTRLL(MYSETV)+JL-1
ENDDO
LLSET2D(:) = .FALSE.

IF (LELAM) THEN
  CALL ELLIPS(KSMAX,KMSMAX,IKNTMP,IKMTMP)
  IMAXMS=KMSMAX/NPRTRW+1
  ALLOCATE(YDSP%NESM0 (0:KMSMAX))
  ALLOCATE(YDSP%NESM0G(0:KMSMAX))
  ALLOCATE(YDSP%NCPL4M(0:KMSMAX))
ELSE
  IMAXMS=KSMAX/NPRTRW+1
  ALLOCATE(YDSP%NASM0 (0:KSMAX))
  ALLOCATE(YDSP%NASM0G(0:KSMAX))
ENDIF

ALLOCATE(YDSP%NUMPP(NPRTRW))
ALLOCATE(YDSP%MS_PROC(IMAXMS,NPRTRW))
ALLOCATE(YDSP%NS2L(NPRTRV))
ALLOCATE(YDSP%NGRIB(KS3D+KS2D))

YDSP%NSMAX=KSMAX
YDSP%NS2G=KS2D
YDSP%NS2L(:)=KS2D*YDSP%NPSURF(:)
YDSP%NS2D=YDSP%NS2L(MYSETV)
YDSP%NS3D=KS3D
...
IF (LELAM) THEN
  ...
  YDSP%NSPEC2G=0
  DO JM=0,KMSMAX
    ICPL4M(JM) = 4*(IKNTMP(JM)+1)
    YDSP%NSPEC2G = YDSP%NSPEC2G + 4*(IKNTMP(JM)+1)
  ENDDO
  YDSP%NCPL4M(:)=ICPL4M(:)
  YDSP%NESM0G(0)=1
  DO JM=1,KMSMAX
    YDSP%NESM0G(JM) = YDSP%NESM0G(JM-1) + ICPL4M(JM-1)
  ENDDO
...
ELSE
  YDSP%NS1D=0
  YDSP%NMSMAX=0
  YDSP%NSPEC2G=(KSMAX+1)*(KSMAX+2)
  YDSP%NASM0G(0)=1
  DO JM=0,KSMAX-1
    YDSP%NASM0G(JM+1) = YDSP%NASM0G(JM) + 2*(KSMAX-JM+1)
  ENDDO

  CALL INI_SPEC_DIST(KSMAX,KSMAX,NPRTRW,MYSETW,KSPEC2=YDSP%NSPEC2,&
    & KASM0=YDSP%NASM0,KUMPP=YDSP%NUMPP,KPTRMS=YDSP%NPTRMS,KALLMS=YDSP%NALLMS)
ENDIF

YDSP%NUMP = YDSP%NUMPP(MYSETW)
DO JP=1,NPRTRW
  DO JM=1,YDSP%NUMPP(JP)
    YDSP%MS_PROC(JM,JP)=YDSP%NALLMS(YDSP%NPTRMS(JP)-1+JM)
  ENDDO
ENDDO

YDSP%NGRIB(:)=0
YDSP%NGRIB3 => YDSP%NGRIB(1:KS3D)
YDSP%NGRIB2 => YDSP%NGRIB(KS3D+1:KS3D+KS2D)

ALLOCATE(YDSP%SP3D(YDSP%NFLEVL,YDSP%NSPEC2,YDSP%NS3D))
ALLOCATE(YDSP%SP2D(YDSP%NSPEC2,YDSP%NS2D))
IF (LELAM) THEN
  ALLOCATE(YDSP%SP1D(YDSP%NFLEVL,YDSP%NS1D))
ELSE
  ALLOCATE(YDSP%SP1D(1,2))
ENDIF
YDSP%SP3D(:,:,:)=0.0_JPRB
YDSP%SP2D(:,:)=0.0_JPRB
YDSP%SP1D(:,:)=0.0_JPRB
...
!cf nspsizeg and nspsizel contain only the horizontal spectral fields
! (eg. not the mean wind profiles for aladin case)

YDSP%NSPSIZEL = (YDSP%NFLEVL*YDSP%NS3D+YDSP%NS2D)*YDSP%NSPEC2
YDSP%NSPSIZEG = (YDSP%NFLEVG*YDSP%NS3D+YDSP%NS2G)*YDSP%NSPEC2G

IPT=0

IND=INTLOC(KGRIB,NGRBVO)
IF (IND>0) THEN
  IPT=IPT+1
  YDSP%VOR => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBVO
ELSE
  YDSP%VOR => SPECTRAL_DUMMY3D
ENDIF

IND=INTLOC(KGRIB,NGRBD)
IF (IND>0) THEN
  IPT=IPT+1
  YDSP%DIV => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBD
ELSE
  YDSP%DIV => SPECTRAL_DUMMY3D
ENDIF

ISC3=IPT
IF (YDSP%NS3D>ISC3) THEN
  YDSP%SCAL => YDSP%SP3D(:,:,ISC3+1:YDSP%NS3D)
ELSE
  YDSP%SCAL => SPECTRAL_DUMMY4D
ENDIF

ITHER=0
IND=INTLOC(KGRIB,NGRBT)
IF (IND>0) THEN
  IPT=IPT+1
  ITHER=ITHER+1
  YDSP%T => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBT
ELSE
  YDSP%T => SPECTRAL_DUMMY3D
ENDIF

IND=INTLOC(KGRIB,NGRB118)
IF (IND>0) THEN
  IPT=IPT+1
  ITHER=ITHER+1
  YDSP%SPD => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRB118
ELSE
  YDSP%SPD => SPECTRAL_DUMMY3D
ENDIF

IND=INTLOC(KGRIB,NGRB119)
IF (IND>0) THEN
  IPT=IPT+1
  ITHER=ITHER+1
  YDSP%SVD => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRB119
ELSE
  YDSP%SVD => SPECTRAL_DUMMY3D
ENDIF

IND=INTLOC(KGRIB,NGRBNHX)
IF (IND>0) THEN
  IPT=IPT+1
  ITHER=ITHER+1
  YDSP%NHX => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBNHX
ELSE
  YDSP%NHX => SPECTRAL_DUMMY3D
ENDIF

IF (ITHER>0) THEN
  YDSP%HV => YDSP%SP3D(:,:,IPT-ITHER+1:IPT)
ELSE
  YDSP%HV => SPECTRAL_DUMMY4D
ENDIF

IGFL=0
IND=INTLOC(KGRIB,NGRBQ)
IF (IND>0) THEN
  IPT=IPT+1
  IGFL=IGFL+1
  YDSP%Q => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBQ
ELSE
  YDSP%Q => SPECTRAL_DUMMY3D
ENDIF

IND=INTLOC(KGRIB,NGRBO3)
IF (IND>0) THEN
  IPT=IPT+1
  IGFL=IGFL+1
  YDSP%O3 => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBO3
ELSE
  YDSP%O3 => SPECTRAL_DUMMY3D
ENDIF

IND=INTLOC(KGRIB,NGRBCLWC)
IF (IND>0) THEN
  IPT=IPT+1
  IGFL=IGFL+1
  YDSP%L => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBCLWC
ELSE
  YDSP%L => SPECTRAL_DUMMY3D
ENDIF

IND=INTLOC(KGRIB,NGRBCIWC)
IF (IND>0) THEN
  IPT=IPT+1
  IGFL=IGFL+1
  YDSP%I => YDSP%SP3D(:,:,IPT)
  YDSP%NGRIB3(IPT)=NGRBCIWC
ENDIF

IF (IGFL>0) THEN
  YDSP%GFL => YDSP%SP3D(:,:,IPT-IGFL+1:IPT)
ELSE
  YDSP%GFL => SPECTRAL_DUMMY4D
ENDIF

! Other 3D fields without specific pointers
DO JJ=1,KS3D
  LLMISSING=.TRUE.
  DO JPT=1,IPT
    IF (YDSP%NGRIB3(JPT)==KGRIB(JJ)) LLMISSING=.FALSE.
  ENDDO
  IF (LLMISSING) THEN
    IF (IPT>=KS3D) CALL ABOR1('SPECTRAL_FIELDS_MOD:ALLOCATE_SPEC: Error 3D fields')
    IPT=IPT+1
    YDSP%NGRIB3(IPT)=KGRIB(JJ)
  ENDIF
ENDDO
IF (IPT/=KS3D) CALL ABOR1('SPECTRAL_FIELDS_MOD:ALLOCATE_SPEC: Error number 3D fields')

! Done with 3D fields, doing 2D fields

IPT=0
IND=INTLOC(KGRIB,NGRBLNSP)
IF (IND>0) THEN
  IPT=IPT+1
  YDSP%NGRIB2(IPT)=NGRBLNSP
  LLSET2D(IPT)=.TRUE.
  IF (SIZE(YDSP%SP2D,2)>0) THEN
    YDSP%SP => YDSP%SP2D(:,IPT)
  ENDIF
ENDIF
IF(.NOT.ASSOCIATED(YDSP%SP)) YDSP%SP => SPECTRAL_DUMMY2D

IND=INTLOC(KGRIB,NGRBZ)
IF (IND>0) THEN
  IPT=IPT+1
  YDSP%NGRIB2(IPT)=NGRBZ
  LLSET2D(IPT)=.TRUE.
  IF (SIZE(YDSP%SP2D,2)>0) THEN
    YDSP%OROG => YDSP%SP2D(:,IPT)
  ENDIF
ENDIF
IF(.NOT.ASSOCIATED(YDSP%OROG)) YDSP%OROG => SPECTRAL_DUMMY2D

! Other 2D fields without specific pointers
DO JJ=1,KS2D
  LLMISSING=.TRUE.
  DO JPT=1,IPT
    IF (YDSP%NGRIB2(JPT)==KGRIB(KS3D+JJ) .AND. LLSET2D(JJ)) LLMISSING=.FALSE.
  ENDDO
  IF (LLMISSING) THEN
    IF (IPT>=KS2D) CALL ABOR1('SPECTRAL_FIELDS_MOD:ALLOCATE_SPEC: Error 2D fields')
    IPT=IPT+1
    YDSP%NGRIB2(IPT)=KGRIB(KS3D+JJ)
    LLSET2D(JJ)=.TRUE.
  ENDIF
ENDDO
IF (IPT/=KS2D) CALL ABOR1('SPECTRAL_FIELDS_MOD:ALLOCATE_SPEC: Error number 2D fields')
...
ALLOCATE(YDSP%MYMS(KNUMP))
!ALLOCATE(YDSP%NALLMS(SIZE(YRMP%NALLMS)))
ALLOCATE(YDSP%NALLMS(SIZE(KALLMS)))
ALLOCATE(YDSP%NPTRMS(NPRTRW))
ALLOCATE(YDSP%NUMLL(NPRTRV))
ALLOCATE(YDSP%NPTRLL(NPRTRV))
ALLOCATE(YDSP%NPSURF(NPRTRV))
ALLOCATE(YDSP%MYLEVS(YDSP%NFLEVL))

YDSP%MYMS(:)=KMYMS(:)
!YDSP%NALLMS(:)=YRMP%NALLMS(:)
YDSP%NALLMS(:)=KALLMS(:)
YDSP%NPTRMS(:)=KPTRMS(:)
YDSP%NUMLL(:)=KNUMLL(:)
YDSP%NPTRLL(:)=KPTRLL(:)
...
ELSE
  ...
  CALL INI_SPEC_DIST(KSMAX,KSMAX,&
   & NPRTRW,MYSETW,KASM0=YDSP%NASM0,KUMPP=YDSP%NUMPP,&
   & KSPEC2=YDSP%NSPEC2)
ENDIF
...
ALLOCATE(YDSP%NGRIB(1))
ALLOCATE(YDSP%SP2D(1,1))
ALLOCATE(YDSP%SP3D(1,1,1))
IF (LELAM) ALLOCATE(YDSP%SP1D(1,1))
