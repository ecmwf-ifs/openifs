...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:MULPOL',0,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:MULPOL',1,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:IPOL',0,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:IPOL',1,ZHOOK_HANDLE)
END SUBROUTINE IPOL
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:EVPOL',0,ZHOOK_HANDLE)

ZEPS = 1000.0_JPRB * EPSILON(PX)
...
! 
!   DO JI=KORDER-1,1,-1
!     ZP(JI) = P(JI) + PX*ZP(JI+1)
!   ENDDO
! 
!   EVPOL = ZP(1)
! ELSE
! evaluation via synthetic division:
...
! EVPOL = remainder
EVPOL = PX*ZQ(K) + ZP(K+1)
! ENDIF

! this fix will be activated in CY40
! IF(ABS(EVPOL) < ZEPS)THEN
!   EVPOL = 0.0_JPRB
! ENDIF

IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:EVPOL',1,ZHOOK_HANDLE)
END FUNCTION EVPOL
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:DPOL',0,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:DPOL',1,ZHOOK_HANDLE)
END SUBROUTINE DPOL
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:GLOBAL2LOCAL',0,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:GLOBAL2LOCAL',1,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:SETTM',0,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:SETTM',1,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:SAMPLE_VALUE',0,ZHOOK_HANDLE)

SAMPLE_VALUE = REAL(KI-1,JPRB)/REAL(KN-1,JPRB)
SAMPLE_VALUE = FT2X(RTMIN, RTMAX, SAMPLE_VALUE)


IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:SAMPLE_VALUE',1,ZHOOK_HANDLE)
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:FX2T',0,ZHOOK_HANDLE)

FX2T = (PX - PXMIN) / (PXMAX - PXMIN)
! FX2T = (PX - 0.5_JPRB * (PXMAX + PXMIN)) / (PXMAX - PXMIN)

IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:FX2T',1,ZHOOK_HANDLE)
END FUNCTION FX2T
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:FT2X',0,ZHOOK_HANDLE)

FT2X = PXMIN + PT * (PXMAX - PXMIN)
! FT2X = 0.5_JPRB * (PXMAX + PXMIN) + PT * (PXMAX - PXMIN)

IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:FT2X',1,ZHOOK_HANDLE)
END FUNCTION FT2X
...
!----------------------------------
SUBROUTINE SETGP2VFE(LDAPPROX,KIN,KOUT,PETA,KTYPE,KORDER,KBASIS,PSPLINE,KOFF,KKNOT,PKNOT,PMTF)
...
REAL(KIND=JPRB)     :: ZMINDETA
REAL(KIND=JPHOOK)     :: ZHOOK_HANDLE

IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:SETGP2VFE',0,ZHOOK_HANDLE)
...
DO JR=1,KOUT
  ...
  IK = -1
  DO JI=1,KKNOT-1
    IF( ABS(PKNOT(JI+1)-PKNOT(JI)) >ZMINDETA )THEN
...
IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:SETGP2VFE',1,ZHOOK_HANDLE)
END SUBROUTINE SETGP2VFE

SUBROUTINE PRINT_MATRIX(CDTAG, KX, KY, PM)
...
REAL(KIND=JPRB), INTENT(IN) :: PM(KX,KY)

INTEGER(KIND=JPIM) :: II, IJ, IK
REAL(KIND=JPHOOK)  :: ZHOOK_HANDLE

IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:PRINT_MATRIX',0,ZHOOK_HANDLE)
...
CALL FLUSH(NULOUT)
DO II=1, MIN(IK,KX)
   WRITE(NULOUT,'(I5,100(1X,E14.8))') II,(PM(II,IJ),IJ=1, MIN(IK,KY))
   CALL FLUSH(NULOUT)
ENDDO

WRITE(NULOUT,*) TRIM(CDTAG), " RIGHT BOTTOM CORNER"
CALL FLUSH(NULOUT)
DO II=MAX(1,KX-IK+1),KX
   WRITE(NULOUT,'(I5,100(1X,E14.8))') II,(PM(II,IJ),IJ=MAX(1,KY-IK+1),KY)
   CALL FLUSH(NULOUT)
ENDDO

IF (LHOOK) CALL DR_HOOK('SUVFE_HLP:PRINT_MATRIX',1,ZHOOK_HANDLE)
