...
IF (ALLOCATED(FLD1%GFL) .AND. ALLOCATED(FLD2%GFL)) THEN
  IF (SIZE(FLD1%GFL,DIM=3) == SIZE(FLD2%GFL,DIM=3)) THEN
    DO JF=1,SIZE(FLD1%GFL,DIM=3)
      CALL DOTPROD3(YDGEOMETRY,JF,FLD1%GFL,FLD2%GFL,ZTMP)
      PPROD = PPROD + ZTMP
    ENDDO
  ELSE
    CALL ABOR1("YOMGFL:DOT_PROD_YOMGFL number of fields different")
  ENDIF
ELSE
  CALL ABOR1("YOMGFL:DOT_PROD_YOMGFL argument not allocated")
ENDIF
