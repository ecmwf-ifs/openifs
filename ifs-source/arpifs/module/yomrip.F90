...
#include "fcttim.func.h"
    ...
    IF (LHOOK) CALL DR_HOOK('YOMRIP:GET_YEAR',0,ZHOOK_HANDLE)

    IF (.NOT.LNF.AND.SELF%NSTADD == 0) THEN
      ! IN CASE OF RESTART:
      ITIMESTEP=NINT(SELF%TSTEP)
      IF (LDTWOTL) THEN
        IZT=NINT(SELF%TSTEP*(REAL(NSTAR2,JPRB)+0.5_JPRB))
      ELSE
    ...
    IF (LHOOK) CALL DR_HOOK('YOMRIP:GET_YEAR',1,ZHOOK_HANDLE)
