...
USE BYTES_IO_MOD, ONLY : BYTES_IO_OPEN, BYTES_IO_WRITE, BYTES_IO_READ, BYTES_IO_FLUSH
...
!  compatibility. Fortran pipes often not properly supported.
  ...
  ELSEIF((NOACOMM == 2).OR.(NOACOMM == 4)) THEN
    ...
    CALL BYTES_IO_OPEN(NCULMR,'Preadm01','w',IRET1)
    CALL BYTES_IO_OPEN(NCULMW,'Pwritm01','r',IRET2)
...
!        -------------------------------------
  ...
  IF((NOACOMM == 2).OR.(NOACOMM == 4)) THEN
    CALL BYTES_IO_WRITE(NCULMR,IMESS,4*NINTLEN,IRET)
    CALL BYTES_IO_FLUSH(NCULMR)
  ELSEIF(NOACOMM == 3) THEN
...
!        ---------------------------------------
  ...
  IF(NOACOMM == 2) THEN
    CALL BYTES_IO_READ(NCULMW,IMESS,4*NINTLEN,IRET)
  ELSEIF(NOACOMM == 3) THEN
...
!  the OASIS namelist input.
  ...
  IF((NOACOMM == 2).OR.(NOACOMM == 4)) THEN
    IF(LMCCICEIC) THEN
      CALL BYTES_IO_OPEN(YDCOU%NCULF(0),'IICEATMO','w',IRET)
    ENDIF
    CALL BYTES_IO_OPEN(YDCOU%NCULF(1),'SSTATMOS','r',IRET)
    CALL BYTES_IO_OPEN(YDCOU%NCULF(2),'ICEATMOS','r',IRET)
    CALL BYTES_IO_OPEN(YDCOU%NCULF(3),'QNET_ATM','w',IRET)
    CALL BYTES_IO_OPEN(YDCOU%NCULF(4),'QSOL_ATM','w',IRET)
    CALL BYTES_IO_OPEN(YDCOU%NCULF(5),'TAUX_ATM','w',IRET)
    CALL BYTES_IO_OPEN(YDCOU%NCULF(6),'TAUY_ATM','w',IRET)
    CALL BYTES_IO_OPEN(YDCOU%NCULF(7),'PMEV_ATM','w',IRET)
    IF(LECURR) THEN
      CALL BYTES_IO_OPEN(YDCOU%NCULF(8),'UV_ATMOS','r',IRET)
      CALL BYTES_IO_OPEN(YDCOU%NCULF(9),'VV_ATMOS','r',IRET)
    ENDIF
  ELSEIF(NOACOMM == 3) THEN
    IF(LMCCICEIC) THEN
