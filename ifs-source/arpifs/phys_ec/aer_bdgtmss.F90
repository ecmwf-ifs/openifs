...
SUBROUTINE AER_BDGTMSS &
 & ( YDEAERATM,YDML_PHY_AER,YDCOMPO,YGFL,KIDIA  , KFDIA   , KLON   , KLEV, KACTAERO, KWHAT, KNWAVL, KTWAVL, KSTART, KSTEP, KLEVTROP, &
 &   PAPH   , PAEROK  , PCAERO , PDZ     , PO3     , PQ      , PRHCL   , PRHO    , PT , PTH, PRSF1, &
 &   PODSS   , PODDU  , PODOM   , PODBC   , PODSU   , PODNI, PODAM, PODSOA, PODVFA  , PODVSU  , &
 &   PAEPM1 , PAEPM25 , PAEPM10, &
 &   PAERMSS, PAERMSST, PAERTAU, PAERTAUT, PAERTAUS, PAERTAUS2, PAERTAUI, PAERTAUB, PAERTAUA, PAERTAUF, PAERAOT, &
...
!     SRémy       : 20221125  Add stratospheric AOD
...
REAL(KIND=JPRB)   ,INTENT(IN)  :: PAPH(KLON,0:KLEV)         , PT(KLON,KLEV)               
...
REAL(KIND=JPRB)   ,INTENT(OUT) :: PAERTAUS(KLON,20), PAERTAUS2(KLON,20)
REAL(KIND=JPRB)   ,INTENT(OUT) :: PAERTAUA(KLON,20), PAERTAUF(KLON,20), PAERTAUB(KLON,KACTAERO,20)
REAL(KIND=JPRB)   ,INTENT(OUT) :: PSIGAER(KLON,6) ! Aerosol extinction at 550 nm, in km-1
...
REAL(KIND=JPRB) :: ZFAC, ZVFRAC, ZRHOP40, ZRATIOMASS
...
ASSOCIATE(LAERLISI=>YGFL%LAERLISI, LAERVOL=>YDEAERATM%LAERVOL, &
 ...
 & RHO_WAT=>YDML_PHY_AER%YREAERSNK%RHO_WAT, &
...
!-- use dry dust for 0.9-20 um bin
...
!-- end computation surface extinction coefficients
  ...
  DO JWAVL=1,KNWAVL
    ...
    PAERTAUS(KIDIA:KFDIA,JWAVL)=0._JPRB
    PAERTAUS2(KIDIA:KFDIA,JWAVL)=0._JPRB
...
!- For SS, vary depending on RH
          ...
          IF (JK < KLEVTROP(JL)) THEN
            PAERTAUS(JL,JWAVL) = PAERTAUS(JL,JWAVL)+PAERTAU(JL,JK,JAER)
            IF (ITYP == 5 .OR. ITYP == 10) THEN
              PAERTAUS2(JL,JWAVL) = PAERTAUS2(JL,JWAVL)+PAERTAU(JL,JK,JAER)
            ENDIF
          ENDIF
...
!-- total optical thickness: as sum over all individual aerosol optical thicknesses
      ...
      ENDDO
!-- Fine mode AOD : bin1 of SS, DU and NI; OM1 and 2, BC1 and 2, SOA1 and 1, SU
!-- Coarse mode AOD : bin2 and 3 of SS, bin 2 of NI
      IF ((IBIN == 1 .AND. ITYP ==1) .OR. (ITYP ==2 .AND. IBIN ==1) .OR. (ITYP==6 .AND. IBIN==1) .OR. ITYP==3 .OR. ITYP==4 .OR. ITYP==5 .OR. ITYP >= 7 ) THEN
        DO JL=KIDIA,KFDIA
...
! Correction factor to account for the fact that obs are usually taken at
! standard atmosphere pressure and temperature

  JAER=0

! Compute PM at 0% RH (dry PM)
  JRH=1
  ...
  IF (NTYPAER(1) == 3) THEN ! Sea-salt (3-bin)
    ...
    PAEPM25(JL) = PAEPM25(JL) + (1.00_JPRB*PAEROK(JL,KLEV,JAER+1) + 0.60_JPRB*PAEROK(JL,KLEV,JAER+2))*RSS_DRY_MASSFAC
...
IF (NTYPAER(2) == 3) THEN ! Desert dust (3-bin)
    ...
    ELSE
      PAEPM1(JL) = PAEPM1(JL) + 0.86_JPRB*PAEROK(JL,KLEV,JAER+1)
      PAEPM25(JL) = PAEPM25(JL) + 1.00_JPRB*(PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2)) + 0.025_JPRB*PAEROK(JL,KLEV,JAER+3)
      PAEPM10(JL) = PAEPM10(JL) + 1.00_JPRB*(PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2)) + 0.58_JPRB*PAEROK(JL,KLEV,JAER+3)
    ENDIF
  ELSE
    PODDU(JL) = 0._JPRB
  ENDIF
  ...
  IF (NTYPAER(3) == 2) THEN ! Organic matter (hydrophilic & hydrophobic)
    ZVFRAC = 1.0_JPRB / ROMGROWTH_RHTAB(JRH)**3
    ZRHOP40 = RHO_WAT*(1.0_JPRB-ZVFRAC) + ZVFRAC*RRHO_OM
    ZRATIOMASS = ZRHOP40/RRHO_OM * (ROMGROWTH_RHTAB(JRH)/ROMGROWTH_RHTAB(1))**3._JPRB
    PODOM(JL) = PAERTAUT(JL,JAER+1,1)+PAERTAUT(JL,JAER+2,1)
    PAEPM1(JL) = PAEPM1(JL) + 0.96_JPRB*(PAEROK(JL,KLEV,JAER+1)*ZRATIOMASS+PAEROK(JL,KLEV,JAER+2))
    PAEPM25(JL) = PAEPM25(JL) + PAEROK(JL,KLEV,JAER+1)*ZRATIOMASS+PAEROK(JL,KLEV,JAER+2)
    PAEPM10(JL) = PAEPM10(JL) + PAEROK(JL,KLEV,JAER+1)*ZRATIOMASS+PAEROK(JL,KLEV,JAER+2)
  ELSE
    PODOM(JL) = 0._JPRB
  ENDIF
  JAER = JAER + NTYPAER(3)
  ...
  IF (NTYPAER(4) == 2) THEN ! Black carbon (hydrophilic & hydrophobic)
    ...
    PAEPM1(JL) = PAEPM1(JL) + 0.96_JPRB*(PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2))
    PAEPM25(JL) = PAEPM25(JL) + PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2)
    PAEPM10(JL) = PAEPM10(JL) + PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2)
  ELSE
    PODBC(JL) = 0._JPRB
  ENDIF
  JAER = JAER + NTYPAER(4)
  ...
  IF (NTYPAER(5) == 1 .OR. NTYPAER(5) == 2) THEN ! Sulphate (SO4 and possibly SO2 precursor, ignored)
    ZVFRAC = 1.0_JPRB / RSO4GROWTH_RHTAB(JRH)**3
    ZRHOP40 = RHO_WAT*(1.0_JPRB-ZVFRAC) + ZVFRAC*RRHO_SO4
    ZRATIOMASS = ZRHOP40/RRHO_SO4 * (RSO4GROWTH_RHTAB(JRH)/RSO4GROWTH_RHTAB(1))**3._JPRB
    PODSU(JL) = PAERTAUT(JL,JAER+1,1)
    PAEPM1(JL) = PAEPM1(JL) + 0.91_JPRB*PAEROK(JL,KLEV,JAER+1)*ZRATIOMASS
    PAEPM25(JL) = PAEPM25(JL) + PAEROK(JL,KLEV,JAER+1)*ZRATIOMASS
    PAEPM10(JL) = PAEPM10(JL) + PAEROK(JL,KLEV,JAER+1)*ZRATIOMASS
  ELSE
    PODSU(JL) = 0._JPRB
  ENDIF
  JAER = JAER + NTYPAER(5)
  ...
  IF (NTYPAER(6) == 2) THEN ! Nitrate (fine and coarse)
    ZVFRAC = 1.0_JPRB / RNIGROWTH_RHTAB(JRH)**3
    ZRHOP40 = RHO_WAT*(1.0_JPRB-ZVFRAC) + ZVFRAC*RRHO_NI(1)
    ZRATIOMASS = ZRHOP40/RRHO_NI(1) * (RNIGROWTH_RHTAB(JRH)/RNIGROWTH_RHTAB(1))**3._JPRB
    PODNI(JL) = PAERTAUT(JL,JAER+1,1)+PAERTAUT(JL,JAER+2,1)
    PAEPM1(JL) = PAEPM1(JL) + 0.91_JPRB*PAEROK(JL,KLEV,JAER+1)*ZRATIOMASS
    PAEPM25(JL) = PAEPM25(JL) + (PAEROK(JL,KLEV,JAER+1) + 0.5_JPRB*PAEROK(JL,KLEV,JAER+2))*ZRATIOMASS
    PAEPM10(JL) = PAEPM10(JL) + (PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2))*ZRATIOMASS
  ELSE
    PODNI(JL) = 0._JPRB
  ENDIF
  JAER = JAER + NTYPAER(6)
  ...
  IF (NTYPAER(7) == 1) THEN ! Ammonium (one bin)
    ZVFRAC = 1.0_JPRB / RAMGROWTH_RHTAB(JRH)**3
    ZRHOP40 = RHO_WAT*(1.0_JPRB-ZVFRAC) + ZVFRAC*RRHO_AM
    ZRATIOMASS = ZRHOP40/RRHO_AM * (RAMGROWTH_RHTAB(JRH)/RAMGROWTH_RHTAB(1))**3._JPRB
  ...
  IF (NTYPAER(8) >=2) THEN ! Secondary organics (2 bins)
    ZVFRAC = 1.0_JPRB / RSOAGROWTH_RHTAB(JRH)**3
    ZRHOP40 = RHO_WAT*(1.0_JPRB-ZVFRAC) + ZVFRAC*RRHO_SOA
    ZRATIOMASS = ZRHOP40/RRHO_SOA * (RSOAGROWTH_RHTAB(JRH)/RSOAGROWTH_RHTAB(1))**3._JPRB
    PODSOA(JL) = PAERTAUT(JL,JAER+1,1)+PAERTAUT(JL,JAER+2,1)
    PAEPM1(JL) = PAEPM1(JL) + 0.96_JPRB*(PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2))*ZRATIOMASS
    PAEPM25(JL) = PAEPM25(JL) + (PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2))*ZRATIOMASS
    PAEPM10(JL) = PAEPM10(JL) + (PAEROK(JL,KLEV,JAER+1)+PAEROK(JL,KLEV,JAER+2))*ZRATIOMASS
  ENDIF
  JAER = JAER + NTYPAER(8)
