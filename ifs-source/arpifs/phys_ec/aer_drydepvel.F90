...
IF (LHOOK) CALL DR_HOOK('AER_DRYDEPVEL',0,ZHOOK_HANDLE)
...
ZEIM_AV_3=(ZSN_AV_3/(ZALPHA(KVEG_ZH)+ZSN_AV_3))**2.0_JPRB

!       -- For Interception
IF (KVEG_ZH==8 .OR. KVEG_ZH==9 .OR. KVEG_ZH==12 .OR. KVEG_ZH==13 .OR. KVEG_ZH==14 ) THEN
   ZEIN=0.0_JPRB
ELSE
   ZEIN=0.5_JPRB*(PWETD/ZCR(KVEG_ZH,KSEASON_WE))**2._JPRB
ENDIF
...
ZREBOUND=1._JPRB
IF (PRHO > 2300._JPRB .AND. PUSTR > 0.335_JPRB .AND. PWETD >=2.5E-6_JPRB .AND. KVEG_ZH < 13) THEN
