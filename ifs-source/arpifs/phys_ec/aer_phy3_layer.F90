...
INTEGER(KIND=JPIM) :: IKCLOUDPH, IKRAINPH
...
ASSOCIATE(TSPHY=>YDPHY2%TSPHY, &
 ...
 & YO3=>YGFL%YO3, NGFL_EXT=>YGFL%NGFL_EXT, YEXT=>YGFL%YEXT, &
...
!*         1.     UNROLL THE DERIVED STRUCTURES AND CALL AER_PHY3
    ...
    IF (YDCOMPO%LCHEM_EQSAM4CLIMPH) THEN
      ! Identify cloud pH field
      IKCLOUDPH=-999_JPIM
      DO JVAR=1,NGFL_EXT
        IF ( YEXT(JVAR)%CNAME == 'CLOUDPH') IKCLOUDPH  = JVAR
      ENDDO
      IF ( IKCLOUDPH <= 0  ) THEN
        CALL ABOR1("CLOUD PH FIELD IS NOT DEFINED")
      ENDIF
      ! Identify rain pH field
      IKRAINPH=-999_JPIM
      DO JVAR=1,NGFL_EXT
        IF ( YEXT(JVAR)%CNAME == 'RAINPH') IKRAINPH  = JVAR
      ENDDO
      IF ( IKRAINPH <= 0  ) THEN
        CALL ABOR1("RAIN PH FIELD IS NOT DEFINED")
      ENDIF
      ! Update cloud and rain pH field produced by EQSAM4CLIM
      PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKCLOUDPH)%MP9_PH)=GEMSL%ZCLOUDPH(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)
      PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKRAINPH)%MP9_PH) =GEMSL%ZRAINPH(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)
    ENDIF
