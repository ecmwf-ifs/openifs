...
SUBROUTINE CALLPAR(YDGEOMETRY,YDVARS,YDSURF,YDMODEL,YDTHF,YDCST,KDIM,&
 ...
 ! Stored quantities
 & PSAVTEND, PGFLSLP,&
 & STATE_T0, TENDENCY_DYN, TENDENCY_CML, STATE_TMP, &
...
!        J. McNorton  24-Aug-2022 : Add Urban Tile
!        R. Hogan     25-Nov-2022 : Removed CLIMAER_LAYER: visibility now computed only from RADVIS_LAYER
!     
!-----------------------------------------------------------------------
...
!     ------------------------------------------------------------------
LOGICAL :: LL_DIAGCLOUDAER, LLISPPT, LL_HRES, LLBUD23
LOGICAL :: LLCLDDIAG=.true.
LOGICAL :: LLDIAG=.true.  ! fully explored in callpar_new
...
REAL(KIND=JPRB)     :: ZTENDENCY_VD9(KDIM%KLON,KDIM%KLEV, &
  &                    YDMODEL%YRML_PHY_G%YRSLPHY%NVTEND_VD)
...
ASSOCIATE(YDSTA=>YDGEOMETRY%YRSTA, YDVAB=>YDGEOMETRY%YRVAB, YDEPHLI=>YDMODEL%YRML_PHY_SLIN%YREPHLI, &
 ...
 & YDSPPT=>YDMODEL%YRML_SPPT, YDSPPT_CONFIG=>YDMODEL%YRML_GCONF%YRSPPT_CONFIG, YDSPP_CONFIG=>YDMODEL%YRML_GCONF%YRSPP_CONFIG, &
 & YSD_VN=>YDSURF%YSD_VN, YSD_VD=>YDSURF%YSD_VD, YSP_RR=>YDSURF%YSP_RR, YSD_VF=>YDSURF%YSD_VF,&
 & YSP_SG=>YDSURF%YSP_SG)

ASSOCIATE(NACTAERO=>YGFL%NACTAERO, NAERO=>YGFL%NAERO, NCHEM=>YGFL%NCHEM, LAERCHEM=>YGFL%LAERCHEM, &
 & LAERNITRATE=>YDMODEL%YRML_CHEM%YRCOMPO%LAERNITRATE, NGHG=>YGFL%NGHG,  &
 & NGEMS=>YGFL%NGEMS, NGFL_EXT=>YGFL%NGFL_EXT, YA=>YGFL%YA, YTKE=>YGFL%YTKE, YEXT=>YGFL%YEXT, YQ=>YGFL%YQ, YI=>YGFL%YI, &
 ...
 & LEMETHOX=>YDEPHY%LEMETHOX, LEO3CH=>YDEPHY%LEO3CH, LEPCLD=>YDEPHY%LEPCLD, LEQNGT=>YDEPHY%LEQNGT, &
 & LERADI=>YDEPHY%LERADI, LERADS=>YDEPHY%LERADS, LSLPHY=>YDEPHY%LSLPHY, LESURF=>YDEPHY%LESURF, LEVDIF=>YDEPHY%LEVDIF, &
 ...
 & LNCLIN=>YDNCL%LNCLIN, LENCLD2=>YDPHNC%LENCLD2, LEPCLD2=>YDPHNC%LEPCLD2, NSTART=>YDRIP%NSTART, &
 ...
 & LPHYSFCLIN=>YDEPHLI%LPHYSFCLIN, LERADIMPL=>YDEPHY%LERADIMPL, &
 & LVEXTRDYNACC=>YDEPHY%LVEXTRDYNACC, LEXTRATEND=>YDEPHY%LEXTRATEND, &
 & RG=>YDCST%RG, RLVTT=>YDCST%RLVTT, RLSTT=>YDCST%RLSTT, RTT=>YDCST%RTT, RCPD=>YDCST%RCPD, &
 & RVTMP2=>YDTHF%RVTMP2, &
 & NSITER=>YDDYN%NSITER,NCURRENT_ITER=>YDDYN%NCURRENT_ITER, &
 & RHOUR=>YDCST%RHOUR, &
 & LCLDBUD_TIMEINT=>YDECLDP%LCLDBUD_TIMEINT, LAERSOA=>YDCOMPO%LAERSOA, &
...
LLKEYS%LLSLPHY = LSLPHY.AND.YDDYNA%LSLAG
LLKEYS%LLRAIN1D = .FALSE. ! key reserved for 1D var rain
...
ELSE
  LLISPPT=.FALSE.
ENDIF
LLDIAG=(.NOT.YDDYNA%LPC_FULL) .OR. (NCURRENT_ITER == NSITER) ! Final call of physics in a given timestep
...
! if prognostic O3 not active, initialise TENDENCY_CML
IF (.NOT. LEO3CH) TENDENCY_CML%O3=0.0_JPRB

! if prognostic TKE not active, initialise TENDENCY_CML
IF (.NOT. YTKE%LACTIVE) TENDENCY_CML%TKE=0.0_JPRB
...
! Set the values of VD9 tendencies (to be used in convection)
IF (LECUMF) ZTENDENCY_VD9(:,:,:)=0._JPRB
...
IF (LPHYLIN) THEN
  ! Parallel physics - doing just copy of state_t0 to state_tmp
...
IF ((.NOT.LLKEYS%LLSLPHY).AND.(YL%LACTIVE).AND.(YI%LACTIVE)) THEN
...
DO JL=KDIM%KIDIA,KDIM%KFDIA
  ...
  IF (PSURF%PSD_VF(JL,YSD_VF%YLSM%MP) > 0.5_JPRB) THEN
    IF (LLPERT_HSDT) THEN
      ...
      !
      IF(PSURF%PSD_VF(JL,YSD_VF%YGETRL%MP) < (1000.0_JPRB * RG)) THEN
        ZFACT=EXP( PN1%MU(1) +PN1%XMAG(1)*PPERT%PGP2DSPP(JL,1,IMP))
        ZFACT=MIN(ZFACT,PN1%XMAX(1))
        ZFACT=MAX(ZFACT,PN1%XMIN(1))
      ELSE
        ZFACT=EXP( PN1%MU(2) +PN1%XMAG(2)*PPERT%PGP2DSPP(JL,1,IMP))
        ZFACT=MIN(ZFACT,PN1%XMAX(2))
        ZFACT=MAX(ZFACT,PN1%XMIN(2))
      ENDIF
      PSURF%PHSTD (JL) = PSURF%PSD_VF(JL,YSD_VF%YGETRL%MP) * ZFACT/RG
    ELSE
      !
...
!*         2.2 SURFACE BOUNDARY CONDITIONS
  ...
  IFLAG=2
  CALL SATUR (YDTHF, YDCST, KDIM%KIDIA, KDIM%KFDIA, KDIM%KLON, KDIM%KTDIA, KDIM%KLEV, YDMODEL%YRML_PHY_SLIN%YREPHLI%LPHYLIN, &
  ...
  ! We used to calculate visibility from climatological aerosol here
  ! (CLIMAER_LAYER), but this is now replaced by the call to
  ! RADVIS_LAYER below
...
IF (LUSEKF_REF.AND.LLDIAG) THEN
! save net radiation at the surface from the unperturbed first sEKF model run
  IF (N_SEKF_PT==0) THEN
    IBLK1=(KDIM%KSTGLO-1)/KDIM%KLON + 1
    DO JL=KDIM%KIDIA,KDIM%KFDIA
      FKF_SURF_SO(JL,IBLK1,NSTEP) = FLUX%PFRSO(JL,KDIM%KLEV)
      FKF_SURF_TH(JL,IBLK1,NSTEP) = FLUX%PFRTH(JL,KDIM%KLEV)
    ENDDO
  ENDIF
! replace radiation with the unperturbed first sEKF values
  IF (N_SEKF_PT>0) THEN
    IBLK1=(KDIM%KSTGLO-1)/KDIM%KLON + 1
    DO JL=KDIM%KIDIA,KDIM%KFDIA
      FLUX%PFRSO(JL,KDIM%KLEV) = FKF_SURF_SO(JL,IBLK1,NSTEP)
      FLUX%PFRTH(JL,KDIM%KLEV) = FKF_SURF_TH(JL,IBLK1,NSTEP)
    ENDDO
  ENDIF
ENDIF
...
IF ( LEGWDG ) THEN
  ...
  CALL GWDRAG_LAYER(YDMODEL,YDSURF,KDIM,PAUX,STATE_T0,TENDENCY_LOC,PSURF,AUXL)
ELSE
...
IF ( LEVDIF ) THEN
  ...
  IF (LPHYLIN) THEN
    ! Simplified scheme
    CALL TURBULENCE_S_LAYER(YDSURF,YDMODEL,KDIM,PAUX,STATE_T0,TENDENCY_CML,AUXL,GEMSL,PSURF,&
      & PCGPP, PCREC, PAG, PRECO, SURFL,FLUX,PDIAG,TENDENCY_LOC)
  ELSE
    ! Full scheme
    CALL TURBULENCE_LAYER(YDSURF,YDMODEL,KDIM, PAUX, STATE_T0, TENDENCY_CML, PRAD, PPERT, LLDIAG,&
...
IF (LECUMF) THEN
  ...
  IF (.NOT.LECUMFS) THEN
    ...
    CALL CONVECTION_LAYER(YDSURF, YDMODEL, KDIM, STATE_T0, TENDENCY_CML, &
     &      TENDENCY_DYN, ZTENDENCY_VD9, PAUX, PPERT,&
     &      LLKEYS, PDIAG, AUXL, PERTL, FLUX, PSURF, GEMSL, TENDENCY_LOC)
  ELSE
    ! SIMPLIFIED MASS FLUX CONVECTION SCHEME (P. Lopez)
    CALL CONVECTION_S_LAYER(YDSURF, YDMODEL, KDIM, STATE_T0, TENDENCY_CML, ZTENDENCY_VD9, PAUX, &
...
ELSEIF( (LENCLD2 .OR. LEPCLD2) .AND. (.NOT. LEPCLD) ) THEN
  ...
  CALL CLOUD_S_LAYER(YDMODEL, KDIM, LLKEYS, PAUX, STATE_T0, TENDENCY_CML,&
...
ELSEIF( LEPCLD ) THEN
  ...
  CALL CLOUD_LAYER(YDSURF,YDECLDP,YDECUMF,YDEPHLI,YDPHY2,YDERAD,YDEPHY,YDVDF,YDSPP_CONFIG,YGFL, &
   &               KDIM,LLDIAG, PAUX, PPERT, STATE_T0, &
...
IF ( LEGWWMS ) THEN
  ...
  IF (LDIAG_STRATO.AND.LLDIAG) CALL UPDATE_FIELDS(YDPHY2, 1, KDIM%KIDIA, KDIM%KFDIA, KDIM%KLON, KDIM%KLEV,&
...
IF(LDIAGTURB_EC.AND.LLDIAG) THEN
 !IF(MOD(NSTEP*TSPHY,RHOUR)==0.0_JPRB.OR.NSTEP==NSTART) THEN
...
! compute and save tendency increments from the unperturbed first sEKF model run
  IF (N_SEKF_PT==0) THEN
    ...
    CALL UPDATE_FIELDS(YDPHY2, 2,KDIM%KIDIA,KDIM%KFDIA,KDIM%KLON,KDIM%KLEV,&
      & PI1=TENDENCY_CML%T, PS1=ZT_SAVE_KF, PO1=FKF_TENT(:,:,IBLK1,NSTEP),&
      & PI2=TENDENCY_CML%Q, PS2=ZQ_SAVE_KF, PO2=FKF_TENQ(:,:,IBLK1,NSTEP),&
      & PI3=TENDENCY_CML%U, PS3=ZU_SAVE_KF, PO3=FKF_TENU(:,:,IBLK1,NSTEP),&
      & PI4=TENDENCY_CML%V, PS4=ZV_SAVE_KF, PO4=FKF_TENV(:,:,IBLK1,NSTEP))
  ENDIF
! replace tendency increments with the unperturbed first sEKF increments
  IF (N_SEKF_PT>0) THEN
    IBLK1=(KDIM%KSTGLO-1)/KDIM%KLON + 1
    CALL UPDATE_FIELDS(YDPHY2,2,KDIM%KIDIA,KDIM%KFDIA,KDIM%KLON,KDIM%KLEV,&
      & PI1=FKF_TENT(:,:,IBLK1,NSTEP), PA1=ZT_SAVE_KF, PO1=TENDENCY_CML%T,&
      & PI2=FKF_TENQ(:,:,IBLK1,NSTEP), PA2=ZQ_SAVE_KF, PO2=TENDENCY_CML%Q,&
      & PI3=FKF_TENU(:,:,IBLK1,NSTEP), PA3=ZU_SAVE_KF, PO3=TENDENCY_CML%U,&
      & PI4=FKF_TENV(:,:,IBLK1,NSTEP), PA4=ZV_SAVE_KF, PO4=TENDENCY_CML%V)
  ENDIF
ENDIF
!     ------------------------------------------------------------------
...
IF (LLDIAG) CALL VDFVINT(KDIM%KIDIA,KDIM%KFDIA,KDIM%KLON,KDIM%KLEV,&
...
IF (LAERVISI) THEN
  CALL RADVIS_LAYER(YDMODEL,YDERAD,YGFL,KDIM,PAUX,STATE_TMP,GEMSL,PDIAG)
ENDIF

!     -----------------------------------------------------------------

!*        10.0   Use precipitation from the refernce run (for the sEKF)
...
! compute and save tendency increments from the unperturbed first sEKF model run
  IF (N_SEKF_PT==0) THEN
    IBLK1=(KDIM%KSTGLO-1)/KDIM%KLON + 1
    DO JL=KDIM%KIDIA,KDIM%KFDIA
      FKF_SURF_CR(JL,IBLK1,NSTEP) = FLUX%PFPLCL(JL,KDIM%KLEV)
      FKF_SURF_LR(JL,IBLK1,NSTEP) = FLUX%PFPLSL(JL,KDIM%KLEV)
    ENDDO
  ENDIF
  IF (N_SEKF_PT>0) THEN
    IBLK1=(KDIM%KSTGLO-1)/KDIM%KLON + 1
    DO JL=KDIM%KIDIA,KDIM%KFDIA
      FLUX%PFPLCL(JL,KDIM%KLEV) = FKF_SURF_CR(JL,IBLK1,NSTEP)
      FLUX%PFPLSL(JL,KDIM%KLEV) = FKF_SURF_LR(JL,IBLK1,NSTEP)
    ENDDO
  ENDIF
ENDIF
...
IF (LNEMOATMFLDS.AND.LLDIAG) CALL NEMOADDFLDS_LAYER(YDSURF,YDMCC,KDIM, SURFL, PSURF)
IF ((LNEMOLIMPUT.OR.CPL_NEMO_LIM).AND.LLDIAG) CALL SET_OCEAN_FLUXES(YDSURF,YDMCC,KDIM,SURFL,PSURF,FLUX)
...
IF(YDEPHY%LEMWAVE.AND.LLDIAG) THEN
  DO JK=1,KDIM%KLEV
   DO JL = KDIM%KIDIA, KDIM%KFDIA
