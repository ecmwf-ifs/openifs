...
!**** *CCLOUD*  - COMPUTES CLOUD COVER AND LIQUID WATER PATH
...
!      A.Mary     2017/05/03 rename CLOUD into CCLOUD to avoid conflict in radiation_interface
...
IF (LHOOK) CALL DR_HOOK('CCLOUD',0,ZHOOK_HANDLE)
ASSOCIATE(CETA=>YDECLD%CETA, LOMEGA=>YDECLD%LOMEGA, RANVA=>YDECLD%RANVA, &
 ...
 & RCPD=>YDCST%RCPD, RLVTT=>YDCST%RLVTT, RLSTT=>YDCST%RLSTT, RTT=>YDCST%RTT, RD=>YDCST%RD, &
...
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('CCLOUD',1,ZHOOK_HANDLE)
