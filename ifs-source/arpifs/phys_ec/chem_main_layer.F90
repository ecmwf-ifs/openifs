...
!-----------------------------------------------------------------------
INTEGER(KIND=JPIM)           ::  JVAR, JL, JT, JK,  IDDFLXA, IWDFLXA, IDDVEL, IKCH4CHEM, IKCO2CHEM, JEXTR_DV
...
INTEGER(KIND=JPIM) :: IKCLOUDPH, IKRAINPH, IKCLOUDPHUPD, IKRAINPHUPD
...
ASSOCIATE(YGFL=>YDMODEL%YRML_GCONF%YGFL, YDPHY2=>YDMODEL%YRML_PHY_MF%YRPHY2, &
 & YDCHEM=>YDMODEL%YRML_CHEM%YRCHEM, YDCOMPO=>YDMODEL%YRML_CHEM%YRCOMPO,YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY)
...
IF (NCHEM > 0 ) THEN  
  ...
  IF (YDCOMPO%LCHEM_EQSAM4CLIMPH) THEN
    ...
    IKCLOUDPH=-999_JPIM
    IKCLOUDPHUPD=-999_JPIM
    DO JVAR=1,NGFL_EXT
      IF ( YEXT(JVAR)%CNAME == 'CLOUDPH') IKCLOUDPH  = JVAR
      IF ( YEXT(JVAR)%CNAME == 'CLOUDPHUPD') IKCLOUDPHUPD  = JVAR
    ENDDO
    IF ( IKCLOUDPH > 0  ) THEN
      ! pH values from EQSAM4Clim (from the previous time step)
      GEMSL%ZCLOUDPH(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)=PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKCLOUDPH)%MP9_PH)
      ! Initialize the future updated pH values with values from EQSAM4Clim
      GEMSL%ZCHEMPH(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)=PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKCLOUDPH)%MP9_PH)
    ELSE
    ...
    IKRAINPH=-999_JPIM
    IKRAINPHUPD=-999_JPIM
    DO JVAR=1,NGFL_EXT
      IF ( YEXT(JVAR)%CNAME == 'RAINPH') IKRAINPH  = JVAR
      IF ( YEXT(JVAR)%CNAME == 'RAINPHUPD') IKRAINPHUPD  = JVAR
    ENDDO
    IF ( IKRAINPH > 0  ) THEN
      ! pH values from EQSAM4Clim (from the previous time step)
      GEMSL%ZRAINPH(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)=PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKRAINPH)%MP9_PH)
      ! Initialize the future updated pH values with values from EQSAM4Clim
      GEMSL%ZCHEMPHR(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)=PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKRAINPH)%MP9_PH)
    ELSE
  ...
  CALL CHEM_MAIN &
    ...
    &  FLUX%PFPLCL, FLUX%PFPLCN, FLUX%PFPLSL, FLUX%PFPLSN, PDIAG%PCOVPTOT,PDIAG%ZLU  ,& 
    ...
    &  GEMSL%ZAEROP(:,:,1:NACTAERO),  GEMSL%ZCLOUDPH,GEMSL%ZRAINPH, GEMSL%ZCHEMPH,GEMSL%ZCHEMPHR, &
    &  GEMSL%ZWETDIAM, GEMSL%ZWETVOL, GEMSL%ZND, GEMSL%ZAERAOTLEV, GEMSL%ZAERAAOTLEV, GEMSL%ZAERASYLEV, &
    &  PTENGFL, GEMSL%ZCEN, GEMSL%ZTENC, PSURF%PSD_VD(:,YSD_VD%YINEE%MP), PSURF%PSD_VD(:,YSD_VD%YICH4%MP), &
    &  PCHEM2AER,ZCHEM2GHG, IKLEVTROP )
  IF (YDCOMPO%LCHEM_EQSAM4CLIMPH) THEN
      ! Update cloud and rain pH field (values updated by tm5_wetchem)
      PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKCLOUDPHUPD)%MP9_PH)=GEMSL%ZCHEMPH(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)
      PGFL(KDIM%KIDIA:KDIM%KFDIA, 1:KDIM%KLEV,YEXT(IKRAINPHUPD)%MP9_PH) =GEMSL%ZCHEMPHR(KDIM%KIDIA:KDIM%KFDIA,1:KDIM%KLEV)
    ENDIF
...
IF ( NGHG > 0 )  THEN
  ...
  &  GEMSL%ZCFLX, YDEPHY,  PSURF%PSD_VD(:,YSD_VD%YINEE%MP), PSURF%PSD_VD(:,YSD_VD%YICH4%MP), PAUX%PGELAT, PGFL, PTENGFL, PSURF%PSD_XA,  GEMSL%ZCEN, GEMSL%ZTENC )
ENDIF 
