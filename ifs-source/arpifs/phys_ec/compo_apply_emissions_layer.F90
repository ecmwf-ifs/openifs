...
INTEGER(KIND=JPIM) :: INEMIS2D
INTEGER(KIND=JPIM) :: INEMIS2DAUX
INTEGER(KIND=JPIM) :: INEMIS3D
...
! Prevent access below lower bound of emissions arrays:
IF (YDMODEL%YRML_GCONF%YGFL%NEMIS2D == 0) THEN
  INEMIS2D = 1
ELSE
  INEMIS2D = YDMODEL%YRML_GCONF%YGFL%NEMIS2D
END IF
IF (YDMODEL%YRML_GCONF%YGFL%NEMIS2DAUX == 0) THEN
  INEMIS2DAUX = 1
ELSE
  INEMIS2DAUX = YDMODEL%YRML_GCONF%YGFL%NEMIS2DAUX
END IF
IF (YDMODEL%YRML_GCONF%YGFL%NEMIS3D == 0) THEN
  INEMIS3D = 1
ELSE
  INEMIS3D = YDMODEL%YRML_GCONF%YGFL%NEMIS3D
END IF
  ...
  CALL COMPO_APPLY_EMISSIONS(YDMODEL, &
   ...
   & PEMIS2D=PSURF%PSD_VF(:,YDSURF%YSD_VF%YEMIS2D(1)%MP:YDSURF%YSD_VF%YEMIS2D(INEMIS2D)%MP), &
   & PEMIS2DAUX=PSURF%PSD_VF(:,YDSURF%YSD_VF%YEMIS2DAUX(1)%MP:YDSURF%YSD_VF%YEMIS2DAUX(INEMIS2DAUX)%MP), &
   & PEMIS3D=PGFL(:,:,YDMODEL%YRML_GCONF%YGFL%YEMIS3D(1)%MP:YDMODEL%YRML_GCONF%YGFL%YEMIS3D(INEMIS3D)%MP), &
