...
SUBROUTINE DDR_ZH_SEASONONE(KMONTH, PLAT,                &
...
IMPLICIT NONE
  ...
  INTEGER (KIND=JPIM), INTENT(IN)   :: kveg_ifs

  INTEGER (KIND=JPIM), INTENT(OUT) :: kseason
  INTEGER (KIND=JPIM), INTENT(OUT) :: kveg_ZH
...
IF (LHOOK) CALL DR_HOOK('DDR_ZH_SEASONONE',0,ZHOOK_HANDLE)
...
! note wet skin tile has been rassigned to original tile - but rain/dew was set to 1
        ...
        SELECT CASE(ktile)
           ...
           CASE (4)
              kveg_ZH = 6_JPIM 
              if (iveg_ifs ==  1_JPIM)  kveg_ZH =  7_JPIM
              if (iveg_ifs ==  2_JPIM)  kveg_ZH =  6_JPIM
              if (iveg_ifs ==  7_JPIM)  kveg_ZH =  6_JPIM
              if (iveg_ifs ==  9_JPIM)  kveg_ZH =  9_JPIM
              if (iveg_ifs ==  10_JPIM)  kveg_ZH = 7_JPIM
              if (iveg_ifs ==  11_JPIM)  kveg_ZH =  8_JPIM
              if (iveg_ifs ==  13_JPIM)  kveg_ZH =  11_JPIM
              if (iveg_ifs ==  16_JPIM)  kveg_ZH =  10_JPIM
              if (iveg_ifs ==  17_JPIM)  kveg_ZH =  10_JPIM
              if (iveg_ifs ==  20_JPIM)  kveg_ZH =  11_JPIM
           CASE (5)
              kveg_ZH = 9_JPIM 
              kseason = 4_JPIM  
           CASE (6)
              kveg_ZH = 5_JPIM 
              if (iveg_ifs ==  3_JPIM)  kveg_ZH =  1_JPIM 
              if (iveg_ifs ==  4_JPIM)  kveg_ZH =  3_JPIM 
              if (iveg_ifs ==  5_JPIM)  kveg_ZH =  4_JPIM 
              if (iveg_ifs ==  6_JPIM)  kveg_ZH =  2_JPIM 
              if (iveg_ifs ==  18_JPIM)  kveg_ZH = 5_JPIM 
              if (iveg_ifs ==  19_JPIM)  kveg_ZH = 10_JPIM 
           CASE (7)
              kveg_ZH = 5_JPIM 
              if (iveg_ifs ==  3_JPIM)  kveg_ZH =  1_JPIM 
              if (iveg_ifs ==  4_JPIM)  kveg_ZH =  3_JPIM 
...
IF (LHOOK) CALL DR_HOOK('DDR_ZH_SEASONONE',1,ZHOOK_HANDLE)

END SUBROUTINE DDR_ZH_SEASONONE
