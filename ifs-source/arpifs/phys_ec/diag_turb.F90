...
!------------------------------------------------------------------------------------------------
  ...
  DO JK=KLEV,NJKT8,-1
    DO JL=KIDIA,KFDIA
       ...
       ZTDISS(JL,JK)=ZTDISS(JL,JK)+ABS(PU(JL,JK)*PTENOGW_U(JL,JK)+PV(JL,JK)*PTENOGW_V(JL,JK))**0.3333
  ...
  DO JK=KLEV-1,NJKT8,-1
    DO JL=KIDIA,KFDIA
      IF (JK <=KLAUNCH.AND.ZHEAT(JL)>2.0_JPRB) THEN
        ! ZHEAT-1 as dry contribution already counted above 
        ZTKE = ABS(PTENOGW_U(JL,JK)*PU(JL,JK)+PTENOGW_V(JL,JK)*PV(JL,JK))*MAX(0.0_JPRB,ZHEAT(JL)-1.0_JPRB)
