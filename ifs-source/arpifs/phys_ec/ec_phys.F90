...
SUBROUTINE EC_PHYS(YDGEOMETRY,YDVARS,YDGMV,YDSURF,YDMODEL,CDCONF,DIMS,&
 ...
 & STATE_T0, TENDENCY_DYN, TENDENCY_CML, STATE_TMP, TENDENCY_VDF, &
 & PTENDENCY_VD9, TENDENCY_SATADJ, TENDENCY_LOC, TENDENCY_PHY)
...
!        F. Vana       14-Dec-2022 : Removed TENDENCY_TMP for real
...
REAL(KIND=JPRB)          ,INTENT(INOUT) :: PTENDENCY_VD9(DIMS%KLON,DIMS%KLEV,YDMODEL%YRML_PHY_G%YRSLPHY%NVTEND_VD)
...
& RMFADVWDD=>YDMODEL%YRML_PHY_EC%YRECUMF%RMFADVWDD, YDCST=>YDMODEL%YRCST, YDCVER=>YDGEOMETRY%YRCVER,&
& YDDYNA=>YDMODEL%YRML_DYN%YRDYNA,YDDYN=>YDMODEL%YRML_DYN%YRDYN,&
& YSD_VD=>YDSURF%YSD_VD, YSD_VF=>YDSURF%YSD_VF, YSP_SB=>YDSURF%YSP_SB, YSP_RR=>YDSURF%YSP_RR,&
& YSP_SL=>YDSURF%YSP_SL, YSP_SG=>YDSURF%YSP_SG, YSD_VN=>YDSURF%YSD_VN, YSD_WW=>YDSURF%YSD_WW )

ASSOCIATE(NERA40=>YGFL%NERA40, YERA40=>YGFL%YERA40, YI=>YGFL%YI, YL=>YGFL%YL, LEGBRAD=>YDEPHY%LEGBRAD, &
& LERAINGG=>YDEPHY%LERAINGG, LFPOS_EC_PHYS=>YDEPHY%LFPOS_EC_PHYS, NPRACCL=>YDEPHY%NPRACCL, & 
& LFPOS_ACC_RESET=>YDEPHY%LFPOS_ACC_RESET, NATMFLX=>YDEWCOU%NATMFLX,  LWCOU=>YDEWCOU%LWCOU, LWCOURNW=>YDEWCOU%LWCOURNW, &
& LWCUR=>YDEWCOU%LWCUR, RMISSW=>YDEWCOU%RMISSW, &
& NGPTOT=>YDGEM%NGPTOT, LNCLIN=>YDNCL%LNCLIN, LENCLD2=>YDPHNC%LENCLD2, LETRAJP=>YDPHNC%LETRAJP, NSITER=>YDDYN%NSITER, &
& NCURRENT_ITER=>YDDYN%NCURRENT_ITER,  LPC_FULL=>YDDYNA%LPC_FULL, RCODEC=>YDRIP%RCODEC, RCOVSR=>YDRIP%RCOVSR, &
& RSIDEC=>YDRIP%RSIDEC, RSIVSR=>YDRIP%RSIVSR, TSTEP=>YDRIP%TSTEP, TSPHY=>YDPHY2%TSPHY)
...
LLPHY_POS=(.NOT.LPC_FULL).OR.(NSITER == NCURRENT_ITER)
...
ZCGPP(DIMS%KIDIA:DIMS%KFDIA)=PSURF%PSD_VF(DIMS%KIDIA:DIMS%KFDIA,YSD_VF%YCGPP%MP)
ZCREC(DIMS%KIDIA:DIMS%KFDIA)=PSURF%PSD_VF(DIMS%KIDIA:DIMS%KFDIA,YSD_VF%YCREC%MP)
...
CALL CALLPAR(YDGEOMETRY,YDVARS,YDSURF,YDMODEL,YDMODEL%YRML_PHY_EC%YRTHF,YDCST,DIMS,&
 & PAUX, PRAD, FLUX, ZDIAG, PSURF, &
 & ZCGPP, ZCREC, ZAG, ZRECO,&
...
IF (LLPHY_POS) THEN
  CALL POSTPHY_LAYER(YDGEOMETRY,YDSURF,YDMODEL,DIMS, LLFSTEP  ,LLMLPP, LLRS6, LLACCRS,&
   & U%PH9, V%PH9, T%PH9, PGFL, ZTENQD, PAUX, PRAD, PSURF, FLUX, ZDIAG, ZDDHS)
ENDIF

!*    4.3  Accumulation of coupled fields

IF (NFRCO /= 0 .AND. LLPHY_POS) THEN
...
!   NATMFLX=2  ::  SURFACE STRESS 
    CASE(2)
      PSURF%PSD_WW(JROF,YSD_WW%YU10N%MP) = PSURF%PUSTRTI(JROF,JTILE4WAM)
      PSURF%PSD_WW(JROF,YSD_WW%YV10N%MP) = PSURF%PVSTRTI(JROF,JTILE4WAM) 
...
!     use the neutral winds and substract ocean currents
      ...
      ELSE
!!      If you use the total stress, you need to remove the TOFD contribution since it is added over land
!!        ZTAU_U = (FLUX%PSTRTU(JROF,DIMS%KLEV)-FLUX%PTOFDU(JROF))*ZRHO_A_M1
!!        ZTAU_V = (FLUX%PSTRTV(JROF,DIMS%KLEV)-FLUX%PTOFDV(JROF))*ZRHO_A_M1
!!      Use the ocean tile stress instead:
        ZTAU_U = PSURF%PUSTRTI(JROF,JTILE4WAM)*ZRHO_A_M1
        ZTAU_V = PSURF%PVSTRTI(JROF,JTILE4WAM)*ZRHO_A_M1
...
!   Surface stress over the ocean tile
    PSURF%PSD_WW(JROF,YSD_WW%YUSTRW%MP) = PSURF%PUSTRTI(JROF,JTILE4WAM)
    PSURF%PSD_WW(JROF,YSD_WW%YVSTRW%MP) = PSURF%PVSTRTI(JROF,JTILE4WAM)
