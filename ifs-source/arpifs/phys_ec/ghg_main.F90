...
! nor does it submit to any jurisdiction
 ...
 SUBROUTINE GHG_MAIN( YDML_CHEM,YGFL,KIDIA, KFDIA, KLON, KLEV, KFLDX, KLEVX, KTRAC, PTSTEP, PRS1, &  
    & PLRCH4, PCFLX, YDEPHY, PNEEFLX, PCH4FLX, PGELAT, PGFL, PTENGFL, PEXTRA, PCEN, PTENC ) 
...
USE YOMCHEM        , ONLY : IEXTR_CH,IEXTR_EM,IEXTR_DD
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PCH4FLX(KLON)
...
ASSOCIATE(LCHEM_DIA=>YDML_CHEM%YRCOMPO%LCHEM_DIA, YGHG=>YGFL%YGHG, & 
 ...
 & NGHG=>YGFL%NGHG, LNEEONLINE=>YDEPHY%LNEEONLINE, LWETONLINE=>YDEPHY%LWETONLINE)
DO JK=1,KLEV
  DO JL=KIDIA,KFDIA
...
IF(IPCO2 >= 0 .AND. LNEEONLINE) THEN
  IF (LCHEM_DIA) THEN
...
IF(IPCH4 >= 0 .AND. LWETONLINE) THEN
  IF (LCHEM_DIA) THEN
     ! add ch4 wetland flux from model to the dry deposition in mass diagnostics in order 
     ! to be able to monitor budget separately for the online emissions
    PEXTRA(KIDIA:KFDIA,NCHEM+NACTAERO+IPCH4, IEXTR_DD) = PEXTRA(KIDIA:KFDIA,NCHEM+NACTAERO+IPCH4, IEXTR_DD) - & 
 &        PCH4FLX(KIDIA:KFDIA)  * PTSTEP     
  ENDIF
ENDIF
