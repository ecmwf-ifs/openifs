...
ALLOCATE(GEMSL%AEREXTSURF(1:KDIM%KLON))
GEMSL%AEREXTSURF(:) = 0.0_JPRB
...
IF (NGHG > 0 .OR. NCHEM > 0 .OR. NAERO > 0 ) THEN
  ...
  ALLOCATE(GEMSL%ZCLOUDPH(1:KDIM%KLON,1:KDIM%KLEV))
  GEMSL%ZCLOUDPH(:,:)= 0.0_JPRB 
  ALLOCATE(GEMSL%ZRAINPH(1:KDIM%KLON,1:KDIM%KLEV))
  GEMSL%ZRAINPH(:,:)= 0.0_JPRB 
  ALLOCATE(GEMSL%ZCHEMPH(1:KDIM%KLON,1:KDIM%KLEV))
  GEMSL%ZCHEMPH(:,:)= 0.0_JPRB 
  ALLOCATE(GEMSL%ZCHEMPHR(1:KDIM%KLON,1:KDIM%KLEV))
  GEMSL%ZCHEMPHR(:,:)= 0.0_JPRB 
...
! ALLOCATE arrays so they can be passed to turbulence scheme etc. in the non-GEMS case
  ...
  ALLOCATE(GEMSL%ZCLOUDPH(1,1))
  ALLOCATE(GEMSL%ZRAINPH(1,1))
  ALLOCATE(GEMSL%ZCHEMPH(1,1))
  ALLOCATE(GEMSL%ZCHEMPHR(1,1))
  ...
  GEMSL%ZCLOUDPH(1,1) = 0.0_JPRB
  GEMSL%ZRAINPH(1,1) = 0.0_JPRB
  GEMSL%ZCHEMPH(1,1) = 0.0_JPRB
  GEMSL%ZCHEMPHR(1,1) = 0.0_JPRB
