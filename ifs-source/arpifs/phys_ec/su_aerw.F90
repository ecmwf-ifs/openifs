...
LOGICAL, POINTER :: LAERCLIMG, LAERCLIMZ, LAERCLIST, LAERDRYDP, LAERELVS,&
 ...
 & LAERINIT, LAERCSTR, LAERRRTM, LAERFILLCLIM, LAERDIAG1, LAERDIAG2, LAERUVP,&
 & LAERVOL, LEPAERO, LAEROMIN, LOCNDMS, LPROGAERVIS, &
...
LAERFILLCLIM => YDEAERATM%LAERFILLCLIM
LPROGAERVIS=>YDEAERATM%LPROGAERVIS
...
! By default we don't fill any "missing" prognostic aerosol species
! with climatological in the radiation scheme
LAERFILLCLIM = .FALSE.
...
ELSE
  ! Prognostic aerosols are active
  LEPAERO     = .TRUE.
  LAERRAD     = .TRUE. ! Copy aerosol optical properties for use in DA and CAMS diagnostics
  LPROGAERVIS = .TRUE. ! Use prognostic aerosols for visibility by default
  LAERRRTM    = .TRUE. ! Use prognostic aerosols in radiation scheme
  LAERFILLCLIM=.FALSE. ! Don't complement prognostic optical properties with climatological ones for mssing species
...
WRITE(UNIT=NULOUT,FMT='('' Interaction prognostic aerosols and radiation: '',&
     &'' LAERCSTR= '',L1,'' LAERRRTM= '',L1,'' LAERFILLCLIM= '',L1)') LAERCSTR,LAERRRTM,LAERFILLCLIM

IF (NACTAERO > 0) THEN

  WRITE(UNIT=NULOUT,FMT='(''NAERO='',I2,'' NACTAERO='',I2,3X,10I3)') NAERO,NACTAERO,(NTYPAER(JAER),JAER=1,10)

!-- if MACC model includes volcanic aerosol, no stratospheric adjustment is included
  IF (LAERVOL) THEN
    LAERCSTR=.FALSE.
  ENDIF
!     ------------------------------------------------------------------
...
!              -------------------
  ...
  WRITE(UNIT=NULOUT,FMT='(" LAERSURF = ",L1 &
   ...
   & ,'' NAERAGING = '',I1 &
   & )')&
   & LAERSURF,LAERELVS, LOCNDMS, RDMSMIN, NDMSO, NPIST,NAERAGING
  WRITE(UNIT=NULOUT,FMT='('' LAERGBUD = '',L1 &
   ...
   & LAERCLIMZ,LAERCLIMG,LAERCLIST,LAERRAD,LPROGAERVIS
