...
!      P. Bechtold   Dec 2022: Cleaning obsolete IGRID/NDGLG dependencies
!     ------------------------------------------------------------------
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV, YDSTA=>YDGEOMETRY%YRSTA, YDVAB=>YDGEOMETRY%YRVAB, &
 ...
 & YDCVER=>YDGEOMETRY%YRCVER, YDPHY2=>YDMODEL%YRML_PHY_MF%YRPHY2, YDESWRT=>YDMODEL%YRML_PHY_RAD%YRESWRT, &
 & LWCOU=>YDMODEL%YREWCOU%LWCOU, LWCOU2W=>YDMODEL%YREWCOU%LWCOU2W, LWCOUHMF=>YDMODEL%YREWCOU%LWCOUHMF)

ASSOCIATE(NSMAX=>YDDIM%NSMAX, NFLEVG=>YDDIMV%NFLEVG, NCOM=>YDDPHY%NCOM, NCSS=>YDDPHY%NCSS, &
 & NTILES=>YDDPHY%NTILES, NCSNEC=>YDDPHY%NCSNEC,  LEAGS=>YDEPHY%LEAGS, LECTESSEL=>YDEPHY%LECTESSEL, &
 & LEFLAKE=>YDEPHY%LEFLAKE, LEAIRCO2COUP=>YDEPHY%LEAIRCO2COUP, LEFARQUHAR=>YDEPHY%LEFARQUHAR, &
 & LELAIV=>YDEPHY%LELAIV, LEOCCO=>YDEPHY%LEOCCO, LEOCLA=>YDEPHY%LEOCLA, LEOCML=>YDEPHY%LEOCML, &
 & LEOCSA=>YDEPHY%LEOCSA, LEOCWA=>YDEPHY%LEOCWA, LEPHYS=>YDEPHY%LEPHYS, LESN09=>YDEPHY%LESN09, &
 & LESSRO=>YDEPHY%LESSRO, LESNML=>YDEPHY%LESNML, LEURBAN=>YDEPHY%LEURBAN, LEVGEN=>YDEPHY%LEVGEN, &
 & LOCMLTKE=>YDEPHY%LOCMLTKE, RLAIINT=>YDEPHY%RLAIINT, RTHRFRTI=>YDEPHY%RTHRFRTI, &
...
CALL SUCUMF2(YDSTA,YDDIMV,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_PHY_SLIN%YRCUMFS,&
           & YDMODEL%YRML_PHY_SLIN%YRECUMF2)
...
CALL SUGWWMS(YDSTA,YDDIMV,YDMODEL%YRML_PHY_EC%YREGWWMS,YDMODEL%YRML_GCONF%YRRIP)
...
CALL SUSURF(KSW=NSW,KCSS=NCSS,KCSNEC=NCSNEC,KSIL=NSIL,KCOM=NCOM,KTILES=NTILES,&
 ...
 & LD_LEFLAKE=LEFLAKE,LD_LEURBAN=LEURBAN,LD_LEOCML=LEOCML,LD_LOCMLTKE=LOCMLTKE,&
