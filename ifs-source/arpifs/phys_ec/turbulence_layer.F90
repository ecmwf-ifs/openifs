...
SUBROUTINE TURBULENCE_LAYER(YDSURF, YDMODEL, &
 ! Input quantities
  & KDIM, PAUX, STATE, TENDENCY, PRAD, PPERT, LDDIAG, &
...
!-----------------------------------------------------------------------
INTEGER(KIND=JPIM) :: JL, JK, JRF, JTILE, JJ, IDIAG, IS, IK
CHARACTER(LEN=1)  :: CLCONF
REAL(KIND=JPRB) :: ZRCPD, ZDZ, ZRHO, ZQTMST, ZRG_R, ZTP1, ZGDPH_R
REAL(KIND=JPRB) :: ZR0VT(KDIM%KLON), ZEVAPMU(KDIM%KLON)
REAL(KIND=JPRB) :: ZFSQLF(KDIM%KLON,0:KDIM%KLEV),ZFSQIF(KDIM%KLON,0:KDIM%KLEV)
...
REAL(KIND=JPRB) :: ZSTRDU(KDIM%KLON,0:KDIM%KLEV),ZSTRDV(KDIM%KLON,0:KDIM%KLEV)
REAL(KIND=JPRB) :: ZI10FG(KDIM%KLON)
...
ASSOCIATE(YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY,YDMCC=>YDMODEL%YRML_AOC%YRMCC, &
 & YDPHY2=>YDMODEL%YRML_PHY_MF%YRPHY2,YDECLDP=>YDMODEL%YRML_PHY_EC%YRECLDP, &
...
ASSOCIATE(LBUD23=>YDEPHY%LBUD23, LPPTILES=>YDEPHY%LPPTILES,&
 ...
 & LCLDBUDI=>YDECLDP%LCLDBUDI, LCLDBUD_VERTINT=>YDECLDP%LCLDBUD_VERTINT, &
...
!*         1.     UNROLL THE DERIVED STRUCTURES AND CALL VDFMAINS
  ...
  CALL VDFOUTER (YDMODEL, CLCONF,&
    ...
    & TSPHY  ,LLKEYS%LLLAND, PSURF%ITVL   , PSURF%ICO2TYP, PSURF%ITVH  , PSURF%PCVL  , PSURF%PCVH   , PSURF%PCUR, &
    ...
    & ZPRES , ZPRESF , PAUX%PGEOM1 , PAUX%PGEOMH , PAUX%PGELAT , PSURF%PSP_RR(:,YSP_RR%YT%MP9) , &
    ...
    & PSURF%PSD_VF(:,YSD_VF%YSST%MP)   , PSURF%ISOTY  , SURFL%ZFRTI  , SURFL%ZALBTI , &
    & PSURF%PSD_VD(:,YSD_VD%YALB%MP), SURFL%ZALBD  , &
    & PAUX%PGELAM, &
    & PSURF%PSD_VF(:,YSD_VF%YAERLTS%MP),PSURF%PSD_VF(:,YSD_VF%YAERSCC%MP),&
    & GEMSL%ZAERMAP, GEMSL%ZAERFLX, GEMSL%ZAERLIF,  GEMSL%ZAERSRC, &
    & PSURF%PSD_VF(:,YSD_VF%YDSF%MP), PSURF%PSD_VF(:,YSD_VF%YDSZ%MP), &
    & SURFL%ZHSDFOR, PSURF%PSD_VF(:,YSD_VF%YLSM%MP), SURFL%ZWLMX, &
    ...
    & AUXL%ZTSKRAD, GEMSL%ZCFLX , GEMSL%ZDDVLC,  GEMSL%ZAERDDP, AUXL%ZSOTEU , AUXL%ZSOTEV , AUXL%ZSOBETA, &
    ...
    & SURFL%ZFRSOTI, SURFL%ZEVAPSNW , ZEVAPMU , ZEXDIAG, ZI10FG  , PSURF%PSD_VD(:,YSD_VD%Y10FGCV%MP), PDIAG%IPBLTYPE, &
    ! Input tendencies
    & tendency%T, tendency%q, tendency%cld(:,:,NCLDQL), tendency%cld(:,:,NCLDQI), &
    & tendency%a, tendency%u, tendency%v, tendency%tke, &
    ! OUTPUT TENDENDCIES
    ...
    & ZSTRDU , ZSTRDV , PDIAG%PKH_VDF, PDIAG%PKM_VDF, PDIAG%ZRI, &
...
PDIAG%PI10FG(KDIM%KIDIA:KDIM%KFDIA)=ZI10FG(KDIM%KIDIA:KDIM%KFDIA) 
FLUX%PSTRDU(KDIM%KIDIA:KDIM%KFDIA,0:KDIM%KLEV)=ZSTRDU(KDIM%KIDIA:KDIM%KFDIA,0:KDIM%KLEV)
FLUX%PSTRDV(KDIM%KIDIA:KDIM%KFDIA,0:KDIM%KLEV)=ZSTRDV(KDIM%KIDIA:KDIM%KFDIA,0:KDIM%KLEV)
