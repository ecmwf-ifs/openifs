...
DO JK=1,NLV1
  DO JL=1,NLAT1
    DO JI=1,NLON1
      YDECMIP%ROZT1(JI,JL,JK)=YDERDI%RO3 * (YDECMIP%ZOZCL(JI,JL,JK,IM1)&
        & +ZTIMI*(YDECMIP%ZOZCL(JI,JL,JK,IM2)-YDECMIP%ZOZCL(JI,JL,JK,IM1)))
    ENDDO
  ENDDO
ENDDO
...
WRITE(NULOUT,*)"UPDECOZV: OZONE (kg/kg) at (JI=5,JL=5,JK=5) FOR (IYR,IM1,IM2,ZTIMI) ", &
    & IYR, IM1, IM2, ZTIMI, YDECMIP%ROZT1(5,5,5)

WRITE(NULOUT,*)"UPDECOZV: GLOBAL AVERAGE OZONE (kg/kg) FOR (IYR,IM1,IM2,ZTIMI) ", &
    & IYR, IM1, IM2, ZTIMI, SUM(YDECMIP%ROZT1)/(NLON1*NLAT1*NLV1)
