...
SUBROUTINE VDFMAIN    ( YDMODEL,CDCONF, &
 ...
 & PTSPHY , LDLAND , KTVL   , KCO2TYP, KTVH   , PCVL   , PCVH   , PCUR, PFWET  ,&
 ...
 & PSST   , KSOTY  , PFRTI  , PALBTI, PALB, PALBD, PGELAM, &
 & PAERLTS, PAERSCC, PAERMAP, PAERFLX, PAERLIF, PAERSRC, PDSF, PDSZ, &
 & PHSDFOR, PLSM, PWLMX  , &
 & PCHAR  , PCHARHQ, PUCURR , PVCURR ,PUSTOKES,PVSTOKES, &
 & PTSKRAD, PCFLX  , PDDVLC , PAERDDP, PSOTEU , PSOTEV , PSOBETA, &
...
!    *PALB*         BROADBAND ALBEDO
!    *PALBD*        BROADBAND ALBEDO
!    *PGELAM*       LONGITUDE IN RAD
...
!    *PAERDDP*      AEROSOL DRY DEP DIAGNOSTIC                    kg/(m2 s)
...
!      J. McNorton 24-08-2022      Add Urban Tile
!     --------------------------------------------------------------------------------------
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAERLTS(KLON)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAERSCC(KLON)
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PDSF(KLON)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PDSZ(KLON)
...
REAL(KIND=JPRB) ::  ZRHO(KLON), ZUSTARGUST(KLON), ZSNMSUM(KLON)
...
REAL(KIND=JPRB)   :: ZSST(KLON), ZPGP2DSPP(KLON)

! A bunch of SPP variables
LOGICAL            :: LLPERT_CFM, LLPERT_CFM1, LLPERT_CFM2, LLPERT_CFM3, LLPERT_TOFDC ! SPP perturbation on?
LOGICAL            :: LLPERT_VDSST, LLPERT_COLDSKIN
INTEGER(KIND=JPIM) :: IPCFM, IPCFM1, IPCFM2, IPCFM3, IPTOFDC, IPVDSST, IPCOLDSKIN ! SPP random field pointer
INTEGER(KIND=JPIM) :: IPN               ! SPP perturbation pointer
TYPE(SPP_PERT)     :: PNCFM, PNCFM1, PNCFM2, PNCFM3, PNTOFDC, PNVDSST, PNCOLDSKIN ! SPP pertn. configs.
REAL(KIND=JPRB)    :: ZPERTOC, ZPERTLA ! used in CFM loop
REAL(KIND=JPRB)    :: ZFACT

REAL(KIND=JPRB) ::    ZGDPH, ZTMST, ZRG, ZRTMST,ZWST2,ZCON2,ZIPBL,ZDUA2
...
ASSOCIATE(YGFL=>YDMODEL%YRML_GCONF%YGFL,YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY,YDECLDP=>YDMODEL%YRML_PHY_EC%YRECLDP, &
 ...
 & YDSPP_CONFIG=>YDMODEL%YRML_GCONF%YRSPP_CONFIG,  YDERAD=>YDMODEL%YRML_PHY_RAD%YRERAD )
ASSOCIATE(NGHG=>YGFL%NGHG, YGHG=>YGFL%YGHG, &
 ...
 & REPQMI=>YDMODEL%YRML_PHY_EC%YRECND%REPQMI, LEAIRCO2COUP=>YDEPHY%LEAIRCO2COUP,LBLEND=>YDEPHY%LBLEND, &
 & RG=>YDCST%RG, RCPD=>YDCST%RCPD, RETV=>YDCST%RETV, RLVTT=>YDCST%RLVTT, RLSTT=>YDCST%RLSTT, &
 & RTT=>YDCST%RTT, RD=>YDCST%RD, &
 & R2ES=>YDTHF%R2ES, R3LES=>YDTHF%R3LES, R3IES=>YDTHF%R3IES, R4LES=>YDTHF%R4LES, &
 & R4IES=>YDTHF%R4IES, R5LES=>YDTHF%R5LES, R5IES=>YDTHF%R5IES, RVTMP2=>YDTHF%RVTMP2, &
...
IF (YDSPP_CONFIG%LSPP) THEN
  ...
  IPN = YDSPP_CONFIG%PPTR%VDSST
  LLPERT_VDSST= IPN > 0
  IF (LLPERT_VDSST) THEN
    PNVDSST=YDSPP_CONFIG%SM%PN(IPN)
    IPVDSST = PNVDSST%MP
  ENDIF

  IPN = YDSPP_CONFIG%PPTR%COLDSKIN
  LLPERT_COLDSKIN= IPN > 0
  IF (LLPERT_COLDSKIN) THEN
    PNCOLDSKIN=YDSPP_CONFIG%SM%PN(IPN)
    IPCOLDSKIN = PNCOLDSKIN%MP
  ENDIF
...
ELSE
  ...
  LLPERT_TOFDC=.FALSE.
  LLPERT_VDSST=.FALSE.
  LLPERT_COLDSKIN=.FALSE.
ENDIF

DO JL=KIDIA,KFDIA
  IF (LLPERT_VDSST) THEN
    ZSST(JL) = PSST(JL) * EXP(PNVDSST%MU(1)+PNVDSST%XMAG(1)*PGP2DSPP(JL, IPVDSST))
  ELSE
    ZSST(JL) = PSST(JL)
  ENDIF
ENDDO
...
IF (LLPERT_COLDSKIN) THEN
  DO JL=KIDIA,KFDIA
    ZFACT=PNCOLDSKIN%XMAG(1)*PGP2DSPP(JL, IPCOLDSKIN)
    ZFACT=MIN(ZFACT,PNCOLDSKIN%XMAX(1))
    ZFACT=MAX(ZFACT,PNCOLDSKIN%XMIN(1))
    ZPGP2DSPP(JL) = 1.0_JPRB + ZFACT
  ENDDO
ENDIF
...
! input data, non-tiled
 ...
 & PCPTGZLEV=ZCPTGZ(:,KLEV), PSST=ZSST, PTSKM1M=PTSKM1M, PCHAR=PCHAR, PCHARHQ=PCHARHQ, &
...
IF (LLSFCFLX) THEN
  DO JL=KIDIA,KFDIA
    ZRHO(JL) = PAPHM1(JL,KLEV)/( RD*PTM1(JL,KLEV)*(1.0_JPRB+RETV*PQM1(JL,KLEV)) )
    ZKHFL(JL) = ZEXTSHF(JL) / ( RCPD*(1.0_JPRB+RVTMP2*PQM1(JL,KLEV)) ) / ZRHO(JL)
    ZKQFL(JL) = ZEXTLHF(JL) / RLVTT / ZRHO(JL)
  ENDDO
ENDIF
...
CALL VDFHGHTN (RPLDARE , RPLRG   , YDMODEL%YRML_PHY_SLIN%YREPHLI,YDECLDP , YDECUMF   , YDVDF, YDSPP_CONFIG, &
             & KIDIA   , KFDIA   , KLON    , KLEV    , PTSPHY, LLAND,&
             & PTM1    , PQM1    , ZSLGSM1 , PUM1    , PVM1      , ZCPTGZ, PQE,&
...
DO JL=KIDIA,KFDIA
  ZLIMIN(JL)=0.0_JPRB
  LLFIRST(JL)=.TRUE.
  LLTEST(JL)=KPBLTYPE(JL)==1.AND.PZINV(JL)<150._JPRB.AND.ZKHVFL(JL)*RCPD>-20._JPRB
ENDDO
DO JK=KLEV,NJKT6,-1
 DO JL=KIDIA,KFDIA
...
IF (LLPERT_TOFDC) THEN
  DO JK=1,KLEV
    DO JL=KIDIA,KFDIA
      ZFACT=EXP(PNTOFDC%MU(1) + PNTOFDC%XMAG(1)*PGP2DSPP(JL,IPTOFDC))
      ZFACT=MIN(ZFACT,PNTOFDC%XMAX(1))
      ZFACT=MAX(ZFACT,PNTOFDC%XMIN(1))
      ZTOFDC(JL,JK)=ZTOFDC(JL,JK)*ZFACT
    ENDDO
  ENDDO
ENDIF
...
IF (LWDS) THEN
  ...
  CALL VDFDIFH(YDMODEL%YRML_AOC%YRMCC,YDEPHY,YDMODEL%YRML_GCONF%YRRIP, &
            ...
            & PTSN(1,1) ,PSNM(1,1), PRSN(1,1) , PTICE  , ZSST, &
            & PTHKICE, PSNTICE, &
...
CALL SURFPP(YSURF,KIDIA=KIDIA,KFDIA=KFDIA,KLON=KLON,KTILES=KTILES, &
 ...
 & PTSTEP=PTSPHY, LPERT_COLDSKIN=LLPERT_COLDSKIN, &
! input
 ...
 & PZ0M=PZ0M, PEVAPSNW=PEVAPSNW,PSSRFLTI=PSSRFLTI, PSLRFL=PSLRFL, PSST=ZSST, &
 & PUCURR=PUCURR, PVCURR=PVCURR, PUSTOKES=PUSTOKES, PVSTOKES=PVSTOKES, PGP2DSPP=ZPGP2DSPP, &
 ! tile depend
 & PZ0MTIW=ZZ0MTIW, PZ0HTIW=ZZ0HTIW, PZ0QTIW=ZZ0QTIW, PZDLTI=ZZZDLTI, PQSAPPTI=ZZQSATI, PCPTSPPTI=ZZCPTSTI, &
! updated
...
!
  DO JL=KIDIA,KFDIA
    ZRHO(JL) = PAPHM1(JL,KLEV)/( RD*PTM1(JL,KLEV)*(1.0_JPRB+RETV*PQM1(JL,KLEV)) )
    ZSNMSUM(JL)=SUM(PSNM(JL,:)) ! pre-compute total snow mass
    ZUSTARGUST(JL)=PUST(JL)
    IF (ZZBUOM(JL) > 0.0_JPRB) THEN
      ZWST2=(ZZBUOM(JL)*ZIPBL)**ZCON2
      ZDUA2=MAX(REPDU2,PUM1(JL,KLEV)**2+PVM1(JL,KLEV)**2)+ZWST2
      ZUSTARGUST(JL)=PUST(JL)*SQRT(SQRT(ZDUA2/MAX(REPDU2,PUM1(JL,KLEV)**2+PVM1(JL,KLEV)**2)))
    ENDIF
  ENDDO
  IF (NACTAERO > 0 .AND. AERO_SCHEME /= "glomap") THEN
    ! aerosol dust emissions
    CALL AER_DUSTEMIS&
 &( YDMODEL, KIDIA,KFDIA,KLON,KLEV, KTILES, NSW, KTRAC, KAERO, &
 &  PALB, PALBD  , PGELAM, PGELAT, PAERLTS, PAERSCC, PGUST, ZUSTARGUST, &
 &  PDSF, PDSZ, PI10FGCV, PAPHM1, PFRTI, PHSDFOR, PLSM, ZRHO, ZSNMSUM, PZ0M, PWSAM1M(:,1), &
 &  PAERFLX, PAERLIF, PAERSRC, PAERMAP, PCFLX, ZCFLXIN, PTENC)
  ENDIF
! dry depostion velocity on-line for chemistry and aerosol range
...
! PDDVLC is updated
     IF (KCHEM_DRYDEP == 1 ) THEN
       ...
       CALL  DEPVEL_WSL (YDMODEL, KIDIA, KFDIA, KLON, KTRAC, KCHEM, KAERO, KTILES, PFRTI,  PGELAT,  &
                           ...
                           &   PTSKM1M, PT2M, PD2M,PAPM1(1:KLON,KLEV),ZSNMSUM, PSSRFL, ZRAQTI, ZRAQ,  &
                           &   PUST, ZUSTARGUST,  PZ0M, PDDVLC, ZDEPVEL )
     ELSE
       ! New GEOS-Chem variant (also based on Wesely)
       CALL  DEPVEL_GC (YDMODEL, KIDIA, KFDIA, KLON, KTRAC, NCHEM, KTILES, PFRTI,  PGELAT, ZSNMSUM,  &
                           &   PCVL, PCVH, PLAIL, PLAIH, KTVL, KTVH,  &
                           &   ZWETB, ZWETL, ZWETLU, ZWETH, ZWETHS, &
                           &   PMU0, PAPM1(1:KLON,KLEV),&
                           &   PGEOM1(1:KLON,KLEV),PGEOH(1:KLON,KLEV),PAHFSTI, &
                           &   PTSKM1M, PT2M, PD2M, PSSRFL, ZRAQTI, ZRAQ,  &
                           &   PUST, ZUSTARGUST,  PZ0M, PDDVLC, ZDEPVEL )
     ENDIF
   ...
   DO JL=KIDIA,KFDIA
     ZWD(JL,1:KTRAC)=ZRHO(JL)*PDDVLC(JL,1:KTRAC)
   ENDDO
  ...
  CALL VDFDIFC(KIDIA,KFDIA,KLON,KLEV,ITOP,KTRAC,&
   ...
   DO JL=KIDIA,KFDIA
     PAERDDP(JL,1:NACTAERO)=PCFLXO(JL,KAERO(1:NACTAERO))-ZCFLXIN(JL,KAERO(1:NACTAERO))
   ENDDO
ENDIF
