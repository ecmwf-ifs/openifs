...
REAL(KIND=JPRB) ::    ZZ0MTIW(KLON,KTILES)       , ZZ0HTIW(KLON,KTILES)       , ZZ0QTIW(KLON,KTILES),&
                    & ZZBUOMTI(KLON,KTILES), ZZQSATI(KLON,KTILES), ZZCPTSTI(KLON,KTILES)
...
ASSOCIATE(LEPPCFLS=>YDEPHLI%LEPPCFLS, &
 ...
 & RTT=>YDCST%RTT, &
 & RVTMP2=>YDTHF%RVTMP2, R2ES=>YDTHF%R2ES, R3LES=>YDTHF%R3LES, R3IES=>YDTHF%R3IES, &
 & R4LES=>YDTHF%R4LES, R4IES=>YDTHF%R4IES, R5LES=>YDTHF%R5LES, R5IES=>YDTHF%R5IES, &
...
CALL SURFEXCDRIVERS( YDSURF=YSURF,&
 & KIDIA=KIDIA, KFDIA=KFDIA, KLON=KLON, KLEVS=KLEVS, KLEVSN=KLEVSN,KTILES=KTILES,&
 & KSTEP=NSTEP,&
 & PTSTEP=PTSPHY, PRVDIFTS=PVDIFTS,&
 & LDSURF2=LESURF2,&
! input data, non-tiled
 & KTVL=KTVL, KTVH=KTVH, PCVL=PCVL, PCVH=PCVH,PCUR=PCUR,&
...
! output data, tiled
 ...
 & PZ0MTIW=ZZ0MTIW, PZ0HTIW=ZZ0HTIW, PZ0QTIW=ZZ0QTIW, PBUOMTI=ZZBUOMTI, PQSAPPTI=ZZQSATI, PCPTSPPTI=ZZCPTSTI, &
...
! input
 ...
 ! tile depend
 & PZ0MTIW=ZZ0MTIW, PZ0HTIW=ZZ0HTIW, PZ0QTIW=ZZ0QTIW, PQSAPPTI=ZZQSATI, PCPTSPPTI=ZZCPTSTI, PBUOMTI=ZZBUOMTI, &
! updated
