...
SUBROUTINE VDFOUTER   (YDMODEL,CDCONF, &
 ...
 & PTSPHY , LDLAND , KTVL   , KCO2TYP, KTVH   , PCVL   , PCVH   , PCUR, &
 ...
 ! Input tendencies
 & PTE0   , PQE0   , PLE0   , PIE0   , PAE0   , PVOM0  , PVOL0  , PTKEE0,&
 ! OUTPUT TENDENCIES
...
!    *PAERDDP*      AEROSOL DRY DEP DIAGNOSTIC                    kf/(m2 s)
...
REAL(KIND=JPRB)   ,INTENT(IN)  :: PTKEE0(KLON,KLEV)
...
REAL(KIND=JPRB)   ,INTENT(OUT) :: PTKEE(KLON,KLEV)
...
INTEGER(KIND=JPIM) :: JROF, JLEV, JTR
LOGICAL         ::    LLTROPQ_OFFL
REAL(KIND=JPRB):: ZUM1(KLON,KLEV), ZVM1(KLON,KLEV), ZTM1(KLON,KLEV), &
 &      ZQM1(KLON,KLEV), ZAM1(KLON,KLEV), ZLM1(KLON,KLEV), ZIM1(KLON,KLEV), &
 &    ZTKE1(KLON,KLEV), ZCM1(KLON,KLEV,KTRAC) 
...
ASSOCIATE(LVDFTRAC=>YDEPHY%LVDFTRAC, ECTMIN=>YDPHY0%ECTMIN,LECT=>YRPHY%LECT)
!     ------------------------------------------------------------------
...
!        INITIALIZE TENDENCIES AND UPDATED STATE 

DO JLEV=1,KLEV
  DO JROF=KIDIA,KFDIA
    PVOM(JROF,JLEV)=PVOM0(JROF,JLEV)
    PVOL(JROF,JLEV)=PVOL0(JROF,JLEV)
    ZUM1(JROF,JLEV)=PUM1(JROF,JLEV) !+PVOM0(JROF,JLEV)*PTSPHY
    ZVM1(JROF,JLEV)=PVM1(JROF,JLEV) !+PVOL0(JROF,JLEV)*PTSPHY
  ENDDO
ENDDO
DO JLEV=1,KLEV
  DO JROF=KIDIA,KFDIA
...
IF (LECT) THEN
  DO JLEV=1,KLEV
    DO JROF=KIDIA,KFDIA
      PTKEE(JROF,JLEV)=PTKEE0(JROF,JLEV)
      !ZTKE1(JROF,JLEV)=ZTKE1(JROF,JLEV)
    ENDDO
  ENDDO
ENDIF
IF (KTRAC > 0) THEN
  DO JTR=1,KTRAC
    DO JLEV=1,KLEV
      DO JROF=KIDIA,KFDIA
        ZTENC(JROF,JLEV,JTR)=PTENC(JROF,JLEV,JTR)
...
CALL VDFMAIN ( YDMODEL, CDCONF, &
  ...
  & ZUM1   , ZVM1   , ZTM1   , ZQM1   , ZLM1   , ZIM1   , ZAM1   , ZCM1   , ZTKE1,&
  & PAPHM1 , PAPM1  , PGEOM1 , PGEOH  , PGELAT , PTSKM1M, PTSAM1M, PWSAM1M, &
  & PSSRFL , PSLRFL , PEMIS  , PHRLW  , PHRSW  , &
  & PTSNOW , PTICE  , &
  & PHLICE , PTLICE , PTLWML , &
...
IF (KTRAC > 0) THEN
  DO JTR=1,KTRAC
    DO JLEV=1,KLEV
      DO JROF=KIDIA,KFDIA
        PTENC(JROF,JLEV,JTR)=ZTENC(JROF,JLEV,JTR)
      ENDDO
    ENDDO
  ENDDO
ENDIF
