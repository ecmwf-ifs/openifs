...
SUBROUTINE WVCOUPLE_UPDATE_GRIB_HANDLES(LDWINIT, KSTPW, PTSTEP, KSTEP, LDPPSTEPS, NGRIB_VERSION, KGRIB_HANDLE_FOR_WAM)
#ifdef WITH_WAVE
...
INTEGER(KIND=JPIM), INTENT(IN)    :: NGRIB_VERSION        ! Grib version to use (1 or 2 for now)
...
SELECT CASE (NGRIB_VERSION)
CASE (1)
  IGRIB_HANDLE = YGBH%NGRIB_HANDLE_GG
CASE (2)
  IGRIB_HANDLE = YGBH%NGRIB_HANDLE_GG2
CASE DEFAULT
  CALL ABOR1('WVCOUPLE_UPDATE_GRIB_HANDLES: Unknown NGRIB_VERSION')
END SELECT


IF (.NOT.LDWINIT) THEN
! INITIALISE GRIB HANDLE FOR WAM
IF(IGRIB_HANDLE < 0 ) THEN
CALL ABOR1("WVXF2GB: PROBLEM NGRIB_HANDLE_GG < 0 ")
ELSE
KGRIB_HANDLE_FOR_WAM=-99
CALL IGRIB_CLONE(IGRIB_HANDLE,KGRIB_HANDLE_FOR_WAM)
ENDIF
ENDIF
...
! AND EVERY HOUR (3 or 6) for long runs AFTER THAT 
! The wave model does not output more often than every hour
! (it will be longer if TSTEP is not a multiple of one hour)
