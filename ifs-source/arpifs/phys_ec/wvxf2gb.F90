...
USE MPL_MODULE        , ONLY : MPL_SEND, MPL_RECV, MPL_WAIT, MPL_WAITANY, &
...
INTEGER(KIND=JPIM) :: I, J, JF, IEND,&
 ...
 & JGL, JSTGLO, JROC, IPROC, ITAG, IREQ, IRREQ, ISREQ, IOFF
...
INTEGER(KIND=JPIM) :: ISENDREQ(NPROC), IRECVREQ(NPROC), KFROM(NPROC), KOFF(NPROC)
...
CALL WVCOUPLE_UPDATE_GRIB_HANDLES(LWINIT, NSTPW, PTSTEP, NSTEP, LPPSTEPS, NGRIB_VERSION, NGRIB_HANDLE_FOR_WAM)
CALL WVCOUPLE_UPDATE_GRIB_HANDLES(LWINIT, NSTPW, PTSTEP, NSTEP, LPPSTEPS, 2, NGRIB_HANDLE_FOR_WAM2)
  ...
  IF( LWVIN_UNINITIALISED )THEN
    ...
    DO JROC=1,NPROC-1
      ...
      CALL MPL_RECV(YDEWCOU%MWVIN_RECVCNT(IPROC),KSOURCE=NPRCIDS(IPROC),KTAG=ITAG,&
                  & KMP_TYPE=JP_NON_BLOCKING_STANDARD,KREQUEST=IRECVREQ(JROC),&
    ...
    ENDDO
    CALL MPL_WAIT(KREQUEST=IRECVREQ(1:NPROC-1),CDSTRING='WVXF2GB: WAIT RECV COUNT')
    ...
    DO JROC=1,NPROC-1
      ...
      ENDIF
    ENDDO
    CALL MPL_WAIT(KREQUEST=IRECVREQ(1:IRREQ),CDSTRING='WVXF2GB: WAIT RECV LIST')
    CALL MPL_WAIT(KREQUEST=ISENDREQ(1:ISREQ),CDSTRING='WVXF2GB: WAIT SEND LIST')
...
! remote tasks need
  ...
  IRECVLEN=YDEWCOU%MWVIN_SENDTOT*K_NFIELDS
  ALLOCATE(ZRECVBUF(IRECVLEN))
  ...
  DO JROC=1,NPROC-1
    ...
    IF( YDEWCOU%MWVIN_SENDCNT(IPROC) > 0 )THEN
      ...
      IOFF=IEND
      KOFF(IRREQ)=IOFF
      IEND=IEND+YDEWCOU%MWVIN_SENDCNT(IPROC)*K_NFIELDS
      CALL MPL_RECV(ZRECVBUF(IOFF+1:IEND),KSOURCE=NPRCIDS(IPROC),KTAG=ITAG,&
                  & KMP_TYPE=JP_NON_BLOCKING_STANDARD,KREQUEST=IRECVREQ(IRREQ),&
...
! Now copy data that was located on this processor
  ...
  DO IREQ=1,IRREQ
    CALL MPL_WAITANY(KREQUEST=IRECVREQ(1:IRREQ),KINDEX=I,CDSTRING='WVXF2GB: WAIT FOR ANY RECV')
    IPROC=KFROM(I)
    IOFF=KOFF(I)
    IF( LLWVDEBUG )THEN
      DO JF=1,K_NFIELDS
        DO J=1,YDEWCOU%MWVIN_SENDCNT(IPROC)
          IF( P_FIELDS(YDEWCOU%MWVIN_SENDIND(YDEWCOU%MWVIN_SENDOFF(IPROC)+J),JF) /=&
            & ZRECVBUF(IOFF+J+YDEWCOU%MWVIN_SENDCNT(IPROC)*(JF-1)) )THEN
      ...
      DO JF=1,K_NFIELDS
        DO J=1,YDEWCOU%MWVIN_SENDCNT(IPROC)
          P_FIELDS(YDEWCOU%MWVIN_SENDIND(YDEWCOU%MWVIN_SENDOFF(IPROC)+J),JF)=&
           &ZRECVBUF(IOFF+J+YDEWCOU%MWVIN_SENDCNT(IPROC)*(JF-1))
       ENDDO
      ENDDO
    ENDIF
  ENDDO
