...
!        R. Hogan  Apr  2022 Added capability to mix prognostic and climatological aerosol
...
USE YOEAERRADDESC, ONLY : IPROGNOSTIC, ICLIMATOLOGICAL
...
REAL(KIND=JPRB) :: ZQAERO (YDMODEL%YRML_PHY_RAD%YRERAD%NRPROMA,KLEV,YDMODEL%YRML_PHY_RAD%YRERAD%YAER_RAD_DESC%NCLIMATOLOGICAL)
...
INTEGER(KIND=JPIM) :: IMU0,IAMU0,IEMISS,ITS,ISLM,IBAS,ITOP,&
 ...
 & IRRA,IDP,IOZ,IHTI,IHPR,IFRSO,ISWFC,IFRTH,ILWFC,IAE,IPAE,ICAE, &
 & IAER,IPROGAERO,IAERO,IPERT,&
...
! Number of prognostic aerosol species in radiation; total number of
! aerosol species in radiation
INTEGER(KIND=JPIM) :: NPROGAER, NRADAER
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDGEM=>YDGEOMETRY%YRGEM, YDMP=>YDGEOMETRY%YRMP, YDGSGEOM_NB=>YDGEOMETRY%YRGSGEOM_NB, &
 & YDHSLMER=>YDMODEL%YRML_DYN%YRSLINT%YRHSLMER, YDLAP=>YDGEOMETRY%YRLAP, YDCSGLEG=>YDGEOMETRY%YRCSGLEG, &
 & YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY, YDEPHLI=>YDMODEL%YRML_PHY_SLIN%YREPHLI, &
 & YDERDI=>YDMODEL%YRML_PHY_RAD%YRERDI, YDERAD=>YDMODEL%YRML_PHY_RAD%YRERAD, &
 & YDPHY3=>YDMODEL%YRML_PHY_MF%YRPHY3, YDRIP=>YDMODEL%YRML_GCONF%YRRIP,  YDCOMPO=>YDMODEL%YRML_CHEM%YRCOMPO, &
 & YDESWRT=>YDMODEL%YRML_PHY_RAD%YRESWRT, YDCHEM=>YDMODEL%YRML_CHEM%YRCHEM, &
...
ASSOCIATE(NACTAERO=>YGFL%NACTAERO, NAERO=>YGFL%NAERO, YO3=>YGFL%YO3, NRESOL=>YDDIM%NRESOL, NSMAX=>YDDIM%NSMAX, &
 ...
 & RMD=>YRCST%RMD, RMO3=>YRCST%RMO3, RPI=>YRCST%RPI, &
...
! RJH 04/2022: Now the radiatively active prognostic aerosols are
! determined by the YAER_RAD_DESC structure
NPROGAER    = YAER_RAD_DESC%NPROGNOSTIC
NRADAER     = YAER_RAD_DESC%NAEROSOL
LLACTAERO   = (NRADAER > 0)
...
IPROGAERO=INDRAD(INEXT,NPROGAER*KLEV,.NOT.LRAYFM .AND. LLACTAERO)
...
IAERO = INDRAD(INEXT,NRADAER*KLEV,.TRUE.)
...
IF( LLDEBUG )THEN
  ...
  WRITE(NULOUT,'("RADINTG: IPROGAERO  =",I0)')IPROGAERO
...
!-- configuration with prognostic aerosols in radiation
  IF( .NOT.LRAYFM .AND. NACTAERO > 0) THEN
...
!-- debug
    ...
    ! RJH 04/2022: PAERO contains all prognostic aerosols - we copy
    ! into the interpolation array only those that are to be
    ! radiatively interactive according to YAER_RAD_DESC
    DO JAERO = 1,NPROGAER
      IAE=(JAERO-1)*KLEV
      ZRGP(1:IL,IPROGAERO+IAE:IPROGAERO+IAE+KLEV-1,IB) &
           &  = MAX(REPSCAER, PAERO(IBEG:IEND,1:KLEV,YAER_RAD_DESC%PROG_INDEX(JAERO)))
    ENDDO
...
CALL GSTATS(1209,0)
!$OMP  PARALLEL DO SCHEDULE(DYNAMIC,1)&
!$OMP& PRIVATE(JSTGLO,J,JLEV,JSW,JL,IAE,IPAE,ICAE,IBEG,IEND,I,I1,I2,IL,IB,LLDOLAST,&
...
!!      ZRGP(1:IL,IAER+I+6-1,IB) = RCAEROS
  ...
  ! RJH 04/2022: Add the prognostic and diagnostic aerosols to the
  ! full array to be sent to the radiation scheme
  IPAE = 0 ! Offset for next prognostic aerosol species
  ICAE = 1 ! Number of next climatological aerosol species
  DO JAERO=1,YAER_RAD_DESC%NAEROSOL
    IAE = (JAERO-1)*KLEV ! Offset for next aerosol species
    IF (YAER_RAD_DESC%DATA(JAERO)%NSOURCE == IPROGNOSTIC) THEN
      ZRGP(1:IL,IAERO+IAE:IAERO+IAE+KLEV-1,IB) = ZRGP(1:IL,IPROGAERO+IPAE:IPROGAERO+IPAE+KLEV-1,IB)
      IPAE = IPAE + KLEV
    ELSEIF (YAER_RAD_DESC%DATA(JAERO)%NSOURCE == ICLIMATOLOGICAL) THEN
      ! Climatological aerosol; note that currently RADACT only places
      ! parametric aerosols (i.e. stratosphere and troposphere
      ! "background" aerosols) in the old Tegen arrays, and these are
      ! copied over (scaled by the mass extinction coefficient) in
      ! RADIATION_SCHEME, so for the moment ZQAERO only contains
      ! climatological aerosols
      ZRGP(1:IL,IAERO+IAE:IAERO+IAE+KLEV-1,IB) = ZQAERO(1:IL,1:KLEV,ICAE)
      ICAE = ICAE+1
    ELSE
      ! Parametric species set to zero
      ZRGP(1:IL,IAERO+IAE:IAERO+IAE+KLEV-1,IB) = 0.0_JPRB
    ENDIF
    IF (YAER_RAD_DESC%DATA(JAERO)%SCALING /= 1.0_JPRB) THEN
      ZRGP(1:IL,IAERO+IAE:IAERO+IAE+KLEV-1,IB) &
           &  = ZRGP(1:IL,IAERO+IAE:IAERO+IAE+KLEV-1,IB) * YAER_RAD_DESC%DATA(JAERO)%SCALING
    ENDIF
  ENDDO
...
!*         4.2     CALL TO ACTUAL RADIATION SCHEME
  IF (YDERAD%LUSEPRE2017RAD) THEN
    ...
    ELSE
      ...
      CALL RADLSWR&
     & (YDGEOMETRY%YRDIMV, YDMODEL, 1 , IL , NRPROMA , KLEV , NRADAER ,&
     & ZRII0,&
  ...
  ELSE
    ...
    CALL RADIATION_SCHEME &
     & (YDMODEL, 1, IL, NRPROMA, KLEV, NRADAER, &
     &  ZRII0, &
