...
ASSOCIATE(NTILES=>YDDPHY%NTILES, &
 ...
 & RG=>YDCST%RG, RLVTT=>YDCST%RLVTT, RLSTT=>YDCST%RLSTT, RTT=>YDCST%RTT, RDAY=>YDCST%RDAY, &
...
IF (LERADS) THEN
  ...
  CALL SURFRAD(YDSURF=YSURF,KDD=IDD,KMM=IMM,KMON=ILMON,KSECO=ISEC,&
   ...
   & PALCOEFF=PALCOEFF,PCUR=PCUR,PCVH=PCVH,&
...
IFLAG=2
CALL SATUR  (YDTHF, YDCST, KIDIA , KFDIA , KLON  , KTDIA , KLEV, YDMODEL%YRML_PHY_SLIN%YREPHLI%LPHYLIN, &
