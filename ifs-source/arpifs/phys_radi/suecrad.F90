...
!        L Descamps Feb 2020 : hard-coded values transformed into parameters (RRCADECOR,RCBDECOR,RATSEA,RRATLAND,RRATDRI,FRACDICE)
!        R Forbes  (Nov 2020)  Changed default LINTERINCLOUDMEAN to F as radintg bug now fixed
!        R J Hogan (Jun 2021)  Added NSWGASOPTICS and NLWGASOPTICS
...
!        Y. Bouteloup (Jan 2022) : Use of EcRad in Arpege/Arome (call to setup_radiation_scheme if LMPHYS)
!        R J Hogan (Jul 2023)  Only calculate visibility (LAERVISI) if radiation scheme is on (LERADI)
! End Modifications
!-------------------------------------------------------------------------------
...
USE YOMCT0            , ONLY : LECMWF, LALLOPR, LARPEGEF, L_OOPS, NCONF
...
USE YOEAERC_TEGEN     , ONLY : LVOLCDATA, CLISTSO4, CVOLCDATA
...
#include "su_radiation_scheme.intfb.h"

!-------------------------------------------------------------------------------

LOGICAL, POINTER :: LERAD1H, LEPO3RA, LONEWSW, LCCNL, LCCNO, LRAYL, LRRTM,&
 & LSRTM, LHVOLCA, LDIFFC, LNOTROAER, LETRACGMS, LAERCLIM,&
 ...
 & LAER3D, LINTERPINCLOUDMEAN, LSPECTRALSOLARCYCLE
...
INTEGER(KIND=JPIM), POINTER :: NICEOPT, NLIQOPT, NSWICEOPT, NLWICEOPT, NSWLIQOPT,&
 ...
 & NAERMACC, NUV, NUVTIM, NRADUV, KMODTS, NSWSOLVER, NLWSOLVER, &
 & NO3CMIP, NGHGCMIP, NRCP, NCMIPFIXYR, &
 & NLWSCATTERING, NSOLARSPECTRUM, NSWWVCONTINUUM, NDUMPBADINPUTS, &
 & NCLOUDOVERLAP, NDUMPINPUTS, NSWGASOPTICS, NLWGASOPTICS
...
CHARACTER(LEN=256), POINTER :: CAERCLIMFILE
CHARACTER(LEN=256), POINTER :: CAEROPTICSMODEL_DD, CAEROPTICSMODEL_BC
CHARACTER(LEN=256), POINTER :: CAEROPTICSMODEL_OM, CAEROPTICSMODEL_SU
...
ASSOCIATE(YO3=>YGFL%YO3, NACTAERO=>YGFL%NACTAERO, NDGENG=>YDDIM%NDGENG, NDGENL=>YDDIM%NDGENL, NDGLG=>YDDIM%NDGLG, &
 ...
 & CVDAESU=>YDERAD%CVDAESU, LOPTRPROMA=>YDERAD%LOPTRPROMA, YAER_RAD_DESC=>YDERAD%YAER_RAD_DESC, &
 ...
 & RADSN=>YDPARAR%RADSN, RCADECOR=>YDERAD%RCADECOR, RCBDECOR=>YDERAD%RCBDECOR, RFACDICE=>YDERAD%RFACDICE, &
 & RRATLAND=>YDERAD%RRATLAND, RRATSEA=>YDERAD%RRATSEA, RRATDRI=>YDERAD%RRATDRI)
...
NSWGASOPTICS    => YDERAD%NSWGASOPTICS
NLWGASOPTICS    => YDERAD%NLWGASOPTICS
...
LSPECTRALSOLARCYCLE => YDERAD%LSPECTRALSOLARCYCLE
...
CAERCLIMFILE    => YDERAD%CAERCLIMFILE

CAEROPTICSMODEL_DD => YDERAD%CAEROPTICSMODEL_DD
CAEROPTICSMODEL_SU => YDERAD%CAEROPTICSMODEL_SU
CAEROPTICSMODEL_BC => YDERAD%CAEROPTICSMODEL_BC
CAEROPTICSMODEL_OM => YDERAD%CAEROPTICSMODEL_OM
...
! Select gas optics scheme in shortwave and longwave.  If both are 0
! then use classic RRTMG; otherwise select an ecCKD gas optics model
! with this number of g-points (if available), noting that SW and LW
! must either both be RRTMG or both ecCKD.  A value of -1 uses the
! default number of g points for ecCKD.
NSWGASOPTICS = 0
NLWGASOPTICS = 0
...
!--------------------------------------------------
! Coeff for dispersion of the droplet size distribution over sea and land
RRATSEA=0.77_JPRB
RRATLAND=0.69_JPRB
! Coeff in adjustment to Martin_et_al(1994) effective radius of particles
RRATDRI=0.222_JPRB
! multiplication factor for ice particle effective radius in RP scheme
RFACDICE=1.0_JPRB
!--------------------------------------------------
...
NDECOLAT=2
!------------------------------------
! if NDECOLAT=2 two coef for the computation of decorrelation height
RCADECOR=0.75_JPRB
RCBDECOR=2.149_JPRB
!------------------------------------
!- RRTM as LW scheme
...
!RCLOUD_FRAC_STD = 0.75_JPRD ! Shonk et al. (QJRMS 2010)
...
! Preferred aerosol optics models. Note that after possible override
! from the NAERAD namelist, the preferred models are used and printed
! in SU_RADIATION_SCHEME.
IF (NACTAERO > 0 .AND. YDEAERATM%LAERRRTM) THEN
  ! Radiatively active prognostic aerosols used by CAMS
  CAEROPTICSMODEL_DD = 'CompositePhiSca'
  CAEROPTICSMODEL_OM = 'Brown2018'
  CAEROPTICSMODEL_SU = 'GACP-NewPSD'
  CAEROPTICSMODEL_BC = 'Bond2006'
ELSE
  ! Climatological aerosols used in NWP
  CAEROPTICSMODEL_DD = 'Woodward2001'
  CAEROPTICSMODEL_OM = 'OPAC'
  CAEROPTICSMODEL_SU = 'GACP'
  CAEROPTICSMODEL_BC = 'OPAC'
ENDIF
...
RAESHSS=1000._JPRD
RAESHDU=3000._JPRD !redefined in updacaec when LDUSEASON=T.
RAESHOM=2000._JPRD
RAESHBC=1000._JPRD
RAESHSU=4000._JPRD
...
! LAERVISI: compute the visibility using climatological or prognostic
! aerosols (if active) in RADVIS_LAYER called from CALLPAR; requires
! optical properties from the radiation scheme so can only be used if
! the radiation scheme is run (LERADI==.true.)
LAERVISI = LERADI
...
LLDEBUG=.FALSE.
LEDBUG=.FALSE.
NRADINT=3 ! Cubic interpolation, 12-point stencil
!NRADINT=0 ! Radiation on model grid (no interpolation)
NRADRES=0
...
! With the ecCKD gas optics scheme we use the Coddington solar
! spectrum, but the following option allows it to vary through the
! solar cycle, i.e. more UV at solar maximum
LSPECTRALSOLARCYCLE = .FALSE.
...
IF (CSOLARIRRADIANCEFILE == ' ') THEN
  ! From 49r1 the file also contains the solar cycle number, which can
  ! be used to modify the solar spectrum according to the time in the
  ! solar cycle
  CSOLARIRRADIANCEFILE = 'total_solar_irradiance_CMIP6_49r1.nc';
ENDIF
...
IF (CAERCLIMFILE == ' ') THEN
  IF (LAER3D) THEN
    ! 3D default file: mass mixing ratio versus lon,lat,lev (and
    ! month)
    CAERCLIMFILE = 'aerosol_cams_climatology_43r3_v2_3D.nc'
    ! 4D default file: mass mixing ratio versus lon,lat,lev, and for
    ! some species year (as well as month)
    !CAERCLIMFILE = 'aerosol_cams_climatology_49r1_v2_4D.nc'
  ELSE
    ! 2D default file: column mass versus lon,lat (and month)
    CAERCLIMFILE = 'aerosol_cams_climatology_43R3a.nc'
  ENDIF
ENDIF
...
IF (LRRTM) THEN
  ...
  ! Get data directory for RRTM
  CALL GET_ENVIRONMENT_VARIABLE("DATA",CLZZZ)
  IF (CLZZZ /= " ") THEN
    CLF1 = TRIM(CLZZZ) // "/ifsdata"
  ELSE
    WRITE(UNIT=KULOUT,FMT='(A)') 'Warning: DATA environment variable not set: assuming RRTM files in current working directory'
    CLF1 = "."
  ENDIF
  
! Read the absorption coefficient data and reduce from 256 to 140 g-points

  CALL RRTM_INIT_140GP(CLF1)
...
IF (LSRTM) THEN
  NTSW=14
  ISW =14
  CALL SRTM_INIT(CLF1,NSWWVCONTINUUM)
  CALL SUSRTAB
  CALL SUSRTAER
  CALL SUSRTCOP
...
IF (LOUTPUT) THEN
  ...
  WRITE(UNIT=KULOUT,FMT='('' LRRTM = '',L5&
   ...
   & LSPECTRALSOLARCYCLE, NSWWVCONTINUUM, NDUMPBADINPUTS, NDUMPINPUTS, &
  ...
  IF (CAERCLIMFILE /= ' ') THEN
    WRITE(UNIT=KULOUT,FMT='('' CAERCLIMFILE =         '',A)') TRIM(CAERCLIMFILE)
  ENDIF
  ...
  IF (NAERMACC /= 0) THEN
  WRITE(UNIT=KULOUT,FMT='('' LAERADCLI= '',L1&
   & )')&
   & LAERADCLI
  ...
  IF (.NOT. LUSEPRE2017RAD) THEN
    ...
    WRITE(UNIT=KULOUT,FMT='('' NLwGasOptics    = '',I3)') NLWGASOPTICS
    WRITE(UNIT=KULOUT,FMT='('' NSwGasOptics    = '',I3)') NSWGASOPTICS
...
IF (.NOT. LUSEPRE2017RAD .AND. (LERADI .OR. LMPHYS)) THEN
  ...
  ! optics) but rather the ecRad scheme. Please note that NAERMACC may
  ! be changed from 0 to 1 in this routine.
  CALL SU_RADIATION_SCHEME(YDMODEL, YDERDI,YDEAERATM,YDEPHY,YDERAD,YDRADIATION,LOUTPUT)
ENDIF
...
IF (NOZOCL==1) THEN
  ...
  ELSEIF (YDRIP%YRECMIP%NO3CMIP /= 0) THEN
    CALL SUECOZV(YDRIP%YRECMIP,INT(YDRIP%GET_YEAR(YDMODEL%YRML_DYN%YRDYNA%LTWOTL),KIND=JPIM))
  ELSE 
...
! Note that YAER_RAD_DESC is modified in SU_RADIATION_SCHEME
IF (YAER_RAD_DESC%NCLIMATOLOGICAL > 0) THEN
  ! Read aerosol climatology from the specified file, requiring
  ! YAER_RAD_DESC to have been set in su_radiation_scheme.F90
  CALL YDRIP%YREAERC%SETUP(LAER3D, YAER_RAD_DESC, TRIM(CAERCLIMFILE))
  ! Set scale heights of climatological aerosols by type
  CALL YDRIP%YREAERC%SET_SCALE_HEIGHT('SS',RAESHSS)
  CALL YDRIP%YREAERC%SET_SCALE_HEIGHT('OM',RAESHOM)
  CALL YDRIP%YREAERC%SET_SCALE_HEIGHT('BC',RAESHBC)
  CALL YDRIP%YREAERC%SET_SCALE_HEIGHT('SU',RAESHSU)
  ! Crude representation of the seasonal cycle of dust scale height,
  ! replicated in updacaec.F90
  CALL YDRIP%YREAERC%SET_VARIABLE_SCALE_HEIGHT('DD', &
       & [ 1000.0_JPRB, 1000.0_JPRB, 2000.0_JPRB, 2000.0_JPRB, 2000.0_JPRB, 3000.0_JPRB, &
       &   3000.0_JPRB, 2000.0_JPRB, 2000.0_JPRB, 2000.0_JPRB, 1000.0_JPRB, 1000.0_JPRB])
ENDIF
