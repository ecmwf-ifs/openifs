...
SUBROUTINE SUSWN(YDESWRT,YDERAD,KTSW,KSW)
...
TYPE(TESWRT)      ,INTENT(INOUT) :: YDESWRT
...
ASSOCIATE(LONEWSW=>YDERAD%LONEWSW, LRAYL=>YDERAD%LRAYL, NTSW=>YDERAD%NTSW, &
 & APAD=>YDESWRT%APAD, BPAD=>YDESWRT%BPAD, RRAY=>YDESWRT%RRAY, &
 & RPDH1=>YDESWRT%RPDH1, RPDU1=>YDESWRT%RPDU1, RPNH=>YDESWRT%RPNH, RPNU=>YDESWRT%RPNU, &
 & RSWCE=>YDESWRT%RSWCE, RSWCP=>YDESWRT%RSWCP, RTDH2O=>YDESWRT%RTDH2O, RTDUMG=>YDESWRT%RTDUMG, &
 & RTH2O=>YDESWRT%RTH2O, RTUMG=>YDESWRT%RTUMG, D=>YDESWRT%D, REXPO3=>YDESWRT%REXPO3, RADJUST=>YDESWRT%RADJUST, &
 & NEXPO3=>YDESWRT%NEXPO3)

WRITE(NULOUT,*) ' SUSWN CALLED - KTSW,KSW ',KTSW,KSW
ZTDHNIS = 0.450_JPRB
ZTDUNIS = 0.375_JPRB
ZTHNIS  = 273._JPRB
ZTUNIS  = 273._JPRB
...
IF (LONEWSW) THEN
  ...
  YDESWRT%RTDH2O = ZTDHNIS
  YDESWRT%RTDUMG = ZTDUNIS
  YDESWRT%RTH2O  = ZTHNIS
  YDESWRT%RTUMG  = ZTUNIS
ENDIF

YDESWRT%RPDH1=ZPDH2O+1.0_JPRB
YDESWRT%RPDU1=ZPDUMG+1.0_JPRB
ZH2O=1.0_JPRB/( 10._JPRB* (RG/RPLRG) * RPDH1 )
ZUMG=1.0_JPRB/( 10._JPRB* (RG/RPLRG) * RPDU1 )
YDESWRT%RPNU = ZUMG/(ZPRUMG**ZPDUMG)
YDESWRT%RPNH = ZH2O/(ZPRH2O**ZPDH2O)

IF(.NOT.ALLOCATED(YDESWRT%RSUN)) ALLOCATE (YDESWRT%RSUN(NTSW))
IF(.NOT.ALLOCATED(YDESWRT%RSUN2)) ALLOCATE (YDESWRT%RSUN2(NTSW))

YDESWRT%RSUN(:)=0.0_JPRB
YDESWRT%RSUN2(:)=0.0_JPRB
RSWCE(:)=0.0_JPRB
RSWCP(:)=0.0_JPRB
...
IF (KSW == 1) THEN
   YDESWRT%RSUN(1)=ZSUN1
   YDESWRT%RSUN2(1)=ZSUN1
...
ELSEIF (KSW == 2) THEN
  ...
  RADJUST=ZADJUS2
  DO JW=1,KSW
    YDESWRT%RSUN (JW)=ZSUN2(JW)
    YDESWRT%RSUN2 (JW)=ZSUN2(JW)
...
ELSEIF (KSW == 4) THEN
  ...
  RADJUST=ZADJUS4
  DO JW=1,KSW
    YDESWRT%RSUN (JW)=ZSUN4(JW)
    YDESWRT%RSUN2 (JW)=ZSUN4(JW)
...
ELSEIF (KSW == 6) THEN
  ...
  RADJUST=ZADJUS6
  DO JW=1,KSW
    YDESWRT%RSUN (JW)=ZSUN6(JW)
    YDESWRT%RSUN2(JW)=ZSUN2KM6(JW)
...
ELSEIF (KTSW == 14 .AND. KSW == 14) THEN
  WRITE (NULOUT,*) 'SUSWN: 14-SPECTRAL INTERVALS --> RRTM_SW'
  
ELSEIF (KSW == 1) THEN
  WRITE (NULOUT,*) 'SUSWN: WARNING : DUMMY INITIALIZATIONS FOR SINGLE SW INTERVAL'
  DO JW=1,KSW
!    RSUN (JW)=HUGE(1._JPRB)
!    RSUN2(JW)=HUGE(1._JPRB)
    RSWCE(JW)=HUGE(1._JPRB)
    RSWCP(JW)=HUGE(1._JPRB)
    DO JC3=1,3
      D(JW,JC3)=HUGE(1._JPRB)
    ENDDO
    DO JC6=1,6
      RRAY(JW,JC6)=HUGE(1._JPRB)
    ENDDO
    DO JI=1,3
      DO JJ=1,7
        APAD(JW,JI,JJ)=HUGE(1._JPRB)
        BPAD(JW,JI,JJ)=HUGE(1._JPRB)
      ENDDO
    ENDDO
  ENDDO
ELSE
  WRITE (NULOUT,*) 'SUSWN: NTSW, NSW: ',KTSW,KSW
