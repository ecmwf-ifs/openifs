...
SUBROUTINE POS(YDCST, YDQTYPE,YDNAMFPSCI,YDTFP,LDHPOS,YDFPVAB,YDGEOMETRY,YDSURF,&
 & YDML_GCONF,YDDYN,YDDYNA,YDPHY,KPROMA,KST,KND,&
...
REAL(KIND=JPRB) :: ZUH(KPROMA)                 ! Updraft helicity
...
! ==== INTEGER VARIABLES =============================================
INTEGER(KIND=JPIM) :: IHILEV, ILEVM, ILOLEV, ILOC, ILOCU, ILOCV, IPTR, ILPT, I, INIT0
...
#include "fpuh.intfb.h"

!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('POS',0,ZHOOK_HANDLE)
ASSOCIATE(YDVAB=>YDGEOMETRY%YRVAB,YDVETA=>YDGEOMETRY%YRVETA,   YDCSGLEG=>YDGEOMETRY%YRCSGLEG, &
 & NGLOBALAT=>YDGEOMETRY%YRMP%NGLOBALAT, YDDIMV=>YDGEOMETRY%YRDIMV, YGFL=>YDML_GCONF%YGFL, YDDIMF=>YDML_GCONF%YRDIMF, &
 & YDCVER=>YDGEOMETRY%YRCVER)
ASSOCIATE(LFPRH100=>YDNAMFPSCI%LFPRH100, FPRHMIN=>YDNAMFPSCI%FPRHMIN,                          &
 & FPRHMAX=>YDNAMFPSCI%FPRHMAX, RFPVCAP=>YDNAMFPSCI%RFPVCAP, NITERPV=>YDNAMFPSCI%NITERPV,   LFPISOPV=>YDNAMFPSCI%LFPISOPV,   &
 & RUH_LOWER_LIMIT=>YDNAMFPSCI%RUH_LOWER_LIMIT, RUH_UPPER_LIMIT=>YDNAMFPSCI%RUH_UPPER_LIMIT, &
 & LISOT_ABOVEG=>YDNAMFPSCI%LISOT_ABOVEG, CATIDX=>YDNAMFPSCI%CATIDX, &
 & NGHG=>YGFL%NGHG, NAEROUT=>YGFL%NAEROUT, NAERO=>YGFL%NAERO, NUVP=>YGFL%NUVP, NERA40=>YGFL%NERA40, &
 & NNOGW=>YGFL%NNOGW, NCHEM=>YGFL%NCHEM, NEDRP=>YGFL%NEDRP, NGFL_EXT=>YGFL%NGFL_EXT, NGFL_EZDIAG=>YGFL%NGFL_EZDIAG, &
 & YLRCH4=>YGFL%YLRCH4, LAERAOT=>YGFL%LAERAOT, LAERLISI=>YGFL%LAERLISI, &
 & RATM=>YDCST%RATM, RCPD=>YDCST%RCPD, RD=>YDCST%RD, RG=>YDCST%RG, RKAPPA=>YDCST%RKAPPA, &
 & RTT=>YDCST%RTT, RV=>YDCST%RV, &
 & YSD_XA=>YDSURF%YSD_XA, YSD_XAD=>YDSURF%YSD_XAD, YSD_XR=>YDSURF%YSD_XR, YSD_XRD=>YDSURF%YSD_XRD, &
 & YSD_DID=>YDSURF%YSD_DID,YSD_DI=>YDSURF%YSD_DI,   &
 & LEDR=>YDPHY%LEDR,&
...
CALL SUPTRPPGFL_POS(NEMIS3D,IPPGFL,YLPPIC%IEMIS3D)
...
LL_UH = YDQTYPE%LL(YDTFP%UH%ICOD)
...
LLGEO=LL_Z.OR.LL_MG.OR.LLAERO.OR.LL_PTB.OR.LL_HTB.OR.&
 & LL_HTPW.OR.LL_MOCO.OR.LL_HTPW1.OR.LL_HTPW2.OR.LL_TPW.OR.LL_CAT.OR.LL_UH
LLCTY=LL_VV.OR.LL_ETAD.OR.LL_CAT
LLWW=LL_VW.OR.LL_WWS.OR.LL_VD.OR.LL_UH
...
LLVOR0=LLPVO.OR.LL_VOR.OR.LL_ABS.OR.LL_STD.OR.LL_SHD.OR.LL_CAT.OR.LL_UH
...
IF (LLQDER .AND. .NOT.(YDGFL%LLQL.AND.YDGFL%LLQM) .AND. .NOT.YDDYNA%LSPRT) THEN
...
CALL GPHPRE(KPROMA,KFLEVG,KST,KND,YDVAB,YDCVER,ZPRESH,PXYB=ZXYB,PRESF=ZPRESF)
...
IF (LLDYN(1)) THEN
  ...
  IF(YDCVER%LVERTFE) THEN
    ...
    ENDDO
  ENDIF
  CALL GPGRXYB(KPROMA,KST,KND,KFLEVG,.FALSE.,YDVAB,YDCVER,ZSP0L,ZSP0M,ZXYB,ZXYBDER)
ENDIF

!*      2.1.3   COMPUTATION OF RT and grad(RT)

IF (LLDYN(1)) THEN
  CALL GPRT(YDDYNA%LSPRT,KPROMA,KST,KND,KFLEVG,RD,RV,&
   & ZR,PT,ZTT0L,ZTT0M,ZQT0L,ZQT0M,ZRT,ZRTL,ZRTM)
  IF (YDDYNA%L_RDRY_VD) THEN
    ZRDT(KST:KND,1:KFLEVG)=RD*PT(KST:KND,1:KFLEVG)
  ELSE
...
IF (LLDYN(1)) THEN
  IF (YDDYNA%LNHDYN) THEN
    ...
    CALL GNHPRE(YDGEOMETRY,YDDYNA%NPDVAR,KPROMA,KFLEVG,KST,KND,ZPDT0,ZPRESF,&
     & PNHPREF=ZNHPREF,PNHPPI=ZNHPPI,PRNHPPI=ZRNHPPI,PDEP=ZPDEP)
    ! total nh pressure at half level
    CALL GNHPREH(YDGEOMETRY,YDDYNA%NPDVAR,KPROMA,KFLEVG,KST,KND,ZPDT0,ZPRESH,&
    ...
    DO JLEV=1,KFLEVG
      ...
      ENDDO
    ENDDO
    CALL GNHGRPRE(YDGEOMETRY,YDDYNA%NPDVAR,KPROMA,KFLEVG,KST,KND,ZXYB(1,1,YYTXYBPP%M_RTGR),ZSP0L,ZSP0M,&
...
IF (LLDYN(2)) THEN
  ...
  ENDDO
  !! If LSACC=.T. the geop calc. is not strictly accurate, as should be calculated via GPGEO_SACC
  !! but for dignostics purposes, makes sense to output the true hyd. GEOP
  ...
  CALL GPGRGEO(YDGEOMETRY,KPROMA,KST,KND,KFLEVG,&
   ...
   & LDNHEE=YDDYNA%LNHDYN,PRNHPPI=ZRNHPPI,PQCHAL=ZQCHAL,PQCHAM=ZQCHAM)
ENDIF
...
IF (LLDYN(3)) THEN
  CALL GPCTY(YDGEOMETRY%YRVFE,YDGEOMETRY%YRCVER,KPROMA,KST,KND,KFLEVG,.FALSE.,YDVAB,YDVETA,&
...
IF (LLDYN(4)) THEN
  ...
  CALL GPHLWI(YDDIMV,KPROMA,KST,KND,ZXYB(1,1,YYTXYBPP%M_LNPR),&
       & ZXYB(1,1,YYTXYBPP%M_ALPH),ZUVH(1,1,YYTHWPP%M_WWI),LDVERINT=YDDYNA%LVEREGINT)
  ...
  ! REK : also needed to compute full level gw.
  LLVFE=.FALSE.
  ...
  ELSE
    CALL GPXX(YDGEOMETRY,KFLEVG,KPROMA,KST,KND,ZGPHL,ZGPHM,ZGPFL,ZGPFM,ZXYB(1,1,YYTXYBPP%M_LNPR),&
     ...
     & ZNHX,PNHPPI=ZNHPPI,LDVFE=LLVFE)
  ENDIF

  ! Vertical divergence:
  IF (YDDYNA%LNHDYN) THEN
    IF (YDDYNA%NVDVAR == 3) THEN
      ! PSVD contains 'dver'
  ...
  ELSE
    IF (YDDYNA%L_RDRY_VD) THEN
      ...
      DO JLEV=1,KFLEVG
        DO JROF=KST,KND
          ZDVER(JROF,JLEV)=(-ZR(JROF,JLEV)/RD)*(ZDIVT0(JROF,JLEV)&
  ...
  CALL GPGW(YDGEOMETRY,YDDYNA%LNHDYN,KFLEVG,KPROMA,KST,KND,LLGWF,LLGDWI,&
...
ELSEIF (CDCONF == 'H') THEN
  DO JLEVP=1,KOPLEV
    DO JROF=KST,KND
      ZGEOP(JROF,JLEVP)=RG*PXLEV(JLEVP)+POROG(JROF)
    ENDDO
  ENDDO
...
ELSEIF (CDCONF == 'F') THEN
  DO JLEVP=1,KOPLEV
    DO JROF=KST,KND
      ZGEOP(JROF,JLEVP)=RG*PXLEV(JLEVP)
    ENDDO
  ENDDO
  CALL FPPS(KPROMA,KST,KND,KFLEVG,KOPLEV,ZGEOPH,ZGEOP,PT,&
   & ZR,ZPRESH,PTSI,ZPRES)
...
ELSEIF (CDCONF == 'S') THEN
  ...
  IF (LLPOSVER) THEN
    ...
    ENDDO
  ELSE
    CALL PPLETA(YDFPVAB,YDCVER,KPROMA,KST,KND,KOPLEV,PXLEV,ZPRESH(1,KFLEVG),ZPRES,ZPH(1,1,1),ZPH(1,1,0))
  ENDIF
  DO JLEVP=1,KOPLEV
    DO JROF=KST,KND
      ZLNPRES(JROF,JLEVP)=LOG(ZPRES(JROF,JLEVP))
...
IF (LL_VD) THEN
  IF (CDCONF == 'S') THEN
    ...
    DO JLEVP=1,KOPLEV
      DO JROF=KST,KND
        ZPPMGDW(JROF,JLEVP)=-RG*(ZPPWVELH(JROF,JLEVP,1)&
    ...
    DO JLEVP=1,KOPLEV
      DO JROF=KST,KND
        ZPPMG(JROF,JLEVP)=ZPPPHI(JROF,JLEVP)+&
         & RCPD*PXLEV(JLEVP)*((ZPRES(JROF,JLEVP)/RATM)**RKAPPA)  
      ENDDO
    ENDDO
  ENDIF
ENDIF
...
IF (LL_VV) THEN
  IF (CDCONF /= 'M') THEN
    CALL PPVVEL(YDGEOMETRY%YRVETA,YDGEOMETRY%YRCVER,KPROMA,KST,KND,KFLEVG,KOPLEV,ILOLEV,KPPM,ILEVB,&
...
IF (LL_PTB .OR. LL_HTB .AND. CDCONF=='K') THEN
  ...
  IF (LL_HTB) THEN
    ...
    IF ( LISOT_ABOVEG ) THEN
      DO JLEVP=1,KOPLEV
        DO JROF=KST,KND
          IF ( ZPPHTB(JROF,JLEVP) < POROG(JROF)*Z1SRG ) THEN
...
IF (LL_CATTI1) THEN
  ...
  ELSE
      DO JLEVP=1,KOPLEV
         ILEVM=INT(PXLEV(JLEVP))
         ZPPCATTI1(KST:KND,JLEVP)=ZCATTI1(KST:KND,ILEVM)
      ENDDO
  ENDIF
ENDIF
IF (LL_EDRDC) THEN
  ...
  ELSE
      DO JLEVP=1,KOPLEV
         ILEVM=INT(PXLEV(JLEVP))
         ZPPEDRDC(KST:KND,JLEVP)=ZEDRDC(KST:KND,ILEVM)
      ENDDO
  ENDIF
ENDIF
...
IF (LL_ICING) THEN
  ...
  ELSE
      DO JLEVP=1,KOPLEV
         ILEVM=INT(PXLEV(JLEVP))
         ZPPICING(KST:KND,JLEVP)=ZICING(KST:KND,ILEVM)
      ENDDO
  ENDIF
ENDIF

!*      4.6.13  UPDRAFT HELICITY

IF (LL_UH) THEN
  CALL FPUH(KST,KND,KPROMA,KFLEVG,RUH_LOWER_LIMIT,RUH_UPPER_LIMIT,ZGEOPH,ZGWF,ZVORT0(1,1),ZUH)
ENDIF
...
IF (YDQTYPE%LL(I)) THEN
  ...
  ENDDO
  CALL FILL_PP2(KPROMA,KST,KND,I,ZPPLNSP,PGPP,PAUX)
ENDIF
...
I=YDTFP%HTPW2%ICOD
IF (YDQTYPE%LL(I)) CALL FILL_PP2(KPROMA,KST,KND,I,ZPPHTPW2,PGPP,PAUX)

I=YDTFP%TOPR%ICOD
IF (YDQTYPE%LL(I)) CALL FILL_PP2(KPROMA,KST,KND,I,ZPPECHOT,PGPP,PAUX)

I=YDTFP%UH%ICOD
IF (YDQTYPE%LL(I)) CALL FILL_PP2(KPROMA,KST,KND,I,ZUH,PGPP,PAUX)
