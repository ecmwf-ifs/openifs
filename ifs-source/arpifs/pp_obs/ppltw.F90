...
!    I. Etchevers : MArs 2022 : modified for single precision
!      -----------------------------------------------------------------
...
INTEGER(KIND=JPIM) :: ILEVT(KPROMA,2), ILEVB(KPROMA,2), INLEV(KPROMA)

INTEGER(KIND=JPIM) :: JROF, JLEV, ISTLEV, IENDLEV, IINCLEV, ILEVTOP

REAL(KIND=JPRB) :: ZDT, ZDIFF, ZPROD, ZDP, ZINCT, ZSECUR
...
ILEVT(KST:KND,:)=KFLEV
ILEVB(KST:KND,:)=KFLEV-1
INLEV(KST:KND)=0
LLDONE(KST:KND)=.FALSE.
ZSECUR=EPSILON(1.0_JPRB)*1000._JPRB

DO JLEV=ISTLEV,IENDLEV,IINCLEV
  DO JROF=KST,KND
    IF (.NOT.LLDONE(JROF)) THEN
      ZPROD=(PT(JROF,JLEV)-PXTEMP)*(PXTEMP-PT(JROF,JLEV-1))
      ZDIFF=(PT(JROF,JLEV)-PXTEMP)
      IF ((ZDIFF > -ZSECUR).AND.(ZPROD >-ZSECUR)) THEN
...
DO JROF=KST,KND
  ...
  ELSE
       ...
       IF (ABS(ZDT) < ZSECUR) THEN
          ZINCT=0.0_JPRB
          ZDT=1.0_JPRB
       ENDIF
       PXGEO(JROF)=PGEO(JROF,ILEVB(JROF,INLEV(JROF)))+ZINCT*ZDP/ZDT
  ENDIF
  PXGEO(JROF)=MAX(PXGEO(JROF),0.0_JPRB)
ENDDO
