...
!****  *PPTHPW*  CALCUL DE LA TEMPERATURE POTENTIELLE PSEUDO-ADIABATIQUE
!                DU THERMOMETRE MOUILLE.

!                COMPUTATION OF THE MOIST IRREVERSIBLE ADIABATIC
!                POTENTIAL TEMPERATURE.

!     Sujet.
!     ------
!     - ROUTINE ABSOLUMENT GENERALE CALCULANT LA TEMPERATURE ENTROPIQUE
!       EXPLICITEE CI-DESSUS QUELQUE SOIT L'ETAT "INPUT" DE L'AIR (P,T,
!       QV,QL,QI SONT INDEPENDANTS) ET QUE L'OPTION "LNEIGE=.T." AIT ETE
!       RETENUE OU PAS.
!     - FULLY GENERAL ROUTINE COMPUTING THE ABOVE-MENTIONED ENTROPIC
...
! PTHPW      : THETA PRIME W.
!            : MOIST IRREVERSIBLE ENTROPIC TEMPERATURE.
...
IF (LHOOK) CALL DR_HOOK('PPTHPW',0,ZHOOK_HANDLE)
ASSOCIATE(NBITER=>YDPHY%NBITER, LNEIGE=>YDPHY%LNEIGE, &
 & RCPD=>YDCST%RCPD, RCPV=>YDCST%RCPV, RKAPPA=>YDCST%RKAPPA, RETV=>YDCST%RETV, &
 & RCW=>YDCST%RCW, RCS=>YDCST%RCS, RLVTT=>YDCST%RLVTT, RLSTT=>YDCST%RLSTT, &
 & RTT=>YDCST%RTT, RATM=>YDCST%RATM, RESTT=>YDCST%RESTT, RALPW=>YDCST%RALPW, &
 & RBETW=>YDCST%RBETW, RGAMW=>YDCST%RGAMW, RALPS=>YDCST%RALPS, RBETS=>YDCST%RBETS, &
 & RGAMS=>YDCST%RGAMS, RALPD=>YDCST%RALPD, RBETD=>YDCST%RBETD, RGAMD=>YDCST%RGAMD, RV=>YDCST%RV, RD=>YDCST%RD)
...
!     SNOW OPTION DEPENDENT CALCULATIONS.
  ...
  IF (LNEIGE) THEN
    ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,ZPITER(JLON)-RATM))
  ELSE
    ZDELTA=0.0_JPRB
  ENDIF
  ...
  ZTITER(JLON)=RTT*(RATM/ZPITER(JLON))**ZKAPPA
ENDDO
...
DO JIT=1,NBITER
  DO JLON=KIDIA,KFDIA
    ...
    ZF=ZS2(JLON)-RCPD*LOG(ZTITER(JLON))+RD*LOG(RATM-ZEW)-ZRS*ZLSTC
    ZDF=-RCPD/ZTITER(JLON)-ZRS*((RATM/(RATM-ZEW))*ZLSTC*ZLH/(RV&
...
DO JLON=KIDIA,KFDIA
  PTHPW(JLON)=ZTITER(JLON)
ENDDO

END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('PPTHPW',1,ZHOOK_HANDLE)
