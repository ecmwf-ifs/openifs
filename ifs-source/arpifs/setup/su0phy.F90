...
!      R. El Khatib 20-Jul-2021 Remove test with NFPSURFEX to reset LMSE : 
!      from now on, one must not set LMSE=.T. (from the object model) if NFPSURFEX=1 (from fullpos objects)
!      M. Janiskova (May 2022): Add LEGWDGF2 switch for the full gwd in TL/AD
!      R. El Khatib 18-Jul-2022 LAPL_ARPEGE in YRPHY
!      F. Vana  Jan-2023: Cleaning and new option LPC_SPHY
! End Modifications
!-------------------------------------------------------------------------------
...
LOGICAL , POINTER ::  LWINDFARM
LOGICAL , POINTER ::  LVOLKER
INTEGER (KIND=JPIM) , POINTER :: NWINDFARMMODORO
...
LOGICAL, POINTER :: LWLCLHR, LOPTLWPR, LETRAJP, LETRAJPT, LERADI2, LERADS2, &
 ...
 & LEVDIF2, LEGWDG2, LEGWDGF2, LECUMF2, LECOND2, LEGWWMS2, LEQNGT2, LESURF2, &
 ...
 & LEVDIF, LEVGEN, LESSRO, LELAIV, LESN09, LESNML, LEURBAN, LECTESSEL, &
 ...
 & LBUD23, LPPTILES, LBUDCYCLE, LEXTRATEND, LEMETHOX, LVDFTRAC, LVDFNEWTRAC, LMFTRAC, & 
 ...
 & LNCLIN, LREGCL, LRCOEF, LTLADDIA, LGLOBRAD, LELIGHT, LEOBCMAX, LRAD_CLOUD_INHOMOG, LDIAGTURB_EC, &
 & LDIAGTURBGRAD_EC, LBLEND
...
ASSOCIATE(NSURFEXCTLMAX=>YDMSE%NSURFEXCTLMAX, NSURFEXCTL=>YDMSE%NSURFEXCTL, &
 ...
 & LGWADV=>YDDYNA%LGWADV,LPC_SPHY=>YDDYNA%LPC_SPHY)
...
LGCHECKNAN => YDPHY%LGCHECKNAN
LHLRADUPD => YDPHY%LHLRADUPD
LWINDFARM => YDPHY%LWINDFARM
LVOLKER => YDPHY%LVOLKER
NWINDFARMMODORO => YDPHY%NWINDFARMMODORO
...
! Simplified phys-dyn interface for early ICI steps
IF (LPC_SPHY .AND. .NOT.LSLPHY) THEN
  IERR=IERR+1
  WRITE(KULOUT,'(1X,A,I3,A)') ' ! SU0PHY: ERROR NR ',IERR,' !!!'
  WRITE(KULOUT,'(1X,A,A)') ' Simplified phys-dyn coupling in ICI scheme LPC_SPHY ',&
   &  'is only available with SLAVEPP (LSLPHY=true) and LEPHYS_NEW=T.'
ENDIF
...
WRITE(UNIT=KULOUT,FMT='('' LMPHYS= '',L5,'' LREASUR = '',L5,'' LAPL_ARPEGE = '',L5)') LMPHYS,LREASUR,LAPL_ARPEGE
...
WRITE(UNIT=KULOUT,&
  &FMT='('' LWINDFARM ='',L1,'' LVOLKER = '',L1, &
  &'' NWINDFARMMODORO = '',I2 )') LWINDFARM, LVOLKER, &
  & NWINDFARMMODORO
...
WRITE(UNIT=KULOUT,FMT=*) 'LGCHECKMV=',LGCHECKMV,' LGCHECKNAN=',LGCHECKNAN
...
WRITE(UNIT=KULOUT,FMT='('' LERADI = '',L5,'' LERADIMPL = '',L5&
 ...
 & )')&
 & LERADI,LERADIMPL,LESURF,LEVDIF,LEOCWA,LEOCCO,LEOCSA,LEOCLA,LECURR,LESURFTRAJ,&
...
WRITE(UNIT=KULOUT,FMT='('' LEMWAVE = '',L5)') LEMWAVE
...
IF (LETRAJP) THEN
  ...
  WRITE(UNIT=KULOUT,FMT='('' LEVDIF2 = '',L4&
   ...
   & ,'' LEQNGT2 = '',L4&
   & )')&
   & LEVDIF2,LEGWDG2,LEGWDGF2,LEGWWMS2,LREGWWMS,LEQNGT2
