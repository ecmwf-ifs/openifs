...
SUBROUTINE SUDYN_STABILITY(YDGEOMETRY,YDDYN,YDDYNA,KULOUT)
...
ASSOCIATE(YDDIMV=>YDGEOMETRY%YRDIMV,YDCVER=>YDGEOMETRY%YRCVER)
...
DO ITRUNC = 1000, 1000, 100
  ...
  DO IRES = 0, 10
    ...
    WRITE(KULOUT,'("T_STABILITY",";",2(I5,";"),6(F15.10,";"),5(F15.2,";"))') &
     & YDCVER%NVFE_TYPE, ITRUNC, YDCVER%RVFE_ALPHA, YDCVER%RVFE_BETA, MAXVAL(ZABSM), MINVAL(ZABSM), &
...
IF (YDDYNA%LNHDYN) THEN
  ...
  CALL SISEVE(YDGEOMETRY,YDDYN,YDDYNA,1,NFLEVG,ZID,ZLAPL,NFLEVG,LD_LSTAR=.TRUE.)
...
CALL GPHPRE(IPROMA,KFLEV,IPROMA,IPROMA,YDGEOMETRY%YRVAB,YDGEOMETRY%YRCVER,ZPRES,PXYB=ZXYB)
...
! * output arguments
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PMM(KDIM,KDIM)
call abor1("CALL SUNH_LINMOD in sudyn_stability should never be called - EXIT")
