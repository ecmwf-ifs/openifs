...
!OCL SCALAR
#endif
SUBROUTINE SUDYNA(YDGEOMETRY,YDDYNA,KULOUT)
...
!   F. Vana  16-Jan-2023: LPGFTEST
!   F. Vana  Jan-2023: LPC_SPHY
! End Modifications
!------------------------------------------------------------------
...
LOGICAL,POINTER :: LNHEE_REFINE_SILAPL,LVEREGINT,LNHEE_REFINE_GRP,LNHEE_REFINE_PREH_BBC
...
LOGICAL,POINTER :: LGRADSP,LPGFSF,LPGREUSE,LPGFTEST,L3DTURB,LSLDIA,LRPRSLTRJ
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDCVER=>YDGEOMETRY%YRCVER)
...
LPC_SPHY => YDDYNA%LPC_SPHY
LMIXETTLS => YDDYNA%LMIXETTLS
LMIXETTLS_PRINT => YDDYNA%LMIXETTLS_PRINT
RMIXNL_TRH => YDDYNA%RMIXNL_TRH
L_RDRY_VD => YDDYNA%L_RDRY_VD
LVEREGINT => YDDYNA%LVEREGINT
LNHEE_REFINE_SILAPL => YDDYNA%LNHEE_REFINE_SILAPL
LNHEE_REFINE_GRP => YDDYNA%LNHEE_REFINE_GRP
LNHEE_REFINE_PREH_BBC => YDDYNA%LNHEE_REFINE_PREH_BBC
...
LGRADSP=.FALSE.
LPGFSF=.FALSE.
! NOTE: Since CY49 the LPGREUSE=true option cannot be used in the non-linear
!       code. It is therefore important to strictly keep LPGREUSE=FALSE.
LPGREUSE=.FALSE.
LPGFTEST=.FALSE.
IF (LECMWF) THEN
  IF (LSLAG) THEN
...
ENDIF
IF(LNHDYN .AND. .NOT. YDGEOMETRY%LNONHYD_GEOM) THEN
  IERR=IERR+1
...
IF (LNHEE) THEN
  ...
  ENDIF
  IF (NVDVAR==5 .AND. YDCVER%LVFE_GW) THEN
     IERR=IERR+1
     WRITE(KULOUT,'(1X,A,I3,A)') ' ! SUDYNA: ERROR NR ',IERR,' !!!'
...
IF (LPGFSF.AND.(LNHQE.OR.LNHEE.OR.LSACC)) THEN
    IERR=IERR+1
    WRITE(KULOUT,'(1X,A,I3,A)') ' ! SUDYNA: ERROR NR ',IERR,' !!!'
...
IF (LPGFSF.AND.LPGFTEST) THEN
  ! Issue a warning
  WRITE(KULOUT,*) ' *****  LPGFSF test is being activated! ***** '
  WRITE(KULOUT,*) ' WARNING: This effectively sets LPGFSF=false. '
  WRITE(KULOUT,*) ' ******************************************** '
ENDIF

IF (LPGREUSE.AND.LPGFSF) THEN
  IERR=IERR+1
  WRITE(KULOUT,'(1X,A,I3,A)') ' ! SUDYNA: ERROR NR ',IERR,' !!!'
  WRITE(KULOUT,*) ' LPGREUSE=true is no longer available!'
ENDIF
...
IF(LNHX .AND. .NOT. YDGEOMETRY%LNHX_GEOM) THEN
  IERR=IERR+1
  WRITE(KULOUT,'(1X,A,I3,A)') ' ! SUDYNA: ERROR NR ',IERR,' !!!'
  WRITE (KULOUT,*) ' LNHX true REQUIRES LNHX_GEOM true'
ENDIF
...
!* --- Tests on LPC_FULL, LPC_SPHY and LPC_CHEAP keys ---
...
ENDIF
IF ( LPC_SPHY.AND..NOT.LPC_FULL ) THEN
  IERR=IERR+1
  WRITE(KULOUT,'(1X,A,I3,A)') ' ! SUDYNA: ERROR NR ',IERR,' !!!'
  WRITE(KULOUT,*) ' LPC_SPHY (simpler phys-dyn coupling in early iterations) HAS MEANING ONLY WITH PC SCHEME LPC_FULL'
ENDIF
IF ( LPC_SPHY.AND.LPC_CHEAP ) THEN
  IERR=IERR+1
  WRITE(KULOUT,'(1X,A,I3,A)') ' ! SUDYNA: ERROR NR ',IERR,' !!!'
  WRITE(KULOUT,*) ' LPC_SPHY (simpler phys-dyn coupling in early iterations) requires LPC_CHEAP=F '
ENDIF
...
IF ( LRUBC .AND. (YDCVER%LVERTFE .OR. YDCVER%NDLNPR /= 1) ) THEN
  IERR=IERR+1
  WRITE(KULOUT,'(1X,A,I3,A)') ' ! SUDYNA: ERROR NR ',IERR,' !!!'
  WRITE(KULOUT,*) ' LRUBC=T requires FD vertical discretisationTRUENDLNPR=1 '
ENDIF
...
LNHQE_C2=LNHQE.AND.(.NOT.YDCVER%LVERTFE).AND.(YDCVER%NDLNPR==2)
...
WRITE(UNIT=KULOUT,FMT='('' LPGFTEST = '',L2)') LPGFTEST
...
WRITE(UNIT=KULOUT,FMT='('' LPC_SPHY = '',L2)') LPC_SPHY
