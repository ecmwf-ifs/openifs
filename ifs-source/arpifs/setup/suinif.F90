...
SUBROUTINE SUINIF(YDGEOMETRY,YDGFL,YDSURF,YDSP,YDCFU,YDXFU,YDFIELD_ECV,YDMODEL,KFILE,LDRDGRIDSP,YDMCUF,KINITMONTH)
...
USE YOMCT0             , ONLY : NCONF, LFBDAP, LELAM, LIFSMIN, LECMWF,CNMEXP,L_OOPS
...
USE FIELD_CONTAINER_OPER_MOD, ONLY : FIELD_CONTAINER_READ_FILE,FIELD_CONTAINER_GPNORM
USE YOMJBECV           , ONLY : YRECVCONFIG, YRECVDATA
...
!*       2.    INITIALIZE ECV FROM INPUT FILE
!              -------------------------------

IF(LECV .AND. YRECVDATA%NVIDS_ECV > 0 .AND. ANY(KFILE==(/0, 1, 12/))) THEN
  IF(KFILE == 1) THEN
    CLFN=YRECVCONFIG%CFNECVFG//CNMEXP(1:4)//'IMIN'
  ELSEIF(KFILE == 0 .OR. KFILE == 12) THEN
    CLFN=YRECVCONFIG%CFNECVFG//CNMEXP(1:4)//'INIT'
  ENDIF
  IF (.NOT. L_OOPS .OR. KFILE /= 1) THEN
    WRITE(NULOUT,*) ' SUINIF: READING ECV FILE ',KFILE,CLFN
    CALL FIELD_CONTAINER_READ_FILE(YDGEOMETRY,YDFIELD_ECV,YRECVDATA%VIDS_ECV,CLFN)
    CALL FIELD_CONTAINER_GPNORM(YDGEOMETRY,YDFIELD_ECV,YRECVDATA%VIDS_ECV)
  ENDIF
ENDIF

!     ------------------------------------------------------------------

!*       3.    INITIALIZE GRID POINT FIELDS
!              ----------------------------
...
!*       4.    INITIALIZE CUMULATED FLUXES FROM INPUT FILE
!              -------------------------------------------
...
!*       5.    INITIALIZE INSTANTANEOUS FLUXES FROM INPUT FILE
!              -----------------------------------------------
