...
USE YOMMP0   , ONLY : NPROC, NPRGPNS, NPRGPEW, LMPOFF, NSPECRESMIN, NPRTRW, &
 ...
 & LSCMEC, NPRINTLEV, LOPT_SCALAR, LOPT_RS6K, SLOWRANKS_DELAY, SLOWRANKS_FACTOR
USE YOMMPI   , ONLY : MINTET   ,MREALT
USE YOMTAG   , ONLY : MT_DISTRIBUTED_VECTOR
USE MPL_MODULE, ONLY : MPL_BUFFER_METHOD, MPL_INIT, MPL_MYRANK, MPL_NUMPROC, &
 & MPL_GROUPS_CREATE, JP_BLOCKING_STANDARD, MPL_CART_COORDS
...
SLOWRANKS_DELAY=2._JPRD
SLOWRANKS_FACTOR=2.0_JPRD
!     ------------------------------------------------------------------
...
!     4.3: check MPL_NUMPROC and NPROC are consistent.

IF(MPL_NUMPROC /= NPROC) THEN
  CALL ABOR1(' SUMPINI: inconsistency between MPL_NUMPROC and NPROC, check NAMPAR0 is consistent with MPI launch')
ENDIF
