...
SUBROUTINE SUPTRGPPC(YDDIMV,YGFL,YDDYNA,LDSLPHY,KGPPC,&
 ...
 & KGPPCF_NHX,KGPPCF_UP,KGPPCF_VP,KGPPCF_TP,KGPPCF_GFLP,KGPPCF_SPPTCLEARP,KGPPCF_BBC,&
 & KFGPPC)
...
INTEGER(KIND=JPIM),INTENT(OUT)   :: KGPPCF_SPPTCLEARP
...
IF( YDDYNA%LPC_CHEAP ) THEN
  ...
  IF( LDSLPHY ) THEN
    ...
    KGPPCF_SPPTCLEARP = ITMP; ITMP = ITMP + NFLEVG  ! only useful with SPPT and rad. masking
    KGPPCF_GFLP= ITMP;    ITMP = ITMP + NDIMSLP*NFLEVG
    KFGPPC = KFGPPC+4*NFLEVG+NDIMSLP*NFLEVG
  ENDIF
