...
SUBROUTINE SUSLAD2(YDGEOMETRY,YDSLREP,YDAD,YDDYN)
...
IF (YDDYN%LSLDP_XYZ) IFLDS=IFLDS+3
...
DO JROF=1,NGPTOT
  ...
  ENDIF
  IF( YDDYN%LSLDP_XYZ )THEN
    ZSLBUF(YDAD%NSLCORE(JROF),IFLDS-2)=YDGSGEOM_NB%GEXCO (JROF)
    ZSLBUF(YDAD%NSLCORE(JROF),IFLDS-1)=YDGSGEOM_NB%GEYCO(JROF)
    ZSLBUF(YDAD%NSLCORE(JROF),IFLDS)  =YDGSGEOM_NB%GEZCO(JROF)
  ENDIF
ENDDO
IF (NPROC > 1) THEN
  LLINC=.FALSE.
...
IF (YDDYN%LSLDP_XYZ) THEN
  ALLOCATE(YDGSGEOMAD_NB%GEXCO    (NGPTOTAD))
  ALLOCATE(YDGSGEOMAD_NB%GEYCO    (NGPTOTAD))
  ALLOCATE(YDGSGEOMAD_NB%GEZCO    (NGPTOTAD))
ELSE
  ALLOCATE(YDGSGEOMAD_NB%GEXCO    (1))
  ALLOCATE(YDGSGEOMAD_NB%GEYCO    (1))
  ALLOCATE(YDGSGEOMAD_NB%GEZCO    (1))
ENDIF
...
DO JROF=1,NGPTOTAD
  ...
  ENDIF
  IF( YDDYN%LSLDP_XYZ )THEN
    YDGSGEOMAD_NB%GEXCO(JROF)=ZSLBUF(NADCORE(JROF),IFLDS-2)
    YDGSGEOMAD_NB%GEYCO(JROF)=ZSLBUF(NADCORE(JROF),IFLDS-1)
    YDGSGEOMAD_NB%GEZCO(JROF)=ZSLBUF(NADCORE(JROF),IFLDS)
  ENDIF
ENDDO
...
DO JKGLO=1,NGPTOTAD,NPROMA
  ...
  ENDIF
  IF (YDDYN%LSLDP_XYZ) THEN
    YDGEOMETRY%YRGSGEOMAD(IBL)%GEXCO =>YDGSGEOMAD_NB%GEXCO (JKGLO:JKGLO+IEND-1)
    YDGEOMETRY%YRGSGEOMAD(IBL)%GEYCO =>YDGSGEOMAD_NB%GEYCO (JKGLO:JKGLO+IEND-1)
    YDGEOMETRY%YRGSGEOMAD(IBL)%GEZCO =>YDGSGEOMAD_NB%GEZCO (JKGLO:JKGLO+IEND-1)
  ELSE
    YDGEOMETRY%YRGSGEOMAD(IBL)%GEXCO =>NULL()
    YDGEOMETRY%YRGSGEOMAD(IBL)%GEYCO =>NULL()
    YDGEOMETRY%YRGSGEOMAD(IBL)%GEZCO =>NULL()
  ENDIF
ENDDO
