...
INTEGER(KIND=JPIM) :: IFILE, I2D, JGFL, JK, I3D, ISPTOGP, IBOUND2D, JSP, IINCR
...
IF(IFILE /= 1 .OR. (IFILE == 1 .AND. .NOT.LTRAJHR)) THEN
  ...
  ALLOCATE(IGRIB2D(NS2D))
  ALLOCATE(IGRIB3D(NS3D+NUMFLDS))
  ALLOCATE(LLMISSING_3D(NFLEVG,NS3D+NUMFLDS))
  ...
  IF(.NOT.LLSPTOGP) THEN
    ZSPA2 => PSPA2(:,1:IBOUND2D)
    ZSPA3 => PSPA3
  ELSE
    ALLOCATE(ZSPA2(NSPEC2,I2D))
    ALLOCATE(ZSPA3(NFLEVL,NSPEC2,I3D+ISPTOGP))
  ENDIF
  ...
  CALL SETUP_IOREQUEST(YL_IOREQUEST,'SPECTRAL_FIELDS',LDGRIB=.TRUE.,KRESOL=NRESOL,&
   & KGRIB3D=IGRIB3D(1:I3D+ISPTOGP),KGRIB2D=IGRIB2D(1:I2D),&
   & LDINTERP=LLINCR)
