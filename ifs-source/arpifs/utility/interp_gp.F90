...
CASE(2)
  ...
  CALL GRID_BICUBIC(KDGLG_I,KLOENG_I,PLATIG_I,IGPTOTG_I,&
   ...
   & PGLFLDI,PGLFLDO)
CASE(3)
  WRITE(NULOUT,*) 'CASE3: ',KLEVEL,CLEVTYPE
  IF (KLEVEL==0 .AND. CLEVTYPE=='SFC') THEN
    WRITE(NULOUT,*) 'CASE3 interpol',KLEVEL,CLEVTYPE
  ...
  ELSEIF (KLEVEL>0 .AND. CLEVTYPE=='SFC') THEN
    IF (.NOT.LSNOWTRAJCONS.AND.&
      ...
      WRITE(NULOUT,*) 'CASE3 interpol snow ml with closest ',KPARAM,KLEVEL,CLEVTYPE
      CALL GRID_CLOSEST(KDGLG_I,KLOENG_I,PLATIG_I,IGPTOTG_I,&
       & KDGLG_O,KLOENG_O(1:KDGLG_O),PLATIG_O(1:KDGLG_O),IGPTOTG_O,&
       & PGLFLDI,PGLFLDO)
    ELSE
      WRITE(NULOUT,*) 'CASE3 interpol snow ml with conservative interp ',KPARAM,KLEVEL,CLEVTYPE
      CALL GRID_BICONSERV(PVAH,PVBH, KDGLG_I,KLOENG_I,PLATIG_I,IGPTOTG_I,&
       & KDGLG_O,KLOENG_O(1:KDGLG_O),PLATIG_O(1:KDGLG_O),IGPTOTG_O,&
       & PGLFLDI,PGLFLDO)
    ENDIF
 ELSEIF (KLEVEL>0) THEN
    ...
    IF (ALLOCATED(YRECVDATA%MGRIBS_ECV_FIDS)) THEN
      LLECV = ANY(KPARAM==YRECVDATA%MGRIBS_ECV_FIDS)
    ELSE
      LLECV = .FALSE.
    ENDIF

    IF (LLECV) THEN
      WRITE(NULOUT,*) 'CASE3 ECV ', KPARAM
      CALL GRID_BICONSERV(PVAH,PVBH, KDGLG_I,KLOENG_I,PLATIG_I,IGPTOTG_I,&
       & KDGLG_O,KLOENG_O(1:KDGLG_O),PLATIG_O(1:KDGLG_O),IGPTOTG_O,&
       & PGLFLDI,PGLFLDO)
    ELSEIF (IGPTOTG_I>IGPTOTG_O) THEN
