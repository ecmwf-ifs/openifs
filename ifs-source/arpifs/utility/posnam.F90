...
INQUIRE(KULNAM,NAME=CLFILE,OPENED=LLOPEN)
IF (.NOT. LLOPEN) THEN
  ! Value of CLFILE is not specified
  ! Next READ statement will automatically open fort.<KULNAM>
  WRITE(NULOUT, "('Unit ',I0,' is not connected')") KULNAM
  WRITE(CLFILE,'(A5,I0)') 'fort.', KULNAM
ENDIF

WRITE(NULOUT,"('Reading namelist ',A,' from file ',A)") CDNAML,TRIM(CLFILE)
