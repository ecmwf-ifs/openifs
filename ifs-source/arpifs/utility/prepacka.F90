...
ASSOCIATE(NHEADMAX=>YDFPOPH%NHEADMAX,CFPCA=>YDFPOPH%CFPCA,CFAMODEL=>YDFPOPH%CFAMODEL,LFAEXTERN=>YDFPOPH%LFAEXTERN)
!      -----------------------------------------------------------
...
DO JDOM = 1, KDOM
  IF (ANY (KMASK (:,JDOM) /= 0)) THEN
    CALL OPENCPFACTX (YDFACTX (JDOM),CFPCA(JDOM),CDMODEL=CFAMODEL(JDOM),LDEXTERN=LFAEXTERN(JDOM))
  ENDIF
ENDDO
