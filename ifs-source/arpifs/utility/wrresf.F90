...
INTEGER(KIND=JPIM) :: IDAY,IEND,IFCTIM,IHOUR,IINC,IMIN,INUM,&
 & IST,ITEMP,JLEV,JSTGLO,IBL,II,JBLK,ILEN_CPL,ILEN_IO,J2D,JPERT,JRF,IRF,&
...
IF (YDSPP_CONFIG%LSPP) THEN
  DO JPERT=1, YDSPP_CONFIG%SM%NACT
    ...
    DO JRF=1, YDSPP_CONFIG%SM%PN(JPERT)%NRF
      ...
      !
      CALL IO_PUT(YL_IOSTREAM, YL_IOREQUEST,PR2=YDSPP%SP_ARP(IRF)%SF%SP2D)
      IF (YDSPP_CONFIG%NPATFR/=1) THEN
        CALL IO_PUT(YL_IOSTREAM, YL_IOREQUEST,PR3=YDSPP%GP_ARP0(IRF)%GP2D)
        CALL IO_PUT(YL_IOSTREAM, YL_IOREQUEST,PR3=YDSPP%GP_ARP1(IRF)%GP2D)
        CALL IO_PUT(YL_IOSTREAM, YL_IOREQUEST,PR3=YDSPP%GP_ARP(IRF)%GP2D)
      ENDIF
    ENDDO
  ENDDO
ENDIF
