...
REAL(KIND=JPRB), ALLOCATABLE, SAVE :: ZCOMBUFS_A(:,:)
REAL(KIND=JPRB), ALLOCATABLE, SAVE :: ZCOMBUFR_A(:,:)
INTEGER(KIND=JPIM), SAVE :: INRECV_PREV=-1,INSEND_PREV=-1, IRECVCOUNT_PREV=-1,ISENDCOUNT_PREV=-1
...
IF ( .NOT. ALLOCATED(ZCOMBUFS_A) ) THEN
  ALLOCATE(ZCOMBUFS_A(-1:KSENDCOUNT,KNSEND))
  ...
  IF (KNSEND > 0 .AND. KSENDCOUNT >=-1) ZCOMBUFS_A(-1,1)=HUGE(1._JPRB)
ELSEIF ( KSENDCOUNT .NE. ISENDCOUNT_PREV .OR. KNSEND .NE. INSEND_PREV ) THEN
  DEALLOCATE(ZCOMBUFS_A)
  ALLOCATE(ZCOMBUFS_A(-1:KSENDCOUNT,KNSEND))
  ISENDCOUNT_PREV = KSENDCOUNT
  INSEND_PREV = KNSEND
  ! Now, force the OS to allocate this shared array right now, not when it starts to be used which is
...
IF ( .NOT. ALLOCATED(ZCOMBUFR_A) ) THEN
  ALLOCATE(ZCOMBUFR_A(-1:KRECVCOUNT,KNRECV))
  IRECVCOUNT_PREV = KRECVCOUNT
  INRECV_PREV = KNRECV
ELSEIF ( KRECVCOUNT .NE. IRECVCOUNT_PREV .OR. KNRECV .NE. INRECV_PREV ) THEN
  DEALLOCATE(ZCOMBUFR_A)
  ALLOCATE(ZCOMBUFR_A(-1:KRECVCOUNT,KNRECV))
  IRECVCOUNT_PREV = KRECVCOUNT
  INRECV_PREV = KNRECV
ENDIF
