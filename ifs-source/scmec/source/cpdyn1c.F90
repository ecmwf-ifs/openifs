...
SUBROUTINE CPDYN1C(YDDIMV, LDTWOTL,KFLEV  ,PDT    ,PRESF  ,PVVEL0 ,PRCORI &
         &,PUT0   ,PVT0   ,PTT0   ,PQT0   ,PTKET0, PAT0   ,PLT0   ,PIT0&
         &,PKAP   ,PUG0   ,PVG0   ,PETADOTDPDETA  ,PDELP9&
         &,PUADV  ,PVADV  ,PTADV  ,PQADV,PTKEADV&
...
REAL(KIND=JPRB) :: PTKET0  (YDDIMV%NFLEVG)
...
REAL(KIND=JPRB) :: ZTKEVADV(YDDIMV%NFLEVG)
...
! upwind differencing
  ...
  IF ( ZVERVEL(JLEV) >= 0.0 ) THEN   ! downward motion
   IF ( JLEV .NE. 1 ) THEN
    ...
    ZTKEVADV(JLEV) = ZVERVEL(JLEV) * ( PTKET0(JLEV) - PTKET0(JLEV-1) ) / PDELP9(JLEV)
  ...
  ELSE                               ! upward motion
   IF ( JLEV .NE. NFLEVG ) THEN
    ...
    ZTKEVADV(JLEV) = ZVERVEL(JLEV) * ( PTKET0(JLEV+1) - PTKET0(JLEV) ) / PDELP9(JLEV)
...
! for etadotdpdeta input (on half levels)
    ...
    DO JLEV=2,NFLEVG-1
      ...
      ZTKEVADV(JLEV)=PETADOTDPDETA(JLEV)*(PTKET0(JLEV+1)-PTKET0(JLEV))
      ZTKEVADV(JLEV)=ZTKEVADV(JLEV)+PETADOTDPDETA(JLEV-1)*(PTKET0(JLEV)-PTKET0(JLEV-1))
      ZTKEVADV(JLEV)=ZTKEVADV(JLEV)/(2.0*PDELP9(JLEV))
    ...
    ZTKEVADV(1)=0.5*(PETADOTDPDETA(1)*(PTKET0(2)-PTKET0(1)))/PDELP9(1)
    ...
    ZTKEVADV(NFLEVG)=0.5*(PETADOTDPDETA(NFLEVG-1)*(PTKET0(NFLEVG)-PTKET0(NFLEVG-1)))/PDELP9(NFLEVG)
...
! pure centered space differencing - for omega input on full levels
    ...
    DO JLEV=2,NFLEVG-1
      ZQVADV(JLEV) = PVVEL0(JLEV) * ( PQT0(JLEV+1) - PQT0(JLEV-1) ) &
                   & / ( 2.0 * PDELP9(JLEV) )      
      ZTKEVADV(JLEV) = PVVEL0(JLEV) * ( PTKET0(JLEV+1) - PTKET0(JLEV-1) ) &
                   & / ( 2.0 * PDELP9(JLEV) )      
    ...
    ZTKEVADV(1)      = PVVEL0(1) * ( PTKET0(2) - PTKET0(1) ) / ( 2.0 * PDELP9(1) )
    ...
    ZQVADV(NFLEVG) =  PVVEL0(NFLEVG) * ( PQT0(NFLEVG) - PQT0(NFLEVG-1) ) &
                   & / ( 2.0 * PDELP9(NFLEVG) )
    ZTKEVADV(NFLEVG) =  PVVEL0(NFLEVG) * ( PTKET0(NFLEVG) - PTKET0(NFLEVG-1) ) &
                   & / ( 2.0 * PDELP9(NFLEVG) )
...
! (this method adds a diffusion term to the centered space differencing)
  ...
  IF (LLAXMETHOD) THEN
    ...
    DO JLEV=2,NFLEVG-1
      ...
      ZTKEVADV(JLEV) = ZTKEVADV(JLEV) - &
                 & ( PTKET0(JLEV+1) - 2 * PTKET0(JLEV) + PTKET0(JLEV-1) ) / 2 / PDT
    ...
    ZTKEVADV(1)      = ZTKEVADV(1)      - ( PTKET0(2)       - PTKET0(1) )       / 2 / PDT
    ZTKEVADV(NFLEVG) = ZTKEVADV(NFLEVG) - ( PTKET0(NFLEVG-1) - PTKET0(NFLEVG) ) / 2 / PDT
...
!TKE
DO JLEV=1,NFLEVG
  PTKETEND(JLEV)=PTKEADV(JLEV)-ZTKEVADV(JLEV)
ENDDO
