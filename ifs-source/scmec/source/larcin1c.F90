...
SUBROUTINE LARCIN1C(YDGEOMETRY,YDMODEL,KFLEV,PLEV&
           &,PQSL,PTKESL,PTSL,PUSL,PVSL,PASL,PLSL,PISL&
           &,PQF,PTKEF,PTF,PUF,PVF,PAF,PLF,PIF,KWISA) !,PWRL0,PWF0)
...
REAL(KIND=JPRB) :: PTKESL (YDGEOMETRY%YRDIMV%NFLEVG)
...
REAL(KIND=JPRB) :: PTKEF (YDGEOMETRY%YRDIMV%NFLEVG)
...
LOGICAL :: LLQMQ, LLQMCLD, LLQMTKE, LLQM3DQ, LLQM3CLD

!     ------------------------------------------------------------------
#include "laitri1c.intfb.h"
#include "laitqm1c.intfb.h"
#include "laitbs1c.intfb.h"
...
LLQMTKE=.TRUE.  ! QM setting for TKE
LLQMCLD=.FALSE. ! QM setting for other cloud variables
LLQM3DQ =.FALSE.! conservative fixer for moisture
LLQM3CLD=.TRUE. ! conservative fixer for cloud variables
...
!*    LINEAR INTERPOLATIONS.
  ...
  CALL LAITLI1C(YDGEOMETRY%YRDIMV,KFLEV,ZDVER,ILEV,PTKESL,PTKEF)
...
!     - for the ice equation quantity.
  ...
  ELSEIF (LLQM3DQ) THEN
    CALL LAITBS1C(YDGEOMETRY%YRDIMV,KFLEV , ZVINTW ,ILEV , &
     &        YDMODEL%YRML_DYN%YRSLINT%YRVSLETA%VRDETAR,  PQSL,PQF )
  ELSE
    CALL LAITRI1C(YDGEOMETRY%YRDIMV,KFLEV , ZVINTW ,ILEV ,PQSL,PQF )
  ENDIF

  IF (LLQMTKE) THEN
    CALL LAITQM1C(YDGEOMETRY%YRDIMV,KFLEV , ZVINTW ,ILEV ,PTKESL,PTKEF )
  ELSE
    CALL LAITRI1C(YDGEOMETRY%YRDIMV,KFLEV , ZVINTW ,ILEV ,PTKESL,PTKEF )
  ENDIF
  ...
  ELSEIF (LLQM3CLD) THEN
    CALL LAITQM1C(YDGEOMETRY%YRDIMV,KFLEV , ZVINTW ,ILEV ,PASL,PAF )
    CALL LAITBS1C(YDGEOMETRY%YRDIMV,KFLEV , ZVINTW ,ILEV , &
     &        YDMODEL%YRML_DYN%YRSLINT%YRVSLETA%VRDETAR,  PLSL,PLF )
    CALL LAITBS1C(YDGEOMETRY%YRDIMV,KFLEV , ZVINTW ,ILEV , &
     &        YDMODEL%YRML_DYN%YRSLINT%YRVSLETA%VRDETAR,  PISL,PIF )
  ELSE
